...
func (ps *PubSub) watchForNewPeers(ctx context.Context) {
	...
	for _, pid := range ps.host.Network().Peers() {
		if !connectednessSupportsPubSub(ps.host.Network().Connectedness(pid)) {
			continue
		}
	...
	for ctx.Err() == nil {
		...
		case event.EvtPeerConnectednessChanged:
			// Relay-backed peers can become pubsub-usable through a limited
			// connection without reliably surfacing a fresh identify/protocol
			// update event afterwards. Treat connectedness changes as a retry
			// signal so we still attempt to open the pubsub stream.
			if connectednessSupportsPubSub(ev.Connectedness) {
				ps.notifyNewPeer(ev.Peer)
			}
			continue
		default:
			continue
