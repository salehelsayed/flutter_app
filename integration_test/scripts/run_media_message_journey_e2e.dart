...
Future<int> _runJourneyForDevice(_DeviceTarget device) async {
  _log(
    'RUN',
    'Starting media message journey E2E on ${device.name} (${device.id})',
  );
...
Future<void> main(List<String> args) async {
  ...
  _log(
    'ORCH',
    'Running media message journey E2E on ${targets.length} device(s): '
        '${targets.map((device) => '${device.name}:${device.id}').join(', ')}',
  );

  var failures = 0;
  for (final device in targets) {
    final exitCode = await _runJourneyForDevice(device);
    if (exitCode != 0) {
      failures += 1;
    }
  }
  ...
  _log('DONE', 'Media message journey E2E passed on all selected devices.');
}
