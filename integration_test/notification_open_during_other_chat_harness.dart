// Notification-open during other chat — role-dispatched two-device harness.
//
// Merged from the former
//   notification_open_during_other_chat_alice_harness.dart (receiver) +
//   notification_open_during_other_chat_bob_harness.dart   (sender)
// into a single target. `main()` reads the SMOKE_ROLE dart-define and
// dispatches to the alice (receiver) path or the bob (sender) path. Both
// harness bodies are preserved byte-for-byte below; only the shared
// signal-file helpers and dart-define constants are deduped.
//
...
// alice path; the bob side is intentionally minimal.
...
const _dbName = String.fromEnvironment(
  'E2E_DB_NAME',
  defaultValue: 'notif_open_during_other_chat.db',
);

// Canonical signal-file coordination (see integration_test/_support/
// signal_files.dart). Reproduces the former inline `_sig(name)` form
// `'$_sharedDir/notifopen_${_runId}_$name'` byte-for-byte: prefix
// 'notifopen_' + runId '${_runId}_'. The 300s default timeout the inline
// waiters used is passed explicitly at each call site (the canonical
// default is 120s).
final SignalDir _signals = SignalDir(
  dir: _sharedDir,
  prefix: 'notifopen_',
  runId: '${_runId}_',
  role: 'notif-open($_role)',
);

// ---------------------------------------------------------------------------
// Recording NotificationService — self-contained (alice path).
//
...
class _RecordingNotificationService implements NotificationService {
  ...
  Future<void> showMessageNotification({
    ...
    print(
      '[ALICE-NO-REC] showMessageNotification contactPeerId=$contactPeerId payload=$payload silent=$silent shown.length=${shown.length}',
    );
  }
...
// ---------------------------------------------------------------------------
// Navigator-backed test app + dispatch wiring (alice path).
//
// Goals (widened scope per the run-#5 finding that the simple-stub
// version did not reproduce the bug):
//   1. Use the production `buildConversationRoute` for conversation
...
class _RecordingNavigatorObserver extends NavigatorObserver {
  ...
  void _record(String op, Route<dynamic>? route, Route<dynamic>? prev) {
    ...
    print(
      '[ALICE-NO-NAV] +${dt}ms $op route=${route?.settings.name} prev=${prev?.settings.name}',
    );
  }
...
// ---------------------------------------------------------------------------
// Replicated route-target dispatch (alice path).
//
...
// `buildConversationRoute` wrapping `ConversationWired`. We
...
void _runAlice() {
  testWidgets(
    'Alice(notif-open during other chat) — repro',
    (tester) async {
      print('\n${'═' * 60}');
      ...
      final messageRepo = MessageRepositoryImpl(
        ...
        dbUpdateMessageStatus: (id, s) =>
            dbUpdateMessageStatus(stack.db, id, s),
        ...
        dbCountMessagesForContact: (p) =>
            dbCountMessagesForContact(stack.db, p),
      ...
      // Uses the production `buildConversationRoute` + real
      ...
      _signals.writeSignal('alice_in_user_c_chat');
      ...
      _signals.writeSignal('alice_backgrounded');
      ...
      print(
        '[ALICE-NO] Awaiting Bob send (notification gate paused, off-conv)',
      );
      ...
      while (DateTime.now().isBefore(deadline)) {
        if (recording.shown.length > preSendShown) {
          ...
          if (last.contactPeerId == bobPeerId) {
            ...
            _signals.writeSignal('alice_notification_received');
            received = true;
            break;
          }
        }
      ...
      if (tapPayload.isEmpty) {
        throw StateError('Notification payload was empty; cannot simulate tap');
      }
      ...
      _signals.writeJson('alice_verdict', verdict);
      ...
      print(
        '[ALICE-NO] coldstart: consumeInitialPayload primed with $bobPeerId',
      );
...
void _runBob() {
  testWidgets(
    'Bob(notif-open during other chat) — sender',
    (tester) async {
      print('\n${'═' * 60}');
