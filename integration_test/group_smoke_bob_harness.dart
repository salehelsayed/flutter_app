...
const _dbName = String.fromEnvironment('E2E_DB_NAME', defaultValue: 'group_smoke_bob.db');
  ...
  testWidgets('Bob(Group) — G1–G5', (tester) async {
    ...
    Future<Map<String, dynamic>?> waitForGroupMsg(
      ...
      while (DateTime.now().isBefore(deadline)) {
        ...
        for (final m in msgs) {
          if (m.text.contains(substring) && m.senderPeerId != stack.identity.peerId) {
            sw.stop();
            return {
    ...
    print('[BOB-G] G1: ${g1 != null ? 'received (e2e=${g1['e2eMs']}ms)' : 'TIMEOUT'}');
    ...
    _writeJson('g2_bob_received', {'count': g2Received.length, 'timings': g2Received});
    ...
    final g4 = await waitForGroupMsg('G4:', timeout: const Duration(seconds: 90));
    print('[BOB-G] G4: ${g4 != null ? 'received (e2e=${g4['e2eMs']}ms)' : 'TIMEOUT'}');
    ...
    // Receive msg1–msg4
    for (var i = 1; i <= 4; i++) {
      final m = await waitForGroupMsg('G5: msg$i', timeout: const Duration(seconds: 30));
    ...
    final msg5 = await waitForGroupMsg('G5: msg5', timeout: const Duration(seconds: 15));
    g5Timeline.add({
      'n': 5,
      'role': 'recv_inbox',
    ...
    final msg6 = await waitForGroupMsg('G5: msg6', timeout: const Duration(seconds: 30));
    g5Timeline.add({'n': 6, 'role': 'recv', ...?msg6});

    // Bidirectional: Bob sends msg7
    final g5b7 = await sendGroup('G5: bob msg7');
    ...
    _writeJson('g6_bob_done', {'note': 'peer discovery measured during G1 setup'});
    ...
    final g7Pre = await waitForGroupMsg('G7: pre-rotation', timeout: const Duration(seconds: 30));
    ...
    final g7Post = await waitForGroupMsg('G7: post-rotation', timeout: const Duration(seconds: 30));
    ...
    final g8 = await waitForGroupMsg('G8:', timeout: const Duration(seconds: 30));
