...
void registerDeletePreservesFriendsSim() {
  testWidgets('DELETE_PRESERVES_FRIENDS 1', (tester) async {
    ...
    await groupRepo.saveMember(
      GroupMember(
        groupId: 'sim-game-night',
        peerId: friendA.peerId,
        username: friendA.username,
        role: MemberRole.admin,
        publicKey: friendA.publicKey,
        mlKemPublicKey: friendA.mlKemPublicKey,
        joinedAt: now,
      ),
    );
    ...
    addTearDown(setGroupExitIntentActionSinks);
    ...
    final l10n = AppLocalizations.of(tester.element(find.byType(OrbitWired)))!;
    ...
    // -- act: swipe-left on the active member row, tap Leave, and confirm.
    ...
    await tester.pump();
    // Confirmation dialog appears; tap its canonical leave action.
