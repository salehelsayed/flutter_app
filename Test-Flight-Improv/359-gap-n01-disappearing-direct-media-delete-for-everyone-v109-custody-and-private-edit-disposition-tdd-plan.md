...
Status: prerequisite-blocked / review pending / default-off / not release-eligible
Type: Modification
...
| 2026-08-11 | Independent `$tdd-review` | Pending | Pending | Pending. |
...
- Plan 358 must be cleanly committed, all reviewed gates green, and its post-execution audit closed. Record that exact commit as `PLAN359_ACCEPTED_BASE`; never use the transient Plan 358 RED commit or a dirty implementation snapshot.
...
- Make private EDIT explicitly unsupported at both authority boundaries:
  ...
  - Receiver: preserve clear/inner identity validation first, then sender/contact authentication. Move the one durable target read immediately after that authentication and before strict-invalid, Plan-353 caption, duplicate, missing-original or generic edit routing. A crossed parent author/contact remains `unauthorized`; otherwise any normalized v1 redacted EDIT—or any same-author EDIT targeting a durable v1 redacted parent—returns `ignoredEdit`. It writes no placeholder, parent, attachment, key or marker; publishes nothing; and sends no application mutation receipt. Existing live transport confirmation and recovered-inbox `ignoredEdit -> rejected` disposition terminally drain the unsupported event without a new enum/outcome. D-234-01's action-specific private-EDIT prohibition controls; D-234-07's unsupported persistence applies to received media items, not an unsupported mutation that grants edit semantics.
...
- TC-359-03a reuses the real current-deletion handler owner rather than adding a duplicate DB-only disappearing suite. Create the hidden representative through the real hide operation so its preserved `available + terminal/high-water` shape cannot be accidentally rejected; do not add another contact-race, reaction-failure or lock-order matrix already proved by Plans 356/357.
...
1. Wait for Plan 358's clean committed GREEN and post-execution audit. Record its exact SHA as `PLAN359_ACCEPTED_BASE`, refresh the Graphify query/source anchors, confirm expected production/test surfaces, and amend/re-review if they differ.
...
8. In `send_chat_message_use_case.dart` and `handle_incoming_chat_message_use_case.dart`, add the persisted/wire private-EDIT guards before effects. On receive, defer strict-invalid handling until after authenticated sender/contact plus the single durable target read, retain crossed-author `unauthorized`, and document the D-234 unsupported-mutation arbitration beside the new guard. No repository/listener/recovered-disposition change is authorized.
...
test "$(git rev-parse --verify "${PLAN359_ACCEPTED_BASE}^{commit}")" = \
  "$(git rev-parse HEAD)"
...
Pending independent `$tdd-review`.

## Arbiter Decision

Pending independent `$tdd-review`; execution remains prerequisite-blocked regardless of review verdict until Plan 358 closes.
