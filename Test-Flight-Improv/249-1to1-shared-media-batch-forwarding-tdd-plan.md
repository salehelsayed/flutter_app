...
Must preserve:
- Plan-232 one-source viewer/bubble Forward remains unchanged and execution-ready -> TC-249-01/12.
...
- Do not reserve a migration version speculatively; group plan 236 owns DB v98 and later execution-ready plans may advance the ledger before this plan is approved.
...
Dependencies:
- `Test-Flight-Improv/232-1to1-received-media-forwarding-tdd-plan.md` for one-source `ForwardProvenance`, picker launch, direct forwarded marker, fresh per-target encryption, dedup, and retry primitives.
- `Test-Flight-Improv/233-1to1-shared-media-library-batch-tdd-plan.md` for stable bounded selection and library UI; it remains execution-ready without Batch Forward.
- `Test-Flight-Improv/236-group-received-media-forwarding-tdd-plan.md` for group DB v98 marker/dedup behavior before group destinations can claim parity.
- `Test-Flight-Improv/240-announcement-received-media-forwarding-tdd-plan.md` for announcement policy.
...
| TC-249-02 | Selection uses stable attachment/message IDs, applies the approved item/byte cap, preserves repository order, coalesces same-parent entries only as approved, and handles mixed ineligible state exactly. | `test/features/conversation/application/build_shared_media_batch_forward_test.dart::selection enforces approved cap order eligibility and parent grouping` | evidence-gated host application / mixed paged library entries + file sizes | HEAD compile RED: builder absent -> exact selected IDs/order/ineligible result follow D-249-01/04 | key by path, exceed cap, reorder by completion, or silently include protected/missing media -> TC-249-02 red | `flutter test test/features/conversation/application/build_shared_media_batch_forward_test.dart`; reserve AUTO + both 1:1 arrays; blocked by D-249-01/04 |
...
| TC-249-09 | If approved retry/provenance state needs new schema, a freshly conflict-checked DB vNEXT preserves the then-current chain and survives PRAGMA/before-after/run-twice/encrypted reopen; otherwise the row records exact N/A. | `test/core/database/migrations/shared_media_batch_forward_state_migration_test.dart::approved vNEXT preserves prior rows and batch retry state idempotently` plus `integration_test/shared_media_batch_forward_sqlcipher_proof_test.dart` | conditional host structure + Android/iOS real SQLCipher | HEAD evidence gap: no schema decision/version -> exact columns/defaults/checks/registry/full-chain/device proof are defined only after D-249-03/05 and fresh ledger check | omit a retry cell/default/guard, lose prior rows, or make reopen/rerun fail -> TC-249-09 red if schema is selected | Reserve no number; refresh literal host/Android/iOS commands and exact `1to1` discovery only if schema is approved, otherwise document N/A with existing persisted-send tests |
...
- SQLCipher branch: only if D-249-05 selects new durable batch state, allocate vNEXT after a fresh ledger check and run dedicated physical Android/iOS PRAGMA/before-after/reopen/run-twice proof. No version is reserved now.
