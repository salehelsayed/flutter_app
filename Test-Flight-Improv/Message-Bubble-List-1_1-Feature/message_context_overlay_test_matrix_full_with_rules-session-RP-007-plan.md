...
- Close source row RP-007 for "Incoming reply remains readable even when the quoted source does not exist locally" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: no execution because already covered.

### closure bar

- Source row RP-007 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- Incoming reply remains readable even when the quoted source does not exist locally
- Exact repo proof exists that replies persist and remain readable when the quoted source ID is absent locally.
...
- test/features/conversation/integration/quote_reply_thread_test.dart:203

### existing tests covering this area

- Re-audit test/features/conversation/integration/quote_reply_thread_test.dart:203 for exact row proof before adding new coverage.
...
- flutter test --no-pub test/features/conversation/integration/quote_reply_thread_test.dart:203
