...
- Close source row CP-007 for "FeedScreen copy remains safe after context changes because messenger and l10n are captured before await" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: tests only.

### closure bar

- Source row CP-007 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- FeedScreen copy remains safe after context changes because messenger and l10n are captured before await
...
- feed_pre_await_context_capture
...
- Named gate: feed_pre_await_context_capture
