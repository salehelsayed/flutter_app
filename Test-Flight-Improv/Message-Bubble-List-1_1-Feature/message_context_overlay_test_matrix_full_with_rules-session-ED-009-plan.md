...
- Close source row ED-009 for "Incoming edit that arrives before the original does not create a phantom row and later converges correctly" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: code changes.

### closure bar

- Source row ED-009 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- Incoming edit that arrives before the original does not create a phantom row and later converges correctly
...
- Re-audit test/features/conversation/application/send_chat_message_use_case_test.dart for exact row proof before adding new coverage.
