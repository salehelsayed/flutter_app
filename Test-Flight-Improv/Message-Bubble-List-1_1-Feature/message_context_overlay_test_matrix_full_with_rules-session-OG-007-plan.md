...
- Close source row OG-007 for "Delete uses overlay-pop then next-frame sheet sequencing" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: tests only.

### closure bar

- Source row OG-007 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- Delete uses overlay-pop then next-frame sheet sequencing
...
- overlay_pop_then_delete_sheet
- single_sheet_presentation
...
- Named gate: overlay_pop_then_delete_sheet
- Named gate: single_sheet_presentation
