...
- Close source row DL-017 for "Sender restart after pending tombstone persistence still converges to the correct visibility state" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: code changes.

### closure bar

- Source row DL-017 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- Sender restart after pending tombstone persistence still converges to the correct visibility state
