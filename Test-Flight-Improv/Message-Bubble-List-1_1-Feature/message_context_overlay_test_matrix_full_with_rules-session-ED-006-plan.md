...
- Close source row ED-006 for "Successful edit reuses the original ID, preserves timestamp / createdAt, sets `editedAt`, and renders `(edited)` on both sides" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: tests only.

### closure bar

- Source row ED-006 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- Successful edit reuses the original ID, preserves timestamp / createdAt, sets `editedAt`, and renders `(edited)` on both sides
...
- test/features/conversation/application/send_chat_message_use_case_test.dart:651
...
- Re-audit test/features/conversation/application/send_chat_message_use_case_test.dart:651 for exact row proof before adding new coverage.
...
- edit_eight_condition_gate
- latest_sent_edit_gate
- edit_same_id_contract
- edit_sender_auth
- delete_dominates_stale_edit
- No earlier session dependency was recorded, but refresh against landed code before execution anyway.

### exact tests and gates to run

- flutter test --no-pub test/features/conversation/application/send_chat_message_use_case_test.dart:651
...
- Named gate: edit_eight_condition_gate
- Named gate: latest_sent_edit_gate
- Named gate: edit_same_id_contract
- Named gate: edit_sender_auth
- Named gate: delete_dominates_stale_edit
