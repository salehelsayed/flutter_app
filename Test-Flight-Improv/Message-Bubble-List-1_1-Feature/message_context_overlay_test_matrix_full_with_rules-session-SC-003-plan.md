...
- Close source row SC-003 for "Stream sender, top-level envelope sender, and decrypted payload sender must agree for mutating actions" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: code changes.

### closure bar

- Source row SC-003 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- Stream sender, top-level envelope sender, and decrypted payload sender must agree for mutating actions
