...
- Close source row SC-006 for "Schema migration / upgrade preserves old messages and loads new overlay state fields safely" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: tests only.

### closure bar

- Source row SC-006 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- Schema migration / upgrade preserves old messages and loads new overlay state fields safely
