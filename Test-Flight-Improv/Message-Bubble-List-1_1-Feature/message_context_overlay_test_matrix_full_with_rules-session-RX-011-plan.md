...
- Close source row RX-011 for "`sendMessage=false` and `storeInInbox=false` must not look like success" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: code changes.

### closure bar

- Source row RX-011 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- `sendMessage=false` and `storeInInbox=false` must not look like success
- Current repo evidence does not prove the full row contract; the risk profile and missing exact tests suggest a likely behavior gap across lib/features/conversation/presentation/widgets/reaction_bar.dart and lib/features/conversation/presentation/widgets/reaction_display.dart.
...
- test/core/database/migrations/016_message_reactions_test.dart

### existing tests covering this area

- Re-audit test/features/conversation/application/send_reaction_use_case_test.dart for exact row proof before adding new coverage.
- Re-audit test/features/conversation/application/remove_reaction_use_case_test.dart for exact row proof before adding new coverage.
- Re-audit test/features/conversation/application/handle_incoming_reaction_use_case_test.dart for exact row proof before adding new coverage.
...
- Re-audit test/core/database/migrations/016_message_reactions_test.dart for exact row proof before adding new coverage.
...
- flutter test --no-pub test/features/conversation/application/send_reaction_use_case_test.dart
- flutter test --no-pub test/features/conversation/application/remove_reaction_use_case_test.dart
...
- flutter test --no-pub test/core/database/migrations/016_message_reactions_test.dart
