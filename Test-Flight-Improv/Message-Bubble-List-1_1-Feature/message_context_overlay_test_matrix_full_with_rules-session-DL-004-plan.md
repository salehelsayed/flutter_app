...
- Close source row DL-004 for "“Delete for me” hard-deletes the local row and cleans local artifacts only" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: no execution because already covered.

### closure bar

- Source row DL-004 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- “Delete for me” hard-deletes the local row and cleans local artifacts only
- Exact repo proof exists for local hard-delete plus reactions/attachments/owned-file cleanup on delete-for-me.
...
- test/features/conversation/application/delete_message_use_case_test.dart:67

### existing tests covering this area

- Re-audit test/features/conversation/application/delete_message_use_case_test.dart:67 for exact row proof before adding new coverage.
...
- flutter test --no-pub test/features/conversation/application/delete_message_use_case_test.dart:67
