...
- Close source row RX-006 for "Tapping an inline reaction chip uses the same toggle / replace semantics as the overlay bar" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: tests only.

### closure bar

- Source row RX-006 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- Tapping an inline reaction chip uses the same toggle / replace semantics as the overlay bar
