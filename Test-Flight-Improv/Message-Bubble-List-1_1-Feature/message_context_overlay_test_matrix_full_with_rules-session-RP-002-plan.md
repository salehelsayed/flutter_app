...
- Close source row RP-002 for "Swipe-to-reply is offered only for incoming non-deleted messages and joins the same quote flow" without merging it into adjacent rows; prove the row contract across the stated hosts, persistence, and transport semantics that the matrix requires.
- Execution ownership for this session: tests only.

### closure bar

- Source row RP-002 is updated in Test-Flight-Improv/Message-Bubble-List-Feature/message_context_overlay_test_matrix_full_with_rules.md to Covered or Closed with exact file-and-test evidence.
...
- Swipe-to-reply is offered only for incoming non-deleted messages and joins the same quote flow
...
- test/core/database/migrations/009_quoted_message_id_test.dart
...
- Re-audit test/core/database/migrations/009_quoted_message_id_test.dart for exact row proof before adding new coverage.
...
- flutter test --no-pub test/core/database/migrations/009_quoted_message_id_test.dart
