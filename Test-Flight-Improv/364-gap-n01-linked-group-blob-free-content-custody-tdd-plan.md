...
Type: Modification
Planning baseline: `153990d78b94028049bb0290c70f5194c6f7204d` (clean Plan-363 common-authority/two-phase-dissolve post-execution audit closure; Graphify `06a4d00c3599f606`, 72,949 nodes / 106,902 edges; DB v114; canonical `GROUP_TESTS` 244/244 unique paths)
...
- Execution baseline: Plan-363 common-authority/two-phase-dissolve audit closure `153990d78b94028049bb0290c70f5194c6f7204d`; current Graphify fingerprint `06a4d00c3599f606`, 72,949 nodes / 106,902 edges. The final anchored review returned `confidence=anchored` and `freshness=current` with anchors at `rotateAndDistributeGroupKey`, `dbCommitGroupKeyWithAuthorityComplete` and `recoverPreparedProtectedGroupDissolve`.
...
- Plan 363 is code-complete, post-execution audited and committed at `153990d78b94028049bb0290c70f5194c6f7204d`; this exact baseline includes sender-side genesis, one common key authority version including zero-target rotations, locally authored prepared/complete survival, and restart-safe two-phase dissolve, and it authorizes Plan-364 host RED/GREEN development. Its protected-kind relay binary is deployed, while production ACK-custody admission remains default-off and still blocks B1b/device acceptance, activation and release.
...
- Add exactly one `AckCustodyKind.groupContentV1` / `group_content_v1` branch. It accepts only incumbent replay `payloadType=group_message` and `payloadType=group_reaction`; reaction ADD/REMOVE is bound by the exact signed `notificationExtension.action` and transition ID, not a new payload type. Plan-363 bootstrap/authority parsers continue rejecting content.
...
```bash
set -euo pipefail
PLAN364_ACCEPTED_BASE='153990d78b94028049bb0290c70f5194c6f7204d'
...
```bash
set -euo pipefail
plan364_base_ref='153990d78b94028049bb0290c70f5194c6f7204d'
...
- [x] Plan 363 is cleanly implemented and post-execution audited at `153990d78b94028049bb0290c70f5194c6f7204d`; its committed Graphify `06a4d00c3599f606`, DB v114, common sender/A/B key authority including zero-target rotations, restart-safe two-phase dissolve, sender/receiver history, credential/protected-runtime APIs and 244/244 `groups` inventory are pinned and revalidated before RED.
...
- Review confirmed one additive `group_content_v1` protected kind over Plan 363 is the smallest production-reachable slice. The existing aggregate group inbox, group topics and full wired conversation are rejected because they cannot provide recipient-independent ACK, historical-authority apply, or a restricted linked surface.
...
- Current state: `EXECUTION_READY / HOST-DEVELOPMENT-UNBLOCKED / LIVE-ACCEPTANCE-BLOCKED`; Plan 363 is pinned at `153990d78b94028049bb0290c70f5194c6f7204d`, Graphify `06a4d00c3599f606`, DB v114 and `GROUP_TESTS` 244/244. No Plan-364 RED or production implementation has started.
