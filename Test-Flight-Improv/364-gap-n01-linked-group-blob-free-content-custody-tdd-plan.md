...
| 2026-08-13 | Final authority/history prerequisite reclosure | Clean Plan-363 source/audit closure `2478134be117f0c3de5f5937f7443c12c1ba7c38`; current anchored Graphify `e11b29f71210cad3` (73,204 / 107,141; 15,549 named tests); final causal c4 `+76`, preservation c4 `+19`, DTR c4 `+7`; all 36 changed Dart files analyzed; 244/244 inventory; three independent clear audits | The final closure adds durable exact ABORTED history, historically authorized membership repair with strict native config and signed version convergence, atomic PREPARED-only metadata authoring followed by native sync and COMPLETE, one atomic fresh all-target key batch, exact row-owned key-draft restart recovery, phase-aware dissolve receive, terminal-free local discovery and identity fail-closed removal. Sender/receiver history and the common authority version survive owner retirement and process restart without a mutable-current authorization dependency. DB remains v114; no Plan-364 source, relay, selector or admission change was made. | Final host prerequisite satisfied. Pin this SHA/graph/API/count baseline in every literal preflight and keep `EXECUTION_READY / HOST-DEVELOPMENT-UNBLOCKED / LIVE-ACCEPTANCE-BLOCKED`. Production admission stays off; B1b/device acceptance/activation/release remain separately blocked. |
| 2026-08-13 | Same-epoch sibling-growth prerequisite audit | Real admission reopen at an unchanged key epoch; pending-distribution operation identity; deferred authority PREPARED/COMPLETE lookup; production runner/adapter reconstruction at `2478134be117f0c3de5f5937f7443c12c1ba7c38` | The authority/history closure still addressed deferred work only by group/sender/member/key epoch. A terminal `{A}` delivery reopened for `{A,B}` therefore loaded its old PREPARED proof, failed the new ACL comparison and retained the pending row forever; an intentional same-device reannounce also had no new address. The old address was already COMPLETE, so ABORTED was not a repair. | Re-block Plan-364 host development. Require a durable generation per admission/reannounce rearm, generation plus canonical deliverable-device-set digest in deferred authority identity and signed data, a protected partial-custody/restart production proof, a committed Plan-363 closure and a fresh graph/inventory audit. Keep production admission off. |
| 2026-08-13 | Same-epoch reopen-generation prerequisite reclosure | Clean Plan-363 source/audit closure `591303dba6e510c1ffbb98504d328802f8317add`; current anchored Graphify `07f6a2a42e17060c` (73,230 / 107,176; 15,555 named tests); focused concurrent affected bundle `+101`; DTR `+7`; changed-Dart analyzer and diff checks; 244/244 inventory; final independent audit | Every admission and same-device reannounce now atomically advances a schema-free durable `operationGeneration`. Deferred authority identity and signed data bind group/sender/member/epoch/generation plus the canonical deliverable-device-set SHA-256. A COMPLETE -> real B admission at the same epoch -> A accepted/B survivor -> reconstructed runner/adapter -> B convergence/finalization passes, and same-device reannounce advances generation with the same digest while old COMPLETE history stays immutable. DB remains v114; no Plan-364 source, relay, selector or admission change was made. | Final host prerequisite satisfied. Repin every current source/graph/API/count field and restore `EXECUTION_READY / HOST-DEVELOPMENT-UNBLOCKED / LIVE-ACCEPTANCE-BLOCKED`. Production admission stays default-off; B1b/device acceptance/activation/release remain separately blocked. |
...
- Execution baseline: final Plan-363 same-epoch reopen-generation audit closure `591303dba6e510c1ffbb98504d328802f8317add`; current Graphify fingerprint `07f6a2a42e17060c`, 73,230 nodes / 107,176 edges with 15,555 named tests. The final anchored review returned `confidence=anchored` and `freshness=current` with anchors at `operationGeneration` and `canonicalDeferredDeviceSetDigest`.
- Pinned shared boundary: DB v114; `AckCustodyKind.groupBootstrapV1` / `group_bootstrap_v1`, `AckCustodyKind.groupAuthorityV1` / `group_authority_v1`, inner `linked_group_bootstrap_v1`; `AuthenticatedGroupAuthorityProof`, `loadAuthenticatedAuthorityVersion`, `loadAuthenticatedGroupAuthorityProofPage`, exact/bounded DB loaders and `runGroupAuthorityPhase`; schema-free `GroupPendingKeyDistribution.operationGeneration` plus `canonicalDeferredDeviceSetDigest`, with deferred authority identity and signed data bound to group/sender/member/epoch/generation/device-set digest; `LinkedTransportCredential` active-state authority; restricted `isLinkedBlobFreeRuntime`, `drainLinkedGroupBootstrap` and `drainLinkedDirectMediaBlobCustody` callbacks; canonical `GROUP_TESTS` 244/244 unique paths.

## Scope Contract

### Prerequisite, selector and rollout authority

- Plan 363 is code-complete, post-execution audited and committed at `591303dba6e510c1ffbb98504d328802f8317add`; this exact baseline includes sender-side genesis, durable PREPARED/COMPLETE/ABORTED history, historically authorized strict membership/metadata repair, stable common fresh authority, generation- and canonical-device-set-bound deferred authority, atomic all-target preparation, row-owned and zero-target restart recovery, durable dissolve survivor progress and one shared non-reentrant phase. It authorizes Plan-364 host RED/GREEN development. Its protected-kind relay binary is deployed, while production ACK-custody admission remains default-off and still blocks B1b/device acceptance, activation and release.
...
```bash
set -euo pipefail
PLAN364_ACCEPTED_BASE='591303dba6e510c1ffbb98504d328802f8317add'
...
```bash
set -euo pipefail
plan364_base_ref='591303dba6e510c1ffbb98504d328802f8317add'
