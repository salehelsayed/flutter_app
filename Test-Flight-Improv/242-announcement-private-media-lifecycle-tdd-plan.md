...
| TC-242-01 | Announcement mapping reuses plan 238's eventual lifecycle migration/state with legacy standard defaults and no competing migration | plan-238 migration + real-SQLCipher proof rows (exact vNEXT paths allocated only after its evidence gate) plus `test/features/groups/domain/models/announcement_private_media_policy_test.dart::announcement rows reuse accepted lifecycle state without another schema` | prerequisite migration sentinels + host model/repository fixture | HEAD lifecycle model RED -> after plan 238 acceptance, known/legacy announcement rows map exactly and this plan leaves the app DB version unchanged | add an announcement column/version or drop an accepted lifecycle map field -> TC-242-01 red | `flutter test test/features/groups/domain/models/announcement_private_media_policy_test.dart`; AUTO plus `GROUP_TESTS`; require the exact migration/SQLCipher commands recorded by accepted plan 238 |
...
| TC-242-04 | Reader bubble/viewer/library/batch surfaces derive one current capability matrix and deny every prohibited egress/forward/bookmark/resume action | `test/features/groups/presentation/announcement_private_media_capabilities_test.dart::reader media surfaces fail closed from one lifecycle policy without gaining compose` | evidence-gated host widget / plans 230/239-241 action harnesses, state table | HEAD policy/capabilities absent -> every surface agrees before/after consume/expiry and all forbidden gateway call counts stay zero | guard viewer only, use stale menu capabilities, or union batch permissions -> TC-242-04 red | `flutter test test/features/groups/presentation/announcement_private_media_capabilities_test.dart`; AUTO plus `GROUP_TESTS`; blocked on accepted action matrix |
...
- TC-242-08 records observable platform behavior and cleanup, not an assertion that screenshots are impossible. Repeat on each physical platform named in the accepted promise.
...
8. Run selected host GREEN, DB/real-crypto/platform/multi-party proof, ordinary/read-only/Go sentinels, group/core/feature gates, analyzer, and diff hygiene.
...
# Named host gates; expect new announcement files selected and zero failures
...
- Profile: external-fixture-blocked until TC-242-00 fixes the promise; expected minimum is three-party/device-lab plus physical platform capture runs inherited from plan 238.
...
- Required setup: Alice as eligible announcement publisher, Bob as online reader, Charlie as offline/restarting reader, three selected devices, configured real relay addresses, policy-specific image/video fixtures, and physical Android/iOS targets for every platform guarantee. Same-account extra devices are additionally required if convergence is promised.
...
- Environment blocker: missing physical platforms, three devices/accounts, relay fixture, or same-account device topology blocks each corresponding promised guarantee.
...
- [ ] Ordinary/read-only/Go/group/core/feature sentinels pass.
- [ ] Every selected physical platform/multi-party/convergence/relay proof passes.
...
- Manual registration: after evidence approval, add seven announcement host files to `GROUP_TESTS`; register provisional three-party scenario/harness/criteria/discovery and physical platform proof only for selected guarantees.
