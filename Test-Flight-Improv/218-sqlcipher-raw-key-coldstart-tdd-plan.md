...
| 2026-07-06 | **SC-5 speed device-proof — REAL APP, Pixel** | release APKs (Phase A + Phase B, FDC_FLOW_LOG), data-preserving `adb install -r` | **Real `mknoon/@pixel` account rekeyed passphrase→raw IN THE LIVE COLD-START PATH and SURVIVED** (hasIdentity:true → ROUTE_MAIN, Online, same QR/username; userVersion:95 preserved). Cold-start identity.db open **227ms→32ms** (N=5 each; ~195ms/~7× saved every launch). One-time rekey ~842ms on 1st Phase-B launch | ✅ MIGRATION + SPEED PROVEN. Ratio gate PASS (32 ≤ 227/5=45). Absolute ≤15ms NOT met on Android — real ~65-table open floor ~32ms; the 15ms came from synthetic 1-table tests. **PBKDF2 (~195ms) fully eliminated** (§D5 re-diagnosis) | iOS real-app |
| 2026-07-06 | **SC-5 speed device-proof — REAL APP, iPhone 11** | profile→release apps (FDC_FLOW_LOG), `devicectl` install (data-preserving); build in PROFILE/dev-signing to launch on device (release=distribution profile won't sideload-launch) | **Real account WITH CONTACTS rekeyed passphrase→raw in the live cold-start path and SURVIVED** (hasIdentity:true, ID_STARTUP_HAS_ID_WITH_CONTACTS → ROUTE_FEED; userVersion:95 preserved). Cold-start open **139ms→3ms** (N=5 each; ~46×) | ✅ MIGRATION + SPEED PROVEN. **Both SC-5 gates PASS on iOS**: raw 3ms ≤ 15ms AND ≤ 139/5. iOS FMDB open is far cheaper than Android's schema load | done |
| 2026-07-06 | QA verdict | — | 218 CLOSED: both phases device-green (8 SC × 2 platforms) + host (SC-6, 641) + Move sentinel + **real-app migration proven on BOTH real devices** (account survives, security intact, cold-start ~7×/~46× faster). Rollout: A-before-B, never downgrade below read-raw build | ✅ ACCEPTED | ship |
