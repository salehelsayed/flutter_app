...
| | **Phase A RELEASED (rollback-safe read floor live in TestFlight)** | build tag + rollout confirmation | soaked; a raw DB is readable by the shipped floor | **must read GO before Phase B — HAND-OFF (ship gate; not compressible in-session)** | §A3 |
| | 4a fresh raw + marker (SC-1/4/6) | | | GO: fresh-raw open p50 captured (informal ≤ passphrase_p50/5) | early reading→Next |
| | 4b legacy atomic rekey (SC-2/3) — brick risk, review in isolation | | | **must read GO** | |
| | 4c Move regression sentinel (SC-7) | | | N/A (non-destructive sentinel; RED only if snapshot switched to raw) | |
| | speed device-proof (SC-5, both platforms) | | | | |
