...
- Issue type: `bug`
- Output doc path: `Test-Flight-Improv/68-introduction-reliability-gap-audit.md`
...
### Group B: Sender-side intro durability still has a local-truth gap
...
- `_sendIntroductionChain(...)` then delivers the intro to both targets before
  saving the new local `IntroductionModel`
  (`lib/features/introduction/application/send_introduction_use_case.dart`).
...
- Resume and retrier wiring only retries those durable outbox deliveries
  ...
  `lib/main.dart`).
- The repo does not show a matching recovery path that reconstructs a missing
  sender-local intro row from the outbox if the app dies after durable envelope
  staging but before `saveIntroduction(...)` completes.
- That leaves a sender-side local-truth gap: recipients can receive or later
  recover the introduction while the sender's own replacement intro row is at
  risk if the process stops in that window.
...
- The remaining gaps are narrower and more specific:
  ...
  - no sender-side crash-window test that proves local intro truth survives if
    the app dies after outbox staging but before local `saveIntroduction(...)`,
...
- Existing partial coverage:
  ...
  - `Test-Flight-Improv/50-two-simulator-user-journey-tests-coverage-audit.md`
- Current gap: no single automated test in the repo currently proves the full
  reported scenario where one user already has the contact while the other user
  still shows `Waiting for ...`, then later reopen/resume heals that exact
  split.
- Current gap: no direct test currently covers the sender-side crash window
  between durable intro outbox staging and the final local
  `saveIntroduction(...)` write.
