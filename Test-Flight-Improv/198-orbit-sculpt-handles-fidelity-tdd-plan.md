...
10. orbit_edit_handle_test.dart::'handle visual contract: circular disc in ≥44pt target, per-knob icon, tip-pill below'  (NEW file, widget tier)
    - Tier: widget. Pump the extracted `OrbitEditHandle` (new public widget) for each knob: hit target ≥44×44 (getSize), disc decoration `shape: BoxShape.circle` ~30px at default scale (203 B3 scales the disc with avatarScale via `effectiveDiscSize`, clamp [24,42]), per-knob icon present (each knob's own `ValueKey('orbit-handle-icon-<name>')`), tip-pill Text == l10n `orbit_handle_*` value + separate letter-free glyph Text (↕/⟷) below the disc (tip center.dy > disc center.dy).
