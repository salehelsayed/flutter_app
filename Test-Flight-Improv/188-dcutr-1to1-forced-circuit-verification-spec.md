...
- The production circuit-establishment path exists and is reachable from a harness: `DialPeerViaRelay` (`node.go:1328`) → `dialPeerViaRelayWithTimeout` (`node.go:1335`), also invoked by the `RelayProbe` bridge command (`bridge.go:926`). The forced-public test seam exists (`node.go:117-118,375`). The upgrade marker is `markPeerUpgradedToDirect`, which fires only on an `info.Limited` circuit entry (`holepunch_tracer.go:159-179`).
