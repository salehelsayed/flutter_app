...
- `test/features/groups/integration/group_media_fanout_test.dart` covers existing discussion members receiving image, video, and voice descriptors, including sender message-id preservation and receiver download behavior. Report 89 now also covers a newly-added member sending image, video, and voice after bootstrap, with existing members receiving exactly one row for each media message.
...
- Host-side app-layer coverage now proves newly-added discussion members receive post-join text, image, video, and voice without pre-join backfill, and a newly-added discussion member can send image, video, and voice to existing members after bootstrap.
...
- Multiple newly-added members in the same group add sequence receive the same post-join media behavior.
