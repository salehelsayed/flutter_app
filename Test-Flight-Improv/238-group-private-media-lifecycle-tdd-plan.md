...
Dependencies:
- Migration dependency: no version is reserved now. After TC-238-00, inspect the actually landed database version and all accepted claims, then allocate exactly the next free version. Plan 236 owns v98 when its v96/v97 predecessors land; later landed work may change this plan's actual predecessor before evidence acceptance.
...
| TC-238-01 | Allocated vNEXT is structurally wired, adds approved default/local lifecycle fields, preserves predecessor rows, maps explicit state, and is run-twice idempotent under host SQLite. | `test/core/database/migrations/group_private_media_lifecycle_vnext_test.dart::GPL-01 allocated vNEXT structurally preserves legacy standard media and approved private lifecycle state idempotently` | evidence-gated host migration / `sqflite_common_ffi` fixture at the freshly verified predecessor | HEAD evidence RED: no version/schema is allocated -> after TC-238-00 and plan refresh, exact schema/default/preservation/time roundtrips, repeated call, allocated version constant/onCreate/onUpgrade/full-chain registration pass structurally | omit an approved default/constraint, serialize UTC incorrectly, or omit a registry branch -> GPL-01 red after refresh | direct structural test; AUTO `core-host-all`; does **not** claim SQLCipher closure |
...
| TC-238-09 | Bubble/viewer/gallery/batch action capabilities centrally deny exactly the operations prohibited by accepted view-once/disappearing/protected states. | `test/features/groups/presentation/group_private_media_capabilities_test.dart::GPL-09 every group media surface shares fail closed private capabilities` | evidence-gated host widget / plans 230/235-237 harnesses, state transition table | HEAD compile RED -> Save/Share/Forward/Bookmark/resume/PiP/thumbnail actions match policy before/after consume/expiry | guard viewer only or use capability union in batch -> GPL-09 red | add file to `GROUP_TESTS`; blocked on action matrix |
...
| TC-238-11 | Android/iOS capture behavior and user copy match the accepted best-effort platform guarantee, including route transitions/background/app switch. | `integration_test/group_private_media_platform_proof_test.dart::GPL-11 protected viewer applies truthful platform capture safeguards` | external-fixture-blocked device proof / physical Android+iOS, screenshot/record session | HEAD device RED: no platform mechanism -> only accepted observable behavior passes; unsupported limits are surfaced, not claimed away | omit secure flag/capture observer, leak app-switch snapshot, or leave protection after route exit -> GPL-11 red | add an exact `classify_path` rule recording this path as `ignored/ignored` manual-native proof; run directly on both promised platforms |
...
| TC-238-13 | Device-local or multi-device consume/expiry behavior matches the accepted convergence promise; conflicts/offline replay are deterministic. | `integration_test/group_private_media_convergence_proof_test.dart::GPL-13 consumed and expired state follows approved device convergence policy` | external-fixture-blocked / one device if local-only; two user devices + relay if convergent | HEAD evidence RED: no policy/protocol/harness -> selected profile proves no unintended sync or authenticated convergence/conflict semantics | N/A until ledger selects local-only vs convergent; then withhold/drop/reorder the state event as mutation | registration and closure command cannot be finalized before decision; row blocks closure if product promises convergence |
...
- TC-238-11 must test observable platform behavior and app-switch snapshots on physical devices. It must not assert “screenshots impossible” on a platform that cannot guarantee that claim.
...
flutter test test/core/database/migrations/group_private_media_lifecycle_vnext_test.dart --plain-name 'GPL-01 allocated vNEXT structurally preserves legacy standard media and approved private lifecycle state idempotently'
...
# Migration chain, Go extra boundary, and registered host gates
...
# Capture proof after accepted Android/iOS guarantee; repeat with a physical id for each promised platform
...
- Profile: `external-fixture-blocked` until TC-238-00 selects the promise. Minimum known boundaries are single-device production SQLCipher and real-Go-bridge; protected capture requires physical Android/iOS named by the accepted copy; convergence/revocation may require two devices plus controlled relay.
...
- Required setup: production `sqflite_sqlcipher` plugin for self-contained GPL-01D DB files and existing real-Go group crypto fixture for GPL-12. GPL-11 requires physical target(s), private image/video fixture, screenshot/screen-record and app-switch observation. GPL-13/14 setup cannot be specified until local-only/convergent and local-cleanup/remote-revocation decisions are accepted.
...
- Environment blocker: missing physical platforms/second device/relay fixture blocks every corresponding promised guarantee and final closure.
...
- [ ] Required real device/multi-device/relay proofs pass for every user-facing guarantee.
...
- Harness registration: new host files into `GROUP_TESTS`; structural migration AUTO `core-host-all`; exact `classify_path` group records for SQLCipher DB/real bridge and exact ignored/manual-native record for capture; convergence/relay get explicit records only after accepted evidence.
...
- Boundary closure: selected combination of host crash/replay, real Go bridge, physical capture, multi-device, and relay proof; currently external-fixture/evidence blocked.
