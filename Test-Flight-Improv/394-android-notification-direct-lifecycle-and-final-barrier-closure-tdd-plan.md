...
- Affected production, test, and gate files: `lib/features/conversation/presentation/screens/conversation_wired.dart`; `lib/features/groups/presentation/screens/linked_group_conversation_wired.dart`; `lib/features/push/application/background_message_handler.dart` (test hook only); their three existing test files; the two UI-23 coverage documents; this plan and `Test-Flight-Improv/00-INDEX.md`. Existing registration in `ONE_TO_ONE_TESTS` and `GROUP_TESTS` is preserved; no gate-script edit is planned.
...
- [ ] TC-394-01 records causal RED, focused GREEN, and the resumed-fallback mutation re-red/revert.
- [ ] TC-394-02 proves post-mount peer mismatch remains unread, while explicit resumed+exact remains positive.
- [ ] TC-394-03 records causal linked-group null-lifecycle RED, focused GREEN, and mutation re-red/revert; explicit resumed+exact marks only the eligible row.
- [ ] TC-394-04 proves both direct denial legs first reach exact event/tone `publishing`, then post zero cards and roll back/reacquire exact owners; unknown visibility posts once; canonical, visibility, event-abort, and tone-abort mutations independently re-red.
- [ ] TC-394-05 preserves one managed typed direct message and one managed typed direct reaction when canonical authority is unknown.
- [ ] Existing registration is verified without duplicating the shared handler test into another curated array.
- [ ] Focused files, `1to1`, `groups`, exact census, scoped analysis/format/diff hygiene, and the one final concurrent `host-all` pass with semantic outcomes.
- [ ] Both UI-23 documents and the index reconcile the post-audit residual without reopening or claiming excluded Apple/release/OEM/optional work.
- [ ] Scope Contract And Guard is respected; no device, migration, relay, or architecture work is introduced.
...
Final verdict: `ready`. Plan classification remains `implementation-ready`; core bet `confirmed`; disposition `execute`. One Plan 394 is sufficient. No user-owned product or release decision remains inside the trimmed Android-led scope.

## Execution Progress

| Time | Phase | Files | Last command/result | Current evidence | Decision/blocker | Next |
|---|---|---|---|---|---|---|
| - | not started | - | - | - | awaiting reviewed plan | execute TC-394-01 RED |
