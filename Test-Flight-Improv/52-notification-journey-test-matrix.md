...
|---|---|---|---|---|---|---|---|---|---|
| GMN-001 | A sends, B/C receive one correct group notification | B and C each get one notification for the correct group with sender-prefixed body text. | P0 | Recommended | Required | Required | N/A | Required | Current local evidence: `test/features/groups/application/group_message_listener_test.dart`, `test/features/push/application/show_notification_use_case_test.dart`. Report 102 GIRD-006 adds canonical group route payload, background fallback display-failure, local replay suppression, and tap-route proof; GIRD-007 foreground group push simulator smoke passed before the later #49 group-reliability blocker. |
...
| Test ID | Scenario | Expected Result | Priority | Unit | Integration | Smoke | Fake Network | 3-party E2E | Current coverage / notes |
|---|---|---|---|---|---|---|---|---|---|
| GMN-101 | Visible remote push plus later local group replay | One group message yields one user-visible notification path; later local replay is suppressed. | P0 | Required | Required | Recommended | Required | Recommended | Covered by `test/integration/group_notification_dedupe_integration_test.dart`, `test/features/push/application/show_notification_use_case_test.dart`, and `test/features/groups/application/group_message_listener_test.dart`. Report 102 GIRD-006 strengthens the data-only fallback display-failure boundary so failed local fallback display does not mark the recent-remote gate and suppress later in-app replay. |
