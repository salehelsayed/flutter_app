...
| Test ID | Scenario | Expected Result | Priority | Unit | Integration | Smoke | Fake Network | 3-party E2E | Current coverage / notes |
|---|---|---|---|---|---|---|---|---|---|
| GMN-101 | Visible remote push plus later local group replay | One group message yields one user-visible notification path; later local replay is suppressed. | P0 | Required | Required | Recommended | Required | Recommended | Current closure evidence: `test/integration/group_notification_dedupe_integration_test.dart`, `test/features/groups/application/group_message_listener_test.dart`, `test/features/groups/application/handle_incoming_group_message_use_case_test.dart`, and `test/features/groups/application/drain_group_offline_inbox_use_case_test.dart` prove same-id replay and live/drain races do not double-emit or double-notify. `integration_test/foreground_group_push_drain_test.dart` command `#2` rerun on 2026-06-04 preserves one user-visible materialization after foreground push catch-up. |
...
| GMN-104 | Offline member reconnect / drain | If C is offline, allowed post-reconnect delivery yields one notification/materialization path without duplicate replay. | P1 | Required | Required | N/A | Required | Recommended | Current closure evidence: `integration_test/group_recovery_e2e_test.dart` command `#8` rerun on 2026-06-04 passed resume drain, inbox dedupe, watchdog rejoin, restored text same-id continuation, and voice retry/re-record same-id recovery; `test/features/groups/application/drain_group_offline_inbox_use_case_test.dart`, `test/features/groups/application/group_message_listener_test.dart`, and `test/integration/group_notification_dedupe_integration_test.dart` cover the direct one-materialization/one-notification drain behavior. Real 3-party provider/device notification proof remains residual-only. |
