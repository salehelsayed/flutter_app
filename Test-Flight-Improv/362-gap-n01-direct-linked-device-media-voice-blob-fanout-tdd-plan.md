...
Status: CODE-COMPLETE (executed 2026-08-12; post-audit repair same day: composer fresh-fanout routing wired, voice/share fresh entries fail closed, drain last-reference counter production-wired) / default-off authoring / recorded deviations: private-parent DFE fanout, voice+share plural ROUTING (now production-guarded fail-closed), device-pair harness program / not release-eligible
Type: Modification
...
- [x] Supported producer/modality adapters pass through one shared owner: the COMPOSER now routes fresh generations to the all-target owner in production, and `sendChatMessage` self-assembles the plural context from persisted survivors; unsupported combinations, private EDIT, linked LAN and selector-off cases are all-zero (recorded deviation: voice/share plural ROUTING pending — both fail closed in production on an initialized roster, proven by tests).
