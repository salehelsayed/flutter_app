...
Status: POST-EXECUTION AUDIT CLOSED (executed 2026-08-12; FOURTH counterexample repair closed 2026-08-13: ownerless historical DFE admission, persisted-v114 zero-resolver replay authority, and default-build causal proof) / default-off authoring / recorded deviations: ALL media-parent DFE and caption EDIT fanout plus voice+share plural ROUTING (all production-guarded fail-closed) / not release-eligible
Type: Modification
...
| 2026-08-12 | Post-audit repair (source review findings) | `prepareAndUploadFreshFanout` had no production caller while the linked gate unlocked media; the drain's `countOtherArtifactReferences` was omitted at its sole production construction | Both defects repaired. (1) The composer's strict lane now decides the target route BEFORE any media crypto: the fanout route drives `prepareAndUploadFreshFanout` (all-target owner, per-target uploads, linked stored rows) and `sendChatMessage` self-assembles the plural context from the persisted survivors (retry-lane pattern; underivable authority still refuses), while a non-legacy route with a required selector OFF refuses without demoting — P/VO initials and the voice/share fresh entries fail closed on an initialized roster (voice/share plural routing remains the recorded deviation, now GUARDED in production rather than silently singular). (2) The production drain construction wires the last-reference counter to `dbCountOtherDirectMediaBlobCustodyRowsReferencingArtifact`, so the first completed target can no longer unlink ciphertext a surviving sibling still needs. Tests: two composer routing/refusal widget proofs (flag-gated like every strict composer proof, PROVEN under the `MKNOON_DIRECT_MEDIA_BLOB_CUSTODY_CLIENT_ENABLED` compilation), one voice fresh-roster fail-closed proof (flag-proven), the external-share test REWRITTEN to pin refusal-before-crypto on an initialized roster, and one production-composed two-sibling drain proof (first retirement preserves the shared artifact, the last unlinks it). A latent post-format DTR placement digest drift in `2d5193afc` (the final format pass ran after the first repin) was also caught and repinned, and a test-side fake-async deadlock was repaired (the composer scenario's `await Directory.systemTemp.createTemp` never completes in the `testWidgets` zone; rebased to the file's sync-teardown pattern). Receipts: composer routing+refusal `+2` under the `MKNOON_DIRECT_MEDIA_BLOB_CUSTODY_CLIENT_ENABLED` define, re-proven `+2` on the formatted final tree; focused `--concurrency=4` wave over the five repaired files plus both DTR contracts `+480 ~8` exit 0; `flutter analyze` `No issues found` (433s); `git diff --check` clean on working/cached/`c78c465a0...HEAD`; one incremental Graphify refresh on the final tree (72,219 nodes / 105,887 edges, fingerprint `942004f8d4648297`, freshness current). |
| 2026-08-12 | Second post-audit repair (strong admission boundary) | Source review found the first repair reached only the notification-tap route and only with the custody selector ON | Seven production defects closed. (1) ROUTE AUTHORITY: `directEventFanout`/`directDeviceTrust`/`modalityGate` reached exactly one of six production `ConversationWired` pushes; they now travel as one `DirectConversationRouteAuthority` bundle threaded MyApp -> StartupRouter -> Feed/FTE/QR -> Orbit/Posts, and the notification route reads the same bundle so routes agree by construction. Severity was LIVE in the normal route: setup and QR flows do read the linked-device selector, but Orbit's contact-trust and QR-staging paths read an obsolete nullable standalone field instead of the route bundle, leaving StartupRouter -> Feed -> Orbit authority inert. (2) COMPOSER: one admission at `_onSend` before the first side effect; the pending list is captured above it because the resolver awaits and `_attemptAddPendingMedia` publishes without an `_isSending` guard; both late `decideRoute` calls deleted so the roster is resolved once; the legacy lane now THROWS on a fanout-routed generation instead of demoting. (3) VOICE: boundary at the widget before the upload lease and quote clear (not merely before `copyToDurableStorage`), with review-hold restoration because `_onReviewSend` destroys the draft before calling; the use-case guard hoisted above the custody selector with the survivor carve-out retained. (4) SHARE: one boundary above `strictFreshSelected` covering all THREE entries (external share, received forward, group->contact forward), scoped to a non-empty batch so text shares still work. (5) RETRIES: both retriers terminalize never-published generations on an initialized roster through the incumbent `markUploadPendingAttachmentsFailedForMessage`, with a FLAG-INDEPENDENT survivor probe; the failed-retrier pin sits above the `directMediaIntent` block because P/VO parents carry no token, and marker-bearing parents are excluded so the terminal leg keeps its zero-read contract. (6) MUTATIONS: media-parent DFE and caption EDIT refuse on an initialized roster before the composer clear. (7) DRAIN: `drainLinkedDirectMediaBlobCustody` was declared and awaited by the application root but never supplied — now wired at cold start and resume to a new linked-scoped loader, on the SAME single drain instance (the phase contract freezes that constructor to one occurrence), with the local-cleanup leg deliberately left unwired because its orphan sweep uses a global inventory. The linked loader narrows the OUTGOING arm only; filtering incoming rows on `contact_account_peer_id IS NOT NULL` would strand media from ordinary single-device contacts. REFUTED during this repair: multi-attachment P/VO does NOT reach the legacy lane — `allowsNewPrivateMedia` requires exactly one attachment, so it refuses pre-persistence already. Receipts: focused c4 over 16 files `+717 ~11` exit 0; new `direct_media_fanout_admission_test.dart` 8/8 and `direct_conversation_route_authority_contract_test.dart` 3/3, both UNSKIPPED in the default compilation; the route census re-red on a causal mutation (Orbit authority removed) and was reverted; DTR 7/7 after repinning application_root, bootstrap and the media adapter with adjacent rationale; completeness 1443/1443; `flutter analyze` clean; 25 changed Dart files format-clean; three diff checks clean. |
| 2026-08-12 | Third post-audit counterexample repair and final closure | Reviewer supplied six live production counterexamples plus missing Task-10/retry/loader/value-sensitive proofs and two Plan-362-caused `1to1` pins | All six paths are closed. Admission treats a null snapshot, initialized zero-target snapshot, malformed peer ID, or read failure as refusal; only a proven uninitialized incumbent roster may remain singular. External share, direct forward, and group-to-contact forward admit before preprocessing/copying, while group delivery is explicitly outside the direct-contact guard; fileless direct share and Feed's inline direct composer carry blob-free fanout authority instead of demoting. Retry protection covers `upload_pending`, completed history, retry-ceiling `upload_failed`, and media-parent mutation retries; it is flag-independent for persisted survivors and observes terminalization CAS failure. Orbit consumes current trust/QR authority from `DirectConversationRouteAuthority`. The linked drain owns incoming plus outgoing prepared/stored/cleanup survivors, and linked receive persists logical account identity while retaining physical transport identity for receipts. Real-SQLite fixtures now seed the persisted incumbent contact authority they claim; the stale supplied-contact fallback was deliberately repinned to fail closed. A clean curated run exposed one additional production counterexample—the direct-contact admission guard swallowed group media—and `_deliverOrdinary` now scopes that guard to contact targets. The two Plan-362-caused preservation pins were reviewed and rationale-repinned, not classified as baseline: received-media transport references `widget.bridge` 33 -> 34 for the mutually exclusive all-target branch, and the `ConversationWired` public-API fingerprint `5e80dc06` -> `c8397430` for the optional route authority. Final proof: focused `TC-362` c4 `+31 ~3` PASS; clean serial curated `1to1` 126/126 PASS. Task 10 ran on USB Pixel 6 `21071FDF600CSC` (Android API 36) plus `emulator-5554` (Android API 37), run `1786559604591`, with both roles and the disposable Go fixture green; host summary `md004_1786559604591_direct_linked_device_event_blob_fanout_host_summary.json`. Bounded deviations remain only the production-refused media-parent mutation and voice/share plural-routing surfaces; activation, GAP-N01, and release eligibility remain open. |
| 2026-08-13 | Fourth post-audit retry repair and audit closure | Reviewer counterexamples for attachmentless ownerless DFE, persisted-v114 survivor replay under roster drift/unavailability, three skipped default-build rows, and missing causal host composition | Exact v109 custody now gets first chance; every remaining attachmentless ownerless media DFE must pass roster admission before tombstone reconstruction or egress, so an initialized zero-target/null/error snapshot refuses instead of replaying to the logical contact. Both failed-message and incomplete-upload replay build and bind v108 solely from persisted v114 recipient/key/manifest authority; they do not resolve the live roster after a generation has durable survivors. Exact tests distinguish an ownerless attachmentless DFE from an exact v109 survivor and restart a pre-v108 v114 generation with the roster resolver throwing/drifted. Default-build proof now covers composer -> durable v114 -> failed first egress -> restart retry without a resolver, two-layer voice refusal, all three share/forward entries before file work, and caption/DFE hosts; no TC-362 row is compilation-skipped. The earlier `+31 ~3` receipt above is retained as an interim historical result and is superseded by the final focused `--concurrency=4` proof: `+69`, all required host IDs `TC-362-01a` through `TC-362-06h` present, zero TC-362 skips. The serial curated selector closed all 126 paths; a legacy private-DFE fixture at path 89 was corrected to seed the incumbent contact authority it claims, its full file passed `+24`, and the runner resumed through path 126 with a terminal PASS. The two Plan-362 preservation pins were re-reviewed against the final public shape: received-media `widget.bridge` remains 34 with its adjacent mutually-exclusive-branch rationale, while the three optional `ConversationWired` test seams move the public-API fingerprint from `c8397430` to `f5df142f` with the assertions unchanged. Analyzer is clean; 103 changed Dart files and changed Go files are format-clean; baseline, unstaged and staged diff checks plus the final Graphify receipt are recorded below. No host-all or device rerun was required by the audit. Activation, GAP-N01 and release eligibility remain open. |
...
- Final post-audit closure: the single requested incremental refresh wrote 72,428 nodes / 106,174 edges and the compact review query reported current fingerprint `4bf3025ee64828d0`. Its broad shortlist is a topology receipt only; the exact retry/admission/route/loader source and causal tests recorded in the fourth execution row are the closure authority. No second refresh was run.
...
### Admission boundary (owner ruling, 2026-08-12 — binding)

"File write" means EVERY new send-owned durable write, including plaintext
`pending_uploads` copies, not only ciphertext.

- Existing picker/share files and an already-created voice recording are INPUTS.
- After read-only MIME/size/modality validation, resolve exactly one of
  `legacy | fanout(snapshot) | refused`.
- That resolution precedes upload leases, composer clearing, message/attachment
  persistence, `copyToDurableStorage`, crypto, custody staging and network.
- `legacy` is allowed ONLY for an uninitialized primary roster.
- An initialized roster whose selectors/capability cannot serve it REFUSES with
  the draft/source intact and ZERO retryable rows.
- Persisted v108/v114 survivors stay flag-independent and drain from stored
  authority; they never re-resolve the roster.

Implemented as one shared resolver,
`lib/features/conversation/application/direct_media_fanout_admission.dart`,
consumed by the composer, the voice widget leg, the voice use case, all three
share/forward entries, both retriers and both media-parent mutations. It reads
the repository, never a selector, which is what lets a DEFAULT build refuse
instead of demoting.

Two sub-rulings recorded rather than left implicit:

- Share/forward transcode output (`image_processor.processImage` /
  `processVideo`, invoked once per batch before any target is known) is a
  DERIVED INPUT, not a send-owned durable write. It is plugin cache, carries no
  DB authority, leaves zero retryable rows, and is unavoidable because the size
  gate deliberately evaluates post-processing budget bytes. The boundary
  therefore sits at the per-contact send, not upstream of batch processing.
- A CLEAN null roster snapshot keeps the incumbent path rather than failing
  closed. Null is returned strictly before the roster is consulted (contact row
  absent, blocked, or without an account public key), so it cannot hide an
  initialized roster, and refusing would replace an exact contact-level
  diagnosis with a generic media failure while preventing no demotion. The
  fail-closed arm is the read ERROR, where an initialized roster genuinely
  could be hidden.
...
for plan362_host_id in \
  TC-362-01a \
  TC-362-02a TC-362-02b TC-362-02c TC-362-02d TC-362-02e \
  TC-362-03a \
  TC-362-04a TC-362-04b \
  TC-362-05b \
  TC-362-06a TC-362-06b TC-362-06c TC-362-06d \
  TC-362-06e TC-362-06f TC-362-06g TC-362-06h; do
...
- [x] Exact-target completion and final-reference cleanup preserve siblings; revocation/contact deletion, Move and receive/auth races converge without orphan state. CORRECTED deviation: it is not only PRIVATE-parent DFE. NO media-parent delete-for-everyone fans out (`delete_message_use_case.dart:537` gates on `ownsDirectTextMutationInboxCustody`, which the lane classifier makes structurally false for any parent with attachments), and NO media-parent caption EDIT fans out either (`send_chat_message_use_case.dart:774` excludes `hasAttachments`, and `editChatMessage` always supplies the parent's media). Both now REFUSE on an initialized roster before the composer clear, instead of silently single-targeting.
- [x] Supported producer/modality adapters pass through one shared owner. CORRECTED: the earlier claim that the composer routes fresh generations to the all-target owner "in production" was true only on the notification-tap route and only with the incumbent custody selector ON. All five fresh producers now resolve ONE admission route through `resolveDirectMediaFanoutAdmission` before any send-owned durable write, selector-independently. Recorded deviation: voice/share plural ROUTING is still pending — both refuse on an initialized roster rather than demoting.
...
- [x] The available-Android SQLCipher proof passed on emulator-5554; Task 10's aggregate event/blob scenario also passed end to end on USB Pixel 6 `21071FDF600CSC` (Android API 36) plus `emulator-5554` (Android API 37), run `1786559604591`, with both device roles and the disposable Go fixture green (`md004_1786559604591_direct_linked_device_event_blob_fanout_host_summary.json`).
