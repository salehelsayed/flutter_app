...
| Dissolve group into a frozen read-only state with optional device-local cleanup | `shipped` | `dissolve_group_use_case.dart`, `group_message_listener.dart`, `send_group_reaction_use_case.dart`, `remove_group_reaction_use_case.dart`, `handle_incoming_group_reaction_use_case.dart`, `delete_group_and_messages_use_case.dart`, `group_info_wired.dart`, `group_conversation_wired.dart`, and the feed group-thread projection stack together enforce current-admin-only dissolve authority, post-dissolve read-only history, frozen write/reaction affordances, and explicit local-only cleanup from Group Info. |
