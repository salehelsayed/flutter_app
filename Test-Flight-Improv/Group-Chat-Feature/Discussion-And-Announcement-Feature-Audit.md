...
- The main user-facing gaps are around membership-event UX, creator/admin identity display, group message-card rendering quality, reaction-participant visibility, long-press message-context parity versus 1:1 conversations, cross-friendship participation journeys, and relay-inbox privacy plus membership-aware decryptability for offline replay, in addition to missing higher-level product tools such as search, pinning, read receipts, scheduled announcements, explicit ownership handoff, one reliability precision gap around how zero-peer plus inbox-fail sends are meant to recover, and two invite-accept recovery gaps: offline reactions are not included in the immediate catch-up path, and bridge-timeout acceptance can leave the group in a degraded local-only state without direct proof that automatic rejoin converges promptly.
...
| 2. Show `X accepted the invite` when invitees join | `missing` | `acceptPendingGroupInvite()` materializes the group locally and drains inbox state in `accept_pending_group_invite_use_case.dart`; `group_list_wired.dart` only shows a local snackbar such as `Joined ...`. No repo-local `invite_accepted`, `member_joined`, or equivalent system message/timeline event is emitted. |
...
| Accepting a pending invite replays offline reactions in the same immediate catch-up pass | `partial` | `accept_pending_group_invite_use_case.dart` calls `drainGroupOfflineInboxForGroup(...)` without supplying `reactionRepo`. In `drain_group_offline_inbox_use_case.dart`, `group_reaction` payloads are only applied when `reactionRepo != null`. The relay group inbox is non-destructive on retrieve, so the skipped reactions are deferred rather than deleted, and later startup/resume or continuity sweeps can drain them with `reactionRepo` present. But the immediate invite-accept catch-up path is still incomplete: a newly joined member can see replayed messages/media right away while associated offline reactions lag until a later global recovery pass. No direct repo-local regression currently proves same-session invite accept also converges offline reactions. |
| Accepting a pending invite still reaches a live subscribed group after bridge timeout or delayed join acknowledgement | `partial` | `materializeAcceptedGroupInvitePayload(...)` persists the group, members, and key before `callGroupJoinWithConfig(...)`, and on timeout or other bridge error it returns `bridgeError` while keeping the accepted group locally saved. `accept_pending_group_invite_use_case.dart` deletes the pending invite even on that path, and current UI surfaces tell the user that recovery is still catching up. Automatic rejoin does exist on startup, resume, and broader recovery orchestration through `rejoin_group_topics_use_case.dart`, `startup_router.dart`, `handle_app_resumed.dart`, and `main.dart`. The gap is narrower than a permanent orphan claim: there is no direct repo-local regression proving that an accept returning `bridgeError` always converges to a live joined topic promptly, and there is no explicit user-facing retry affordance if automatic recovery does not. |
...
| Persistent join/acceptance audit trail in the chat history | `missing` | No current join-accept system event is emitted when an invitee accepts from the pending-invite flow. |
...
- Pending-invite acceptance already treats `bridgeError` as a degraded accepted state rather than a hard reject. The group can be persisted locally while topic join is still pending later recovery, and current snackbars already say recovery is still catching up. What is missing is explicit proof and UX ownership that this degraded state always converges promptly.
...
- No direct repo-local test was found for an `invite accepted` or `member joined` timeline event after pending-invite acceptance.
...
- Existing reaction drain coverage proves `group_reaction` replay works when `reactionRepo` is supplied, and current startup/resume wiring later drains group inboxes with `reactionRepo` present. But no direct repo-local test currently proves `acceptPendingGroupInvite(...)` recovers queued offline reactions in the same immediate catch-up window, or even documents that the current behavior intentionally defers those reactions until a later continuity sweep.
- `handle_incoming_group_invite_use_case_test.dart` proves the lower-level `group:join` timeout path returns `bridgeError` while keeping the group persisted locally, and the shipped UI already surfaces `recovery is still catching up` for that result. But no direct repo-local regression currently proves `acceptPendingGroupInvite(...)` plus later startup/resume/watchdog recovery converges that accepted group back to a live subscribed topic promptly, or defines an explicit retry UX if automatic recovery does not.
