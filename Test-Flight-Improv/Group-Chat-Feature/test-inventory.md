...
| `PL-006` | Covered | PL-006 accepted during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-PL-006-plan.md`. `go-relay-server/media_test.go` adds `TestPL006RemovedPeerCannotDownloadPostRemovalGroupMedia`, proving a removed peer receives `not authorized` for post-removal media while an active recipient can download the bytes. Existing MD-011 fake-network and offline replay coverage now carries PL-006 row tags without losing the MD-011 selector text: `group_media_fanout_test.dart` has `PL-006 MD-011 removed member is excluded from future media descriptors and downloads`, and `drain_group_offline_inbox_use_case_test.dart` has `PL-006 MD-011 removed member cannot decode future media replay with only the old epoch`. `group_multi_party_device_real_harness.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add `pl006RemovedMediaProof` for `private_online_remove`, validating post-removal upload ACLs, Bob media download, Charlie direct-download denial, zero output bytes, no Charlie post-removal message, no media rows, and no pending downloads. Evidence passed: focused PL-006 relay/fake-network/replay/criteria selectors, old MD-011 fake-network/replay selector preservation, `private_online_remove` criteria selector, PL-005 media ACL preservation, GM-004/ML-005/KE-006 preservation selectors, gofmt, Dart format with 0 changed, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779298927256` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_online_remove_m8Kl1s` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, with orchestrator verdict `private_online_remove verdicts valid for alice, bob, charlie`. Broad `groups` remained red at `+250 -9` on known non-PL-006 residuals; `completeness-check` remained red at `735/736` on unmatched `test/shared/fakes/fake_group_pubsub_network_test.dart`. PL-007+ post-readd media, reactions, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `PL-007` | Covered | PL-007 accepted during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-PL-007-plan.md`. `group_media_fanout_test.dart` adds `PL-007 re-added member downloads only post-readd media`, proving Bob receives/downloads Alice's removed-window media while Charlie has no removed-window descriptor, message, media row, pending download, `media:download`, `blob:decrypt`, direct-download bytes, or plaintext; after re-add Bob and Charlie both download post-readd media and Charlie still has no removed-window artifact. `group_multi_party_device_real_harness.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add `pl007ReaddMediaProof` for `private_readd_current`, validating removed-window ACLs, post-readd ACLs, Bob downloads in both windows, Charlie direct-download denial for removed-window media, zero removed-window residue, and Charlie post-readd download/persist/decrypt. Evidence passed: focused PL-007 fake-network selector (`+1`), focused PL-007 criteria selector (`+5`), PL-005 and PL-006 fake-network preservation selectors, GE-023 smoke selector, `private_readd_current` criteria selector, PL-006 replay and relay preservation selectors, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779300390272` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_readd_current_WyxlOa` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, with orchestrator verdict `private_readd_current verdicts valid for alice, bob, charlie`. Broad `groups` remained red at `+250 -9` on known non-PL-007 residuals; `completeness-check` remained red at `735/736` on unmatched `test/shared/fakes/fake_group_pubsub_network_test.dart`. PL-008+ media progress, reactions, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `PL-008` | Covered | PL-008 accepted during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-PL-008-plan.md`. Production code stayed untouched because current main already coalesces `media:upload_progress` while keeping `group_message:received` lossless below dispatcher capacity. `go-mknoon/node/node_test.go` adds `TestPL008EventDispatcherCoalescesMediaProgressWithoutDroppingGroupMessages`, proving thirty progress events coalesce while six interleaved group message events deliver once, FIFO, with latest progress preserved, `coalesced > 0`, and `dropped = 0`. `go_bridge_client_test.dart` adds `PL-008 bridge routes group messages while media progress events arrive`, proving twenty progress events do not suppress three interleaved group-message callbacks. `group_messaging_smoke_test.dart` adds `PL-008 media upload progress storm does not drop fake-network group messages`, proving thirty-six progress events during Alice/Bob fake-network sends still leave Alice, Bob, and Charlie with exactly one copy of each message and the final upload progress observed. Evidence passed: focused PL-008 Go, bridge, and fake-network selectors; native DE-011/DE-012/DE-020 preservation; media progress bridge preservation; DE-012 listener preservation; DE-020 fake-network preservation; gofmt; Dart format; scoped analyzer; and scoped diff hygiene. The first parallel fake-network attempt failed only on Flutter native-asset install-name rewriting and passed on serial rerun. Smoke and 3-Party E2E are `N/A`, so no simulator/live proof is required or claimed. PL-009+ reactions, media ACL/download/rendering, upload retry, notifications, privacy, stress, broader dispatcher-overflow rows, source-doc, and COMPLETE_1 closure remain separate. |
| `PL-009` | Covered | PL-009 accepted during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-PL-009-plan.md`. `send_group_reaction_use_case_test.dart` adds `PL-009 active member publishes reaction command and stores local reaction once`; `handle_incoming_group_reaction_use_case_test.dart` adds `PL-009 active sender reaction applies once to the correct message`; and `group_reaction_roundtrip_test.dart` adds `PL-009 media-free active member reaction reaches Alice and Charlie exactly once`. `group_multi_device_real_harness.dart` adds the row-owned reaction repo/outbox/listener stream wiring needed by live proof, while `group_multi_party_device_real_harness.dart`, `run_group_multi_party_device_real.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `private_reaction_roundtrip`. Evidence passed: focused PL-009 app/fake-network/criteria selectors, existing reaction send/receive/fake-network preservation selectors, runner discovery, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779310544911` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_reaction_roundtrip_gcZ0Nw` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, with orchestrator verdict `private_reaction_roundtrip proof passed: private_reaction_roundtrip verdicts valid for alice, bob, charlie`. The known side-path `GROUP_REACTION_INBOX_STORE_FAILED` for missing `recipientPeerIds` remains outside PL-009 active-route scope. PL-010+ removed/re-added reaction rows, media, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `PL-010` | Covered | PL-010 accepted during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-PL-010-plan.md`. `send_group_reaction_use_case_test.dart` adds `PL-010 removed member reaction send is rejected without publish or local mutation`; `handle_incoming_group_reaction_use_case_test.dart` adds `PL-010 removed sender reaction is ignored without mutating visible state`; and `group_reaction_roundtrip_test.dart` adds `PL-010 removed member reaction is ignored by Alice and Bob without visible mutation`. `group_multi_party_device_real_harness.dart`, `run_group_multi_party_device_real.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `private_removed_reaction_rejected`, proving Alice/Bob visible reaction state remains unchanged while removed Charlie retains the old local target and the app-peer reaction attempt is rejected without local reaction persistence. Evidence passed: focused PL-010 app/fake-network/criteria selectors, PL-009 reaction preservation selectors, GM-004/GM-016 removed-member smoke preservation selectors, Go native `TestGL008|TestGK025|TestGK033|TestGA003`, runner discovery, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779311981796` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_removed_reaction_rejected_3Ssozo` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, with orchestrator verdict `private_removed_reaction_rejected proof passed: private_removed_reaction_rejected verdicts valid for alice, bob, charlie`. PL-011+ re-added reaction rows, media, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `PL-011` | Covered | PL-011 accepted during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-PL-011-plan.md`. `send_group_reaction_use_case_test.dart` adds `PL-011 re-added member with current key publishes reaction and stores once`; `handle_incoming_group_reaction_use_case_test.dart` adds `PL-011 re-added sender reaction applies once after current membership update`; and `group_reaction_roundtrip_test.dart` adds `PL-011 re-added member reaction reaches Alice and Bob exactly once`. `group_multi_party_device_real_harness.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `pl011ReaddReactionProof` on the existing `private_readd_current` path, proving Charlie reacts only after current re-add/key state and Alice/Bob observe exactly one stream reaction. Evidence passed: focused PL-011 app/fake-network/criteria selectors, PL-009/PL-010 reaction preservation selectors, `private_readd_current` PL-007/PL-004/ML-007/RA-016 preservation selectors, GM-006/GM-007 membership smoke selectors, runner discovery, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779313301896` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_readd_current_8mwrgL` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, with orchestrator verdict `private_readd_current proof passed: private_readd_current verdicts valid for alice, bob, charlie`. PL-012+ media schema rows, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `PL-012` | Covered | PL-012 accepted during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-PL-012-plan.md`. `go-mknoon/bridge/bridge_test.go` adds `TestPL012GroupPublishOptsPreserveMediaSchemaVariants`; `send_group_message_use_case_test.dart` adds `PL-012 media schema variants survive live publish and replay payloads`; and `group_media_fanout_test.dart` adds `PL-012 fake-network media schema variants survive fanout and downloads`. `group_multi_party_device_real_harness.dart`, `run_group_multi_party_device_real.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `pl012`, proving image, video, file, GIF, and voice descriptors preserve content type, dimensions, duration, waveform, thumbnail/blob ids, file metadata, hashes, and encryption metadata across bridge publish opts, replay, fanout, downloads, and live proof verdicts. Evidence passed: focused PL-012 Go/app/fake-network/criteria selectors, PL-002 bridge/app/fake-network preservation, PL-011 criteria preservation, runner discovery, gofmt, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779315442068` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_pl012_IzemJ1` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, with orchestrator verdict `pl012 proof passed: pl012 verdicts valid for alice, bob, charlie`. First live attempt `1779314975420` exposed current-main privacy-preserving verdict shape, so row-owned harness/criteria fixtures now accept `mediaAttachments` plus `hasEncryptionMetadata` instead of requiring raw encryption key/nonce fields. PL-013 partial-download cleanup, PL-014 metadata leak checks, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `PL-013` | Covered | PL-013 accepted during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-PL-013-plan.md`. `go-mknoon/node/media.go` now routes native download body writes through `copyMediaDownloadToFile`, and `media_test.go` adds `TestPL013MediaDownloadRemovesPartialOutputOnIncompleteTransfer`, proving incomplete native writes delete the partial output and same-path retry succeeds. `download_media_use_case.dart` now deletes partial output candidates for explicit failed `media:download` responses, and `download_media_use_case_test.dart` adds `PL-013 removes partial failed download and retry succeeds`, proving failed-result cleanup plus retry-to-done behavior. `group_media_fanout_test.dart` adds `PL-013 partial fake-network media download cleans up and retries`, proving Bob's first fake-network download writes partial bytes then fails, leaves no local partial or localPath, records failed attachment state, and succeeds on row-owned retry. Evidence passed: Go PL-013 selector, Go idle-timeout preservation, focused Flutter unit selector, focused fake-network selector, MD-012 unit preservation, PL-012/PL-006/PL-007 fake-network preservation, gofmt, Dart format, scoped analyzer, and scoped diff hygiene. A combined preservation bundle reproduced a non-PL-013 failure in `one recipient media download failure remains observable per recipient` before any download path because that existing selector lacks current-main key bootstrap. 3-Party E2E is N/A, so no simulator/device proof is required or claimed. PL-014 metadata leak checks, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `PL-014` | Covered | PL-014 accepted during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-PL-014-plan.md`. Production behavior stayed untouched because current main already bounds relay/media transport metadata and sanitizes diagnostics while preserving per-object encrypted media metadata only where the encrypted message descriptor requires it. `go-mknoon/node/media_test.go` adds `TestPL014MediaMetadataAndProgressEventsDoNotExposeSecrets`, proving media upload/list metadata and upload progress/complete events omit group-key, plaintext, private-key, and unsafe secret-shaped fields. `send_group_message_use_case_test.dart` adds `_expectNoForbiddenKeys` and `PL-014 media metadata omits group keys plaintext and private keys from diagnostics and relay replay`, proving media publish descriptors omit protected fragments, relay inbox replay remains opaque, diagnostics stay sanitized, and forbidden secret-shaped keys are absent. `group_media_fanout_test.dart` adds `expectNoFragmentsInJson`, `expectNoForbiddenKeys`, and `PL-014 fake-network media fanout keeps relay records and download metadata free of group secrets`, proving relay records, recipient download commands, persisted downloaded attachment metadata, and publish descriptors stay free of protected fragments and unsafe secret-shaped keys. Evidence passed: focused PL-014 Go/app/fake-network selectors, native upload-progress preservation, GO-008/EK-002/GI-035 pending-inbox log privacy preservation, IR-014 relay replay opacity preservation, gofmt, Dart format, scoped analyzer, and scoped diff hygiene. 3-Party E2E is N/A, so no simulator/device proof is required or claimed. PL-005/PL-006/PL-007 media ACLs, PL-012 schema rendering, PL-013 partial-download cleanup, MD-004 key-separation implementation, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-001` | Covered | UP-001 accepted during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-001-plan.md`. Production behavior stayed untouched because current main already had the supporting membership/config sync behavior. `group_membership_config_sync_use_case_test.dart` adds `create add remove and re-add keep local DB and Go config payloads aligned`, proving DB members and generated `groupConfig` state hashes remain aligned. `group_membership_smoke_test.dart` adds `UP-001 create add remove and re-add keep DB and bridge groupConfig snapshots aligned`, proving bridge `group:updateConfig` snapshots match DB membership through each operation. `group_info_wired_test.dart` adds `UP-001 reloads visible member list from local DB after create add remove and re-add`, proving the visible member list reloads from local DB truth. `pubsub_test.go` adds `TestUP001UpdateGroupConfigTracksAddRemoveReaddValidator`, proving native validator acceptance follows add/remove/re-add config updates. `group_multi_party_device_real_harness.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `up001MembershipConfigSyncProof` on `private_readd_current`. Evidence passed: focused UP-001 Go/application/fake-network/widget/criteria selectors, native validator preservation, gofmt, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779318063584` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_readd_current_VnO3xx` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, with orchestrator verdict `private_readd_current proof passed: private_readd_current verdicts valid for alice, bob, charlie`. UP-002+ timeline/UI state rows, notifications, media/reactions, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-002` | Covered - `blocked_external_fixture` | UP-002 row-owned host and criteria coverage was imported during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-002-plan.md`, but the row is not fully covered by current live proof because it is closed as `blocked_external_fixture`. `group_membership_timeline_message_test.dart` adds `UP-002 builds readable durable add remove and re-add timeline events`, proving readable add/remove timeline text, stable ids, ordering, delivered/incoming state, and created timestamps. `group_membership_smoke_test.dart` adds `UP-002 timeline shows durable add remove and re-add events after reopen`, proving Alice, Bob, and Charlie retain durable add, remove, and re-add timeline rows after listener restart with final Alice/Bob/Charlie membership. `group_multi_party_device_real_harness.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `up002DurableTimelineProof` on `private_timeline_truth`. Evidence passed: focused UP-002 unit/fake-network/criteria selectors, `private_timeline_truth` criteria preservation, GM-024 preservation, Dart format, scoped analyzer, and scoped diff hygiene. Required iOS 26.2 `private_timeline_truth` live proof failed before verdict twice: run `1779318990228` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_timeline_truth_n9VBNV` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, and retry run `1779319473323` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_timeline_truth_Ajm5fk` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `CD5929A6-EA0A-421D-A6D3-55BD707E0F76`; both runs failed in the existing ML-015 self-removal/rejoin handoff before `up002DurableTimelineProof` could execute. UP-003 compose gating, UP-004 unread counts, UP-005 invite state, UP-006 stale banner/system row, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-003` | Covered | UP-003 accepted during worktree-to-main integration on 2026-05-20 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-003-plan.md`. Production behavior stayed untouched because current main already had active-member and current-send-key compose gating. `send_group_message_use_case_test.dart` adds `UP-003 active member without current key cannot send until key is installed`, proving a member without a current key cannot publish or create a local message until key generation 2 is installed. `group_conversation_wired_test.dart` adds `UP-003 composer enables only for an active member with current key`, proving stale removed callbacks do not publish, removed users see the read-only banner, pending re-add without a current key sees the key-wait banner, and the composer returns after current key installation. `group_membership_smoke_test.dart` adds `UP-003 removed and pending re-add member cannot send until current key is installed`, proving fake-network removed and pending re-add states reject sends before current key installation and allow a send at the new epoch. `group_multi_party_device_real_harness.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `up003ComposeGateProof` on `private_readd_current`. Evidence passed: focused UP-003 application/widget/fake-network/criteria selectors, `private_readd_current` criteria preservation, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779320829314` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_readd_current_nhu2AN` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, with orchestrator verdict `private_readd_current proof passed: private_readd_current verdicts valid for alice, bob, charlie`. UP-004 unread counts, UP-005 invite state, UP-006 stale banner/system row, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-004` | Covered - `blocked_external_fixture` | UP-004 row-owned host and criteria coverage was imported during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-004-plan.md`, but the row is not fully covered by current live proof because it is closed as `blocked_external_fixture`. Production behavior stayed untouched because current main already had the supporting unread-count behavior. `handle_incoming_group_message_use_case_test.dart` adds `UP-004 excludes removed-window unread and clears post-readd unread on open`, proving removed-window traffic is ignored for a removed recipient's unread count and post-readd unread clears after opening the group. `group_membership_smoke_test.dart` adds `UP-004 unread counts exclude removed-window traffic and clear after readd open`, proving active recipients count removed-window traffic, removed Charlie does not count or display removed-window traffic, post-readd traffic counts for Charlie, and unread clears after open. `group_multi_party_device_real_harness.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `up004UnreadChurnProof` on `private_timeline_truth`. Evidence passed: focused UP-004 application/fake-network/criteria selectors, `private_timeline_truth` criteria preservation, unknown-sender cutoff preservation, GM-024 preservation, Dart format, scoped analyzer, and scoped diff hygiene. Required iOS 26.2 `private_timeline_truth` live proof failed before verdict: run `1779322181727` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_timeline_truth_OZYti4` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`; Charlie repeatedly skipped offline inbox decode with a missing epoch-1 replay key, then exited before verdict, Alice timed out waiting for `gmp_1779322181727_charlie_self_removed`, and Bob timed out waiting for `gmp_1779322181727_rejoin_key.json`. UP-005 invite state, UP-006 stale banner/system row, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-005` | Covered | UP-005 accepted during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-005-plan.md`. Production behavior stayed untouched because current main already had the supporting invite-state persistence and Group Info rendering behavior. `create_group_with_members_use_case_test.dart` adds `UP-005 invited but unjoined members stay pending or failed until joined evidence`, proving delivered invitees remain `sent`, failed Dave remains `needsResend`, and only explicit joined evidence promotes Bob to `joined`. `group_info_wired_test.dart` adds `UP-005 pending and failed invite states are visually distinct from joined members`, proving Bob renders `Joined`, Charlie renders `In their inbox`, Dave renders `Resend needed`, Eve renders `Cannot send`, non-joined rows do not contain `Joined`, only Dave has the resend CTA, and Eve has cannot-send detail. `group_membership_smoke_test.dart` adds `UP-005 invite delivery statuses keep pending failed and joined recipients distinct`, proving fake-network invite delivery statuses remain distinct and failed Dave has no joined local group. `group_invite_status_matrix_harness.dart` and `run_group_invite_status_matrix_sim.dart` add and validate `up005InviteStateProof` with `relayLifecycleProof=false` on the seeded Group Info display proof. Evidence passed: focused UP-005 application/widget/fake-network selectors, ML-004 preservation, GM-036 preservation, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 seeded display proof run `1779323319703` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_invite_status_matrix_LMHGyP` on creator/Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, accepted-one/Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, accepted-two/Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, and pending-unaccepted/Dana `CD5929A6-EA0A-421D-A6D3-55BD707E0F76`, with orchestrator verdict `Group invite status matrix display proof passed`. Relay/testpeer lifecycle invite delivery, unread counts, notifications, share targeting, media, reactions, security/privacy, stress, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-006` | Covered - `blocked_external_fixture` | UP-006 row-owned host and criteria coverage was imported during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-006-plan.md`, but the row is not fully covered by current live proof because it is closed as `blocked_external_fixture`. `group_info_wired.dart` now resolves latest add/join evidence from `member_joined`, `member_added`, and `members_added` events before rendering active member state. `group_membership_timeline_message_test.dart` adds `UP-006 re-add timeline row stays active added state after removal`; `group_info_wired_test.dart` adds `UP-006 re-added member renders joined instead of stale removed or pending invite`; and `group_membership_smoke_test.dart` adds `UP-006 re-add system row and member state stay active after reopen`. `group_multi_party_device_real_harness.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `up006ReaddUiStateProof` on `private_timeline_truth`, requiring active re-add labeling, ordered remove-before-readd timeline truth, Alice/Bob/Charlie final membership, and no stale removed or pending-invite state reuse. Evidence passed: focused UP-006 application/widget/fake-network/criteria selectors, `private_timeline_truth` criteria preservation, UP-002/UP-004/UP-005/UP-001/GM-024/ML-004/GM-036 preservation checks, Dart format, scoped analyzer, and scoped diff hygiene. Required iOS 26.2 `private_timeline_truth` live proof failed before verdict: run `1779324592018` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_timeline_truth_E9x4di` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`; Charlie repeatedly skipped offline inbox decode with a missing epoch-1 replay key, then timed out waiting for self-removal, Bob timed out waiting for `gmp_1779324592018_rejoin_key.json`, Alice timed out waiting for `gmp_1779324592018_charlie_self_removed`, and the orchestrator exited before Charlie wrote a verdict. UP-007 transaction guard, UP-008 restart retry, UP-009 sender identity, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-007` | Covered | UP-007 accepted during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-007-plan.md`. Production transaction-guard behavior stayed untouched because current main already had the supporting guard in `db_write_transaction.dart`, `GoBridgeClient.send`, and `FakeBridge.send`. `group_bridge_transaction_guard_test.dart` adds `UP-007 create add remove re-add and send keep bridge calls outside write transactions`, using instrumented group and message repositories to mark DB write transaction windows and an instrumented bridge to timestamp every command. The test runs `createGroupWithMembers`, `addGroupMember`, `removeGroupMember`, re-add, and `sendGroupMessage`, then asserts no `group:create`, `group:updateConfig`, `group:publish`, `group:inboxStore`, signing, encryption, invite, or other bridge command overlaps a write window. Evidence passed: focused UP-007 selector, full `db_write_transaction_guard_test.dart` core guard suite, focused `drain_lock_window_test.dart --plain-name "no bridge.send falls inside any inbox-page transaction window"`, Dart format, scoped `dart analyze`, scoped `flutter analyze --no-pub`, and scoped diff hygiene. Smoke, Fake Network, and 3-Party E2E are `N/A`, so no simulator/device proof was required or claimed. UP-008 restart retry, UP-009 sender identity, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-008` | Covered | UP-008 accepted during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-008-plan.md`. Production send/retry/resume behavior stayed untouched because current main already had the supporting behavior in `send_group_message_use_case.dart`, `retry_failed_group_inbox_stores_use_case.dart`, and lifecycle resume wiring. `group_messages_db_helpers_reliability_test.dart` adds `UP-008 pending outbound retry row survives database restart and stays eligible`, proving a file-backed outgoing `pending` row with `wire_envelope` and `inbox_retry_payload` survives close/reopen and remains selected by `dbLoadGroupMessagesWithFailedInboxStore`. `retry_failed_group_inbox_stores_use_case_test.dart` adds `UP-008 restart retry promotes pending outbound row without duplicate rows`, proving a fresh post-restart bridge retries one persisted pending row, promotes that same row to `sent`, sets `inboxStored`, clears retry material, creates no duplicate, and is idempotent. `group_resume_recovery_test.dart` adds `UP-008 pending outbound group message survives restart and reconciles through inbox retry`, proving app-restart-style lifecycle recovery closes Alice's pending row in place, Bob keeps one live copy, Carol drains one replay copy, and no pending/failed duplicate remains. Evidence passed: three focused UP-008 selectors, Dart format, scoped analyzer with `No issues found`, and scoped diff hygiene. 3-Party E2E is `N/A`, so no simulator/device proof was required or claimed. UP-009 sender identity, notifications, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-009` | Covered - `blocked_external_fixture` | UP-009 row-owned host and criteria coverage was imported during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-009-plan.md`, but the row is not fully covered by current live proof because it is closed as `blocked_external_fixture`. Current-main `group_sender_display_name.dart`, `group_conversation_screen.dart`, and `group_conversation_wired.dart` already had the display-helper and current-member render plumbing. `handle_incoming_group_message_use_case.dart` now prefers the current member username for post-re-add inbound rows after the latest sender-removal cutoff and avoids overwriting it with stale wire usernames. `group_conversation_screen_test.dart` adds `UP-009 re-added sender label prefers current member identity over stale wire username`; `group_messaging_smoke_test.dart` adds `UP-009 re-added sender identity uses current member label when wire name is stale`; and `group_multi_party_device_real_harness.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `up009ReaddSenderIdentityProof` on `private_timeline_truth`. Evidence passed: focused UP-009 widget/fake-network/criteria selectors, `private_timeline_truth` criteria preservation, Dart format, scoped analyzer, and scoped diff hygiene. Required iOS 26.2 `private_timeline_truth` live proof failed before verdict: run `1779326806744` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_timeline_truth_TQIlp4` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`; Charlie repeatedly skipped offline inbox decode with a missing epoch-1 replay key, then timed out waiting for self-removal, Bob timed out waiting for `gmp_1779326806744_rejoin_key.json`, Alice timed out waiting for `gmp_1779326806744_charlie_self_removed`, and the orchestrator exited before Charlie wrote a verdict. UP-010 route-target notifications, notification/share-target rows, media/reaction rows, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-010` | Covered - `blocked_external_fixture` | UP-010 row-owned host, integration, and criteria coverage was imported during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-010-plan.md`, but the row is not fully covered by current live proof because it is closed as `blocked_external_fixture`. `prepare_notification_route_target_use_case.dart` now accepts the local peer id for group inbox drain, `resolve_group_notification_route_target_use_case.dart` only opens an existing group when the local peer is a current member and otherwise uses pending-invite or missing-state routing, and `main.dart` plus `startup_router.dart` pass the current identity peer id into notification route preparation/resolution. `resolve_group_notification_route_target_use_case_test.dart` adds UP-010 active-current-member, stale-removed-member, re-add drain recovery, and pending-invite fallback tests; `notification_deeplink_integration_test.dart` adds `UP-010 group push routes only after current local membership is resolved`; and `group_multi_party_device_real_harness.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `up010NotificationRouteProof` on `private_timeline_truth`. Evidence passed: focused UP-010 application/integration/criteria selectors, `private_timeline_truth` criteria preservation, Dart format, scoped analyzer, and scoped diff hygiene. Required iOS 26.2 `private_timeline_truth` live proof failed before verdict: run `1779327964029` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_timeline_truth_2lnqcg` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`; Charlie repeatedly skipped offline inbox decode with a missing epoch-1 replay key, then timed out waiting for self-removal, Bob timed out waiting for `gmp_1779327964029_rejoin_key.json`, Alice timed out waiting for `gmp_1779327964029_charlie_self_removed`, and the orchestrator exited before Charlie wrote a verdict. UP-011 mute behavior, UP-012 removed-member notification privacy, UP-014 share targets, media/reaction rows, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-011` | Covered - `blocked_external_fixture` | UP-011 row-owned host, fake-network, and criteria coverage was imported during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-011-plan.md`, but the row is not fully covered by current live proof because it is closed as `blocked_external_fixture`. Production mute behavior stayed unchanged because current main already persists local group mute state and suppresses local group notifications without suppressing delivery. `set_group_muted_use_case_test.dart` adds `UP-011 persists local mute without changing group membership`; `group_message_listener_test.dart` adds `UP-011 muted group persists unread delivery without local notification`; and `group_membership_smoke_test.dart` adds `UP-011 muted group keeps delivery unread and suppresses notifications through churn`. `group_multi_party_device_real_harness.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `up011MutedDeliveryProof` on `private_timeline_truth`, requiring Bob's local mute, delivery through normal/removal/re-add traffic, unread updates, zero notification delta, final Alice/Bob/Charlie membership, and final epoch convergence. Evidence passed: focused UP-011 mute/listener/fake-network/criteria selectors, `private_timeline_truth` criteria preservation, Dart format, scoped analyzer, and scoped diff hygiene. Required iOS 26.2 `private_timeline_truth` live proof failed before verdict: run `1779329038402` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_timeline_truth_DCCOPs` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`; Charlie repeatedly skipped offline inbox decode with a missing epoch-1 replay key, then timed out waiting for self-removal, Bob timed out waiting for `gmp_1779329038402_rejoin_key.json`, Alice timed out waiting for `gmp_1779329038402_charlie_self_removed`, and the orchestrator exited before Charlie wrote a verdict. UP-012 removed-member notification privacy, UP-013 route lifecycle, UP-014 share targets, media/reaction rows, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-012` | Covered | UP-012 accepted during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-012-plan.md`. Production behavior stayed unchanged because current main already excludes removed members from durable group recipients and suppresses local notifications for non-current or absent group recipients. `send_group_message_use_case_test.dart` adds `UP-012 post-removal sends exclude removed members from durable notification recipients`, proving Bob remains the only durable notification recipient, Charlie is excluded, notification title/body metadata is omitted from durable inbox store payloads, and replay message id/text are preserved. `group_membership_smoke_test.dart` adds `UP-012 removed member receives no post-removal notifications until rejoin`, proving fake-network post-removal delivery and local notifications exclude Charlie and no notification payload/preview leaks `While removed`. `group_multi_party_device_real_harness.dart`, `run_group_multi_party_device_real.dart`, `group_multi_party_device_criteria.dart`, and `group_multi_party_device_criteria_test.dart` add and validate `private_removed_notification_privacy` with `up012NotificationPrivacyProof` for Alice, Bob, and Charlie. Evidence passed: focused UP-012 unit/fake-network/criteria selectors, all-scenario-list preservation, `private_online_remove` criteria preservation, runner discovery, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779330599692` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_removed_notification_privacy_ggTtmI` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, with orchestrator verdict `private_removed_notification_privacy proof passed: private_removed_notification_privacy verdicts valid for alice, bob, charlie`. UP-013 route lifecycle, UP-014 share targets, background push fallback redesign, media/reaction rows, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-013` | Covered | UP-013 accepted during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-013-plan.md`. Production behavior stayed unchanged because current main already persists incoming group messages before UI broadcast and reloads conversation route state from `GroupMessageRepository` on route build/reopen. `group_message_listener_test.dart` adds `UP-013 persists incoming group message without UI stream subscriber`, proving app-level listener persistence does not require a mounted conversation route or subscribed UI stream. `group_conversation_wired_test.dart` adds `UP-013 route reopen renders message persisted while widget unmounted`, proving a message saved while the widget is absent renders after reopening from repository hydration. `group_messaging_smoke_test.dart` adds `UP-013 route-unmounted recipient persists incoming message for reopen`, proving fake-network listener persistence while no conversation route is mounted and later repository load visibility. Evidence passed: focused UP-013 listener/widget/smoke selectors, Dart format, scoped analyzer, and scoped diff hygiene; the first parallel listener-selector attempt failed only on a macOS native-assets `lipo` startup/build race, and the serial rerun passed. Fake Network and 3-Party E2E are `N/A`, so no simulator/device proof was required or claimed. UP-014 share targets, outbound route-unmount send durability, notification-open routing, media/reaction rows, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `UP-014` | Covered | UP-014 accepted during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-UP-014-plan.md`. `share_target_picker_wired.dart` now loads only locally writable group targets by requiring a current identity, current local member row, latest group key, non-archived/non-dissolved group state, and announcement admin role where applicable. It also revalidates selected groups after runtime readiness and before delivery, dropping stale group ids if the local member was removed after the picker loaded. `share_target_picker_wired_test.dart` adds `UP-014 filters removed pending and dissolved groups from share targets`, proving removed-member, pending/no-key, dissolved, and announcement-reader groups are absent while a current writable group remains selectable, and `UP-014 stale removed group selection is dropped before delivery`, proving a group selected while writable is not delivered after the local member row is removed before Send. Existing share picker selectors for active contact/writable group loading, multi-target send, and partial-failure selection were updated to seed current member/key state. Evidence passed: focused UP-014 selector (`+2`), existing share picker regression selectors for loading, coordinator send, and partial failure, full `share_target_picker_wired_test.dart` (`+11`), Dart format, scoped analyzer, and scoped diff hygiene. 3-Party E2E is `N/A`, so no simulator/device proof was required or claimed. Contact sharing, batch delivery result semantics, media policy, notification routing, group conversation compose behavior, source-doc, and COMPLETE_1 closure remain separate. |
| `SV-001` | Covered | SV-001 accepted during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-SV-001-plan.md`. COMPLETE_1/current-main GA-002 and GA-026 already covered raw non-member validator rejection and safe validator diagnostics, so the integration imported only the missing row-owned delta. `pubsub.go` now redacts local unauthorized publish/reaction errors; `pubsub_authorization_forward_test.go` adds `TestSV001NeverMemberRawPubSubRejectsBeforeAcceptAndForward`, proving a never-member cannot locally publish, cannot raw-inject through PubSub, is not accepted or forwarded, and diagnostics avoid plaintext group id, peer id, and attacker public-key leakage. `group_message_listener_test.dart` adds `SV-001 never-member message is quarantined before stream storage or notification`; `group_messaging_smoke_test.dart` adds `SV-001 never-member fake-network publish is rejected by all recipients`; and the live harness, runner, criteria validator, and criteria tests add `private_never_member_publish_rejected`. Evidence passed: focused SV-001/GA Go selector bundle, runner discovery, listener selector, fake-network selector, SV-001 criteria selector (`+4`), LP-002/RP-017 Go preservation bundle, gofmt, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779333243408` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_never_member_publish_rejected_z2Oq7f` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, and Dana `CD5929A6-EA0A-421D-A6D3-55BD707E0F76`, with orchestrator verdict `private_never_member_publish_rejected proof passed: private_never_member_publish_rejected verdicts valid for alice, bob, charlie, dana`. SV-002+ removed/re-added/forged/tampered/replay security rows, notification/media/share rows, Android, physical iOS, source-doc, and COMPLETE_1 closure remain separate. |
| `SV-002` | Covered | SV-002 accepted during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-SV-002-plan.md`. COMPLETE_1/current-main GA-003 and GM-017 already covered overlapping removed-member config/key and stale-subscription rejection, so the integration imported only the missing row-owned delta. `group_message_listener.dart` now bypasses membership-dependent buffering for unknown/non-member traffic with an older key epoch than the current group key so removed old-key sends fail closed through the existing removed-after-cutoff path. `pubsub_authorization_forward_test.go` adds `TestSV002RemovedMemberOldKeyRawPubSubRejectsBeforeAcceptAndForward`, proving Charlie cannot locally publish, cannot raw-inject through PubSub, is not accepted or forwarded, and diagnostics avoid sensitive leakage. `handle_incoming_group_message_use_case_test.dart`, `group_message_listener_test.dart`, and `group_messaging_smoke_test.dart` add direct/listener/fake-network proofs that timeline, latest-message, unread, notification, and reaction state do not mutate. The live harness, runner, criteria validator, and criteria tests add `private_removed_old_key_publish_rejected`. Evidence passed: focused SV-002/GA/GM Go selector bundle, focused SV-002 Flutter selector bundle (`+7`), runner discovery, SV-001 preservation selector (`+1`), GM-017 affected overlap selector bundle (`+7`), gofmt, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779335139616` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_removed_old_key_publish_rejected_fApKDh` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, with orchestrator verdict `private_removed_old_key_publish_rejected proof passed: private_removed_old_key_publish_rejected verdicts valid for alice, bob, charlie`. Existing blockers and adjacent SV rows remain separate. |
| `SV-003` | Covered | SV-003 accepted during worktree-to-main integration on 2026-05-21 by `Private_group_chat_reliability_test_matrix_full_with_rules-worktree-to-main-integration-session-INTEGRATE-SV-003-plan.md`. COMPLETE_1/current-main and UP-003 already covered the production sender-side current membership/config/key gate in `send_group_message_use_case.dart`, so production send behavior was inspected and left unchanged. The integration imported only the missing row-owned proof surfaces. `pubsub_key_rotation_grace_test.go` adds `TestSV003PublishBlockedUntilCurrentConfigAndKeyInstalled`, proving a re-added member with stale config/key cannot publish until current config and key are installed. `send_group_message_use_case_test.dart` adds `SV-003 pending re-add cannot publish until current config and key are installed`; `group_messaging_smoke_test.dart` adds `SV-003 pending re-add publish is blocked until current config and key`; and the live harness, criteria validator, and criteria tests add `sv003PendingReaddCurrentProof` to `private_readd_current`. Evidence passed: focused SV-003/GA/GM Go selector bundle, focused SV-003 Flutter selector bundle (`+4`), UP-003 preservation selector bundle (`+4`), private-readd criteria preservation selector (`+49`), runner discovery, gofmt, Dart format, scoped analyzer, scoped diff hygiene, and iOS 26.2 live proof run `1779336387722` in `/var/folders/nd/_55d26s936d0fb_5l9s00t980000gn/T/group_multi_party_private_readd_current_r9dt84` on Alice `5A9A8286-001B-4BF1-8F40-5A3AB8BF8FE3`, Bob `279B82AE-2BB9-4924-9AAE-581870ED3FA9`, and Charlie `116B4AF6-C1A9-4F36-B929-0A7130B5E83C`, with orchestrator verdict `private_readd_current proof passed: private_readd_current verdicts valid for alice, bob, charlie`. Existing blockers and adjacent SV rows remain separate. |
...
| PL-009 active member publishes reaction command and stores local reaction once | Active-member publish and local storage |
...
| PL-009 active sender reaction applies once to the correct message | Active-sender incoming reaction idempotence |
...
| PL-009 media-free active member reaction reaches Alice and Charlie exactly once | Three-role active-member reaction fan-out |
...
| | PL-009 accepts private reaction roundtrip verdicts | Validates three-role active reaction stream and storage evidence |
| | PL-009 rejects reaction proofs without receiver stream evidence | Fails missing Alice/Charlie reaction stream proof |
