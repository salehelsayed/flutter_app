...
1. **Silent send-drop after a change.** A member still on the previous epoch publishes; every recipient's validator rejects it as `bad_signature_or_epoch`. The sender sees "sent"; nobody receives it.
...
- `hasKeyRotationGrace()` gates on `now.Before(keyInfo.GraceDeadline)` — `go-mknoon/node/pubsub.go:1221-1226`.
- `GraceDeadline` is stamped `time.Now().Add(KeyRotationGracePeriod)` **at the moment each receiver applies the new key** — `UpdateGroupKey` (`pubsub.go:834`) and `RefreshJoinedGroupStateIfNewer` (`pubsub.go:773`). A member who learns of the rotation late still gets only 30s relative to *its own* clock.
- The grace bound is checked in both `verifyGroupEnvelopeSignature` (`pubsub.go:1239`) and `decryptGroupEnvelopePayload` (`pubsub.go:1256`).
- Out-of-grace previous-epoch envelopes fail verification → `logPubSubValidationReject("bad_signature_or_epoch", …)` → `pubsub.ValidationReject` — `pubsub.go:1519-1521`. GossipSub treats `ValidationReject` as malicious: no re-delivery, no re-fetch, and the peer is penalized.
...
- `decryptGroupEnvelopePayload` returns `"no group key available for epoch %d"` for anything outside `{current, prev-within-grace}` — `pubsub.go:1248-1261`.
- **Mitigation already present (do not regress):** the *live* path is the only caller of `decryptGroupEnvelopePayload` (`pubsub.go:1570`). Inbox/offline replay decrypts in Dart via `getKeyByGeneration` against the persisted per-generation `group_keys` table (`group_repository.dart:62`), so historical epochs *are* retained for replay. The 30s/single-prev limit constrains **only the in-memory live path.**

### C. Future-epoch live message rejected (Go)

- `verifyGroupEnvelopeSignature` accepts only `env.KeyEpoch == KeyEpoch` or `== PrevKeyEpoch` within grace; a future epoch (`env.KeyEpoch > keyInfo.KeyEpoch`) falls through to `return false` → `ValidationReject` (`pubsub.go:1233-1245`, `1519-1521`). The reliable send path still stores the envelope to the relay group inbox, so it can self-heal via inbox replay — but on the live path the peer is penalized and the message is dropped.

### D. Live decrypt-fail recovery is reactive only (Go → Dart)

- On missing key / decrypt error / parse error, `handleGroupSubscription` emits a diagnostic (`group:decryption_failed` / `group:payload_parse_failed`) and `continue`s — `pubsub.go:1562-1566`, `1572-1576`, `1604-1607`. No in-Go key/config re-request.
- `emitGroupDecryptionFailed` carries `groupId/senderId/keyEpoch/error/decryptMs` (+ optional `localKeyEpoch`) and **never the ciphertext or messageId** — `pubsub.go:1664-1675`.

### E. Key-repair is fully passive; no active pull (Dart)

- Every `RequestGroupKeyRepair` callsite is wired to `emitGroupKeyRepairRequest`, which only calls `emitFlowEvent('GROUP_KEY_REPAIR_REQUESTED')` — `group_pending_key_repair_service.dart:56-67`. **It sends nothing over the wire.** A repo-wide grep for `group_key_request` / `requestMissingKey` / `key_repair_pull` returns no implementation.
- Retries fire only from `GroupKeyUpdateListener._retryPendingGroupKeyRepairs` after a *new* `group_key_update` is saved (`group_key_update_listener.dart:534-539`), scoped to one `(groupId, keyEpoch)`.

### F. Pending repairs only retried on the next key update — not on resume or timer (Dart)
...
- `queueLiveGroupDecryptionFailureRepair` persists a placeholder + repair with `replayEnvelopeJson: null` and a **synthetic** id `live:groupId:senderId:keyEpoch:localKeyEpoch` used as both `repair.id` and `message.id` — `group_pending_key_repair_service.dart:264-307`.
- `_retryOne` short-circuits at `'waiting for replay envelope'` whenever `replayEnvelopeJson` is null — `group_pending_key_repair_service.dart:424-441`. A live placeholder can **never** decrypt via retry.
- The only supersede call is inside the durable-offline path (`queueMissingGroupReplayKeyRepairFromEnvelope` → `_supersedeLiveDiagnosticRepairForDurableReplay`, line 145, gated by `_isLiveDiagnosticRepairForSender` requiring `id.startsWith('live:')` && `replayEnvelopeJson == null`, lines 199-243).
- The **live** receive path `_handleMessage` calls `_emitGroupMessage(result)` on success (`group_message_listener.dart:736-737`) but **never supersedes** the live placeholder. The real message persists under its real wire `messageId`, which never matches the synthetic `live:` id → **stuck placeholder + duplicate** whenever the real message arrives via the live/membership-flush/push-loss path rather than the durable inbox drain.

### H. Single replay failure → permanent "undecryptable" (Dart)

- `_retryOne`'s catch block (`group_pending_key_repair_service.dart:531-545`) re-queues (`recordAttempt`, return false) **only** when `key == null` AND the error string contains `'Missing group replay key'`. For **any** other error it immediately calls `_finalizeUndecryptable`, rewriting the message to `Message could not be decrypted.`
- The happy path throws `StateError('replay validation rejected')` (`:510`), `StateError('replay did not replace pending placeholder')` (`:518`), `StateError('missing reaction repository')` (`:458`), reaction rejections (`:472`) — all terminal on first occurrence even when `getKeyByGeneration` would return the key. `repair.attempts` is tracked (`group_pending_key_repair.dart:19`) but never gates the terminal transition.
...
| R4 | Repair retries are triggered by exactly one event (a fresh `group_key_update` for the same epoch). No resume sweep, no timer, no "retry all groups". | Dart |
...
- `decryptGroupEnvelopePayload` (`pubsub.go:1248-1261`) and `verifyGroupEnvelopeSignature` (`pubsub.go:1228-1246`): allow **decrypt + verify of any retained epoch** the node holds, regardless of `GraceDeadline`. Security is enforced by the existing membership/permission check (`isAllowedWriter`, device-signature verify), **not** by the timer.
...
- **Affected:** `group_pending_key_repair_service.dart`, `group_key_update_listener.dart`, `group_message_listener.dart`, `lib/main.dart` (replace the `emitGroupKeyRepairRequest` wiring at `:1766,1778,1875,1960,3053,3178` and `startup_router.dart:596`), plus a new incoming handler use case.

### 4. Retry pending repairs on resume, after rejoin, and on a bounded timer (R4) — Dart, effort medium

- Add to the repository interface (`group_pending_key_repair_repository.dart`): `getAllPendingRepairs({int limit})` and/or `getPendingRepairsForGroup(String groupId)`, with the corresponding DB-helper query (`SELECT … WHERE status = 'pending_key'`).
...
- **DB impact:** new read query only; no schema change (existing `status` index suffices, add one if absent).

### 5. Supersede live placeholders from the real-message path + give them a TTL (R5) — Go + Dart, effort medium

Two complementary fixes:

- **(a) Dart — reconcile on live persist (the precise gap):** in `group_message_listener._handleMessage`, after a successful `_emitGroupMessage(result)` (`group_message_listener.dart:736-737`), call a supersede that deletes any `live:` pending-key placeholder/repair for the same `groupId + senderPeerId + keyEpoch`. Reuse `_supersedeLiveDiagnosticRepairForDurableReplay` logic (`group_pending_key_repair_service.dart:199-233`) but drive it from the live path, not only the durable-replay path. This removes the stuck-placeholder-plus-duplicate outcome.
- **(b) Go — make live placeholders replayable:** include the **encrypted envelope (or at least the wire `messageId`)** in `group:decryption_failed` (`emitGroupDecryptionFailed`, `pubsub.go:1664-1675`). Then `queueLiveGroupDecryptionFailureRepair` can persist a real `replayEnvelopeJson` and `_retryOne` can decrypt/replay once the key arrives (instead of looping `waiting for replay envelope`), and the placeholder can be deduped by real `messageId`.
...
- **Wire impact:** `group:decryption_failed` gains an optional `envelope`/`messageId` field (additive, backward compatible).

### 6. Make "undecryptable" terminal only on confirmed-cryptographic failure after bounded attempts (R6) — Dart, effort small

Rework `_retryOne`'s catch block (`group_pending_key_repair_service.dart:531-545`):
...
| `go-mknoon/node/pubsub.go` | Retained-epoch decrypt/verify (`:1228-1261`); future-epoch → `ValidationIgnore` + `group:key_epoch_behind` (`:1519-1522`); append-not-overwrite in `UpdateGroupKey`/`RefreshJoinedGroupStateIfNewer` (`:764-776`, `:828-836`); add envelope/messageId to `group:decryption_failed` (`:1664-1675`) | 1, 2, 5b |
| `lib/features/groups/application/group_pending_key_repair_service.dart` | Real outbound pull; live-path supersede; TTL self-clear; classify-and-bound `_retryOne` finalize (`:424-574`) | 3, 5a/c, 6 |
| `lib/features/groups/application/group_message_listener.dart` | Supersede live placeholder after real persist (`:736-737`); handle `group:key_epoch_behind` | 5a, 2 |
| `lib/features/groups/application/group_key_update_listener.dart` | Trigger pull when retry finds key still missing (`:534-539`) | 3, 4 |
...
| `lib/main.dart`, `lib/.../startup_router.dart` | Replace `emitGroupKeyRepairRequest` wiring with production pull (`main.dart:1766,1778,1875,1960,3053,3178`; `startup_router.dart:596`) | 3 |
...
### Unit tests (Go)
- `pubsub` table tests: a previous-epoch envelope is **decryptable/verifiable beyond 30s** as long as the key is retained (extends `pubsub_key_rotation_grace_test.go:31-60`); eviction past K generations correctly rejects.
...
- Live placeholder supersede: real message via `_handleMessage` deletes the `live:` placeholder and finalizes its repair (no duplicate).
- Outbound pull: missing key triggers a `group_key_repair_request`; admin handler re-delivers the exact epoch and does not mint a new one.
- Resume sweep: persisted pending repair whose key now exists is repaired on `handleAppResumed`.
...
| **New `group_key_repair_request` wire type and additive `group:decryption_failed` field** must stay backward compatible during staged rollout. | All additions are optional/additive; older peers ignore unknown fields. The pull is best-effort — old admins simply won't answer, falling back to today's passive behaviour (strictly no worse). |
| **DB query additions** (`getAllPendingRepairs`). | Read-only query; add an index on `status` if not present. No destructive migration. |
...
5. #5b/c envelope-in-diagnostic + TTL (Go + Dart).
...
| 5. Supersede live placeholders + replayable diagnostics + TTL | Medium | Go + Dart |
