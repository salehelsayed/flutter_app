...
- `group_info_wired.dart:1182` — `_buildGroupConfig(group, members)` embedded in the `member_role_updated` system payload.
- `group_config_payload.dart:41`–`:54` — the payload includes `name`, `description`, `avatarBlobId`, `avatarMime`, and `metadataUpdatedAt`.

On receive, the `member_role_updated` dispatch (`group_message_listener.dart:1761`–`1779`) and the `members_added` path gate **only** on `_shouldIgnoreStaleMembershipEvent` (the membership watermark), **not** on `_shouldIgnoreStaleMetadataEvent` (the metadata watermark, which is only applied to `group_metadata_updated` at `:1800`). `_handleMemberRoleUpdated` calls `_applyAuthoritativeGroupConfigSnapshot` (`:3541`), which at `:4629`–`:4650` overwrites `name`/`description`/`avatarBlobId`/`avatarMime` **unconditionally** and sets `lastMetadataEventAt: resolvedMetadataUpdatedAt ?? group.lastMetadataEventAt` (`:4647`–`:4648`) — with no comparison of the snapshot's `metadataUpdatedAt` against the receiver's current `lastMetadataEventAt` before overwriting visible metadata. A stale snapshot reverts the name, and `lastMetadataEventAt` is preserved, so the regression sticks until the next metadata edit.

### 3. Metadata edit commits locally before publish; failure leaves the admin diverged with no rollback

`update_group_metadata_use_case.dart:69`–`78` builds the updated group and persists `groupRepo.updateGroup(updated)` **inside** the use case — with no broadcast and no rollback hook beyond the optional `beforePersist` callback. The actual `callGroupPublish` (`group_info_wired.dart:1585`) and `callGroupInboxStore` (`:1632`) run **afterward, outside** the use case's commit. The surrounding `catch` (`group_info_wired.dart:1670`–`1691`) only emits `GROUP_INFO_FL_METADATA_UPDATE_ERROR`, shows a snackbar, and calls `_loadGroupInfo()` — it does **not** revert `groupRepo.updateGroup`.
...
`group_membership_event_watermark.dart:38`–`44` — `isStaleGroupMembershipEvent` returns `true` for any `eventAt` **not strictly after** `lastMembershipEventAt`. The sender uses wall-clock microsecond timestamps as the version: `changedAt = DateTime.now().toUtc()` (`group_info_wired.dart:1117`) embedded as `microsecondsSinceEpoch` in the `sourceEventId` (`:1162`) and passed as `eventAt`. The receiver's `_shouldIgnoreStaleMembershipEvent` (`group_message_listener.dart:4085`) drops events where `!eventAt.isAfter(watermark)`. The role/dissolve dispatch (`:1763`, `:1782`) calls it **without** `allowEqualVersionReplay`, and only the add path has the `_membershipAddAdvancesLocalMember` escape hatch (`:4089`–`:4100`). There is no `(eventAt, sourceEventId)` tuple, no per-actor sequence number, and the sender never advances to `max(now, last + 1µs)`. Single-device microsecond collisions are improbable; the realistic trigger is **cross-admin clock skew** producing equal/inverted timestamps.

### 5. (Low) Receiver-side validator config-sync failures are logged but not reconciled in-session

`_syncGroupConfig` (`group_message_listener.dart:4739`–`4798`) tries once, retries once, returns `false`. Callers — `_handleMemberRoleUpdated` (`:3546`–`3559`), `_handleGroupMetadataUpdated` (`:3621`–`3634`), and the members-added paths (`:2932`, `:3317`) — only `emitFlowEvent('CONFIG_SYNC_FAILED')` and continue. The DB apply via `_applyAuthoritativeGroupConfigSnapshot` is already committed, so the DB reflects new membership while the Go validator still enforces the old config; the member can then send messages the rest of the group rejects. No pending-config-resync marker exists anywhere in `lib/features/groups`.
...
| B | Membership/role events carry a full `GroupConfig` (including visible metadata), but receivers gate those events only on the **membership** watermark, never the **metadata** watermark. | Stale snapshots from membership events silently revert name/description/avatar. |
...
> Ordered by value-to-effort. Improvements 1–2 are the high-value, low/medium-effort core of this theme; 3 closes the metadata-atomicity gap; 4–5 are the larger ordering/observability hardening.
...
In `group_message_listener.dart`, `_applyAuthoritativeGroupConfigSnapshot` must only overwrite visible metadata when the snapshot's `metadataUpdatedAt` is *strictly newer* than the receiver's `lastMetadataEventAt` (or equal-and-repairing, mirroring `_metadataReplayRepairsVisibleFields` at `:4441`).
...
Key points:
- The **member-set and role deltas are always applied** (that's what membership/role events are *for*); only the visible-metadata fields are conditionally preserved.
- The avatar-changed / avatar-delete bookkeeping at `:4615`–`:4627` must be gated behind `applyVisibleMetadata` too, so a stale snapshot doesn't delete the on-disk avatar.
- `lastMetadataEventAt` must **never** move backward.

**Stronger alternative (recommended long-term):** exclude visible metadata fields entirely from membership/role snapshots in `buildGroupConfigPayload` (a `includeVisibleMetadata: false` mode), so name/description/avatar reconcile *only* through `group_metadata_updated` events. This removes the regression vector at the source rather than re-guarding it on every receive path. It is a **wire-shape change** to the snapshot payload, so it must be rolled out behind the existing `schemaVersion`/state-hash machinery and kept backward-compatible with peers still sending full snapshots — which is why the receive-side guard above is the safer first step.
...
2. **Wired flow** (`group_info_wired.dart:1585`–`1658`): wrap `callGroupPublish` + `callGroupInboxStore` in a try/catch. On failure:
   - **Minimum:** revert `groupRepo.updateGroup` to the captured pre-edit snapshot, delete the optimistic `metadataTimelineMessage` (`:1572`–`1581`), and surface a clear "changes not sent" error (not a generic snackbar).
...
2. **Tie-breaker on compare.** Extend the stale check to a `(eventAt, sourceEventId)` tuple so two *distinct* events at the same instant are ordered deterministically instead of conflated:
   ...
   ```
   The receiver's `_shouldIgnoreStaleMembershipEvent` (`group_message_listener.dart:4085`) and the watermark store must then persist the winning `sourceEventId` alongside `lastMembershipEventAt`.

**DB impact:** persisting the watermark's tie-break `sourceEventId` needs a nullable `last_membership_event_id` column (migration). **Wire impact:** none new — `sourceEventId` is already minted and shipped (`group_info_wired.dart:1162`); this change *consumes* it for ordering. Because this touches the watermark contract on both sender and receiver, it is the highest-risk item and should land last, after 1–3 stabilize.

### 5. (Low) In-session config-sync retry + user signal (medium)

On `_syncGroupConfig` returning `false`, persist a per-group pending-config-resync marker (can reuse the `pending_group_broadcasts` table from Improvement 3) and re-attempt the validator update on the next listener tick / app foreground / `rejoinGroupTopics`, rather than only at the next app restart. Optionally surface a transient "reconnecting group" indicator. This narrows the existing self-heal window (currently bounded by `rejoin_group_topics_use_case.dart:131`–`139`) from "next restart" to "next tick" and makes the drift visible.
...
| `lib/features/groups/application/group_message_listener.dart` | Gate visible-metadata overwrite in `_applyAuthoritativeGroupConfigSnapshot` on metadata watermark (`:4615`–`:4650`); tie-breaker in `_shouldIgnoreStaleMembershipEvent` (`:4085`); in-session config-sync retry at `_syncGroupConfig` callers (`:3546`, `:3621`, `:2932`, `:3317`) (Improvements 2, 4, 5). |
...
| `lib/features/groups/presentation/screens/group_info_wired.dart` | Wrap publish+inbox in try/catch with revert or pending-resend; mint role version via `nextMembershipEventAt` (`:1117`, `:1585`–`1692`) (Improvements 3, 4). |
...
| Stricter metadata gating (Imp. 2) could *suppress* a legitimate metadata change that legitimately rides a membership event. | Today metadata is supposed to flow via `group_metadata_updated`; the membership snapshot is meant only to carry member/role deltas. The equal-version repair path (`_metadataReplayRepairsVisibleFields`) is preserved so genuine repairs still land. Validate with the convergence harness. |
...
**Rollout order:** (1) role lock + stale gate → ship and observe; (2) metadata regression guard (receive-side first); (3) metadata atomicity/rollback; (4) tie-breaker (sender monotonicity first, then comparator); (5) in-session config-sync retry. Items 1–2 deliver most of the trust win at low risk and need no wire changes.
...
| 2. Stop membership snapshot metadata regression (receive guard) | **Medium** (~1–1.5 days) | None (strong variant: wire-shape change) |
