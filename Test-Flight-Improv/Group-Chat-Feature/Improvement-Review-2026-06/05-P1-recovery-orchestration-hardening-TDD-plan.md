...
> **PHASE 1b + PHASE 2 IMPLEMENTED 2026-06-17 (uncommitted, host-green, TDD; adversarial /workflow review CLEAN — 0 findings).** Unblocked once findings 02/06/10 committed (`b63c19d5`) leaving `handle_app_resumed.dart` + the drain use-case clean.
> - **Phase 1b** `handle_app_resumed.dart`: the resume outbound-repair follow-ons (3d recover-stuck, 3e upload retry, 3f retry-failed) are now wrapped in `runWithGroupRecoveryGate` so `isGroupRecoveryInProgress()` stays true while resume re-sends group messages (per-step try/catch preserved; finding-02 Step 8i key-repair stays outside). New test in `handle_app_resumed_group_recovery_test.dart` (mutation-verified).
> - **Phase 2** `drain_group_offline_inbox_use_case.dart`: `GroupOfflineInboxDrainResult.hasMorePages` + `_drainGroupInbox` `onFirstPageStopped` callback + new `drainGroupOfflineInboxContinuation`. `handle_app_resumed.dart`: resume drains the fast **first page** (`drainAllPages: false`, feeds ack via `isSuccessful`) inside the gate, then `if (drainHasMorePages) unawaited(drainGroupOfflineInboxContinuation(...))` OUTSIDE the gate. Verified safe: `AcknowledgeGroupRecovery` only clears the node's `needsGroupRecovery` flag (does NOT purge the relay inbox), cursor advance is atomic, drain is dedup-idempotent → no loss; continuation resumes from page 2 (no page-1 re-process). Conditional `hasMorePages` keeps small-inbox (≤1 page) tests unaffected. 2 drain unit tests + a resume wiring-lock. Gates: drain + resume + integration **231 green** (only red = pre-existing `:1538`); analyze/format clean.
> **PHASE 4 IMPLEMENTED 2026-06-17 (uncommitted, host-green, TDD, SQLCipher device-proofed 5/5 on iPhone 16e).** Migration **087** `group_message_retry_backoff_columns` (DB v87; 086 was taken by finding-07): `retry_attempt_count` + `next_eligible_at` on `group_messages` (read in `fromMap`, **never written by `toMap`** so row updates can't clobber backoff state; managed by dedicated setters). Per-row exponential backoff (base 30s ×2, cap 30min) + terminal `send_failed` after 10 attempts in `retry_failed_group_messages_use_case`; eligibility filter skips backed-off + terminal rows; manual retry re-arms a terminal row (`resetRetryStateForManualRetry`); reconnect clears backoff (`clearRetryBackoff`, wired into the retrier online transition). Jitter (P1.6) = injectable `Random?` (null→unchanged `Timer.periodic`; prod→self-rescheduling ±20%). UI: `send_failed` renders as failed (LetterCard icon/color/label) + shows the existing manual-retry affordance (group_conversation_screen `isFailedSend`). Repo defaults + `noSuchMethod` fakes meant only `InMemoryGroupMessageRepository` needed the 3 new methods. Tests: 4 use-case backoff/terminal/manual/reconnect + 2 jitter + 2 LetterCard send_failed + SQLCipher proof `group_message_retry_backoff_db_proof_test.dart` (5/5 iPhone 16e). Broad group regression: 294 green (fixed 1 repo-impl test that built `group_messages` without 087).
>
> **PHASE 3 GO (per-group ack) — INVESTIGATED, NOT A CLEAN CHANGE.** The Go recovery model is fundamentally **node-wide**: `RelaySessionManager.needsGroupRecovery` is a single bool set by the watchdog (`RecordWatchdogRestart`, `relay_session.go:665`) and cleared by `AcknowledgeGroupRecovery` (`:652`); the Go node has **no per-group recovery state** (and doesn't know the group list — groups live in Dart/DB). A `groupId`-parameterized ack has no per-group state to clear, so it would be a no-op param + a gomobile rebuild of the shared xcframework for zero behavioral value. The node-wide ack + the **Dart eligibility fix (Phase 3, done)** — ack over the recoverable set so one transient/no-key group no longer blocks it — is the correct complete solution. True per-group recovery would require redesigning the Go signal to be per-group (feed the group list Dart→Go, per-group state), a separate large feature with unclear value.
>
> **PHASE 3 BOUNDED REJOIN RETRY IMPLEMENTED 2026-06-17 (the meaningful Dart-only half; uncommitted, host-green, TDD, SQLCipher device-proof pending iPhone 16e).** Migration **088** `group_rejoin_state(group_id PK, rejoin_attempt_count, next_eligible_at)` (DB v88; onCreate+onUpgrade<88). `rejoin_group_topics_use_case` loads rejoin states once per pass; a group inside its backoff window is **skipped as `RejoinOutcome.deferred`** (not attempted → not an error → does NOT block the node-wide ack, mirroring the no-key carve-out); on rejoin error it records failure (increment + exp backoff base 30s ×2 cap 30min) and emits `GROUP_REJOIN_PERMANENTLY_STUCK` at attempt ≥10; on success it clears the row. Accessors live on `GroupRepository` (3 default-no-op methods + `GroupRejoinState` class → zero threading through the 4 rejoin call sites); `GroupRepositoryImpl` injects db helpers, `InMemoryGroupRepository` implements in-memory. 3 new tests (deferred-skip + ack-not-blocked, permanently-stuck event, clear-on-success); rejoin suite 32 green. **Regression: stash-confirmed 0 new failures** — the `group_repository_impl_test` `last_membership_event_id` reds are finding-07's (its setUp omits migration 083), not mine.
