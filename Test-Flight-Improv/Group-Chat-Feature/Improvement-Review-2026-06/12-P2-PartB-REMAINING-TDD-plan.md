...
> **All host-implementable phases DONE + VERIFIED GREEN 2026-06-17 (R1, R2, R3, R4, R5-lock; see "Host-verification closeout" below).** Only the inherently-external steps remain: R6 (device-matrix run) and R7 (flag flip + UX-013 re-close), which depend on a real-device run that cannot execute on host. Decisions taken: D1=safety-number prompt, D2=TOFU, D3=B5 off-by-default, D4=one-shot-with-retry. Everything inert until `kMultiDeviceSyncEnabled` + device verification.

- **R1 — emit-on-restore: DONE.** New `group_device_announce_marker.dart` (one-shot, written on restore success) + `maybeAnnounceRestoredDeviceOnStartup` orchestrator (marker → build binding from `p2pService.currentState.peerId` + identity keys → `announceRestoredDeviceToGroups` → clear w/ D4 retry), wired into `startup_router._doStartP2P`. 6 tests.
- **R3 — B4 per-device safety numbers: DONE.** `build` optional `deviceFingerprints` (byte-identical v1 when empty, `v2` when present); `compare` nullable `savedDevices` (null = exact v1); **migration `084`** `group_member_device_snapshots` + helpers + `GroupMemberDeviceSnapshotRepository` mixin; TOFU helper `resolveGroupMemberDeviceSafety` wired into both group screens via runtime cast; **gated by `kMultiDeviceSyncEnabled`** (flag-OFF = exact v1, no cross-version mismatch). 17 tests; migration chain v84 green.
- **R2 — trust-prompt gate: DONE.** The listener no longer auto-admits — `_handleDeviceAnnounce` now HOLDS the announce pending a user decision via an injectable hold seam. New `PendingSiblingDevice` model + **migration `085`** `pending_sibling_devices` + helpers + `PendingSiblingDeviceRepository` mixin; `holdPendingSiblingDevice` / `verifyAndAdmitPendingSiblingDevice` / `rejectPendingSiblingDevice` use cases (flag-gated); `PendingSiblingDevicePrompt` widget (shows the device's safety number + Verify/Reject) wired into `group_info_screen`'s security area, loaded + handled in `group_info_wired`. 8 backbone + 2 widget tests; full listener suite + group_info_wired + migration chain v85 green.
- **R4 — B5 enablement: DONE** (per the corrected batch finding). New default-off `kForwardRotateOnAddEnabled` flag + a **single post-batch rotation** in `contact_picker_wired._inviteSelected` after the config-sync + `members_added` broadcast (NOT the per-member `addGroupMember` rotation, which the batch `syncBridgeConfig:false` path skips). Best-effort, creator-gated, never reverts added members. Default-off → contact_picker regression unchanged (28 green).
- **R5 — legacy-fallback lock: DONE** (3 tests) — locks that a device-less member resolves via `legacyDeviceIdentity`, so **no backfill migration is needed**. R5 items 1 (hydrate single-read) + 3 (convergence-mirror port) intentionally NOT done — low value / the real path already has dedicated coverage.

**Corrected findings:** next free migration was **`084`** then **`085`** (concurrent `083` landed mid-session; DB v85 live now); B4 is **flag-gated** (v1→v2 is a user-facing security-string change with cross-version mismatch risk); B5 at contact_picker needed the **post-batch** rotation, not the per-member one.

**Remaining (inherently external — cannot run on host):**
- **R6 — device matrix:** the new `b1b_sibling_device_convergence` scenario (steps fully specified below) must run on a real iPhone-sim + Pixel with the real Go bridge and `--dart-define=...MULTI_DEVICE_SYNC=true` (test-scoped). The host sim (`group_sibling_device_admission_test`) proves the wiring; only the device run proves per-device ML-KEM key separation. *(Scenario role-handler code was intentionally NOT written blind into the 27k-line multi-party harness — it cannot be compiled/verified on host, so the precise steps below are the spec for the device run.)*
- **R7 — flag flip + UX-013 re-close:** enable `kMultiDeviceSyncEnabled` only AFTER R6 is green; flip `groupMultiDeviceImplemented` (already keyed off the flag, contract test stays green); re-close UX-013 citing the device verdict. The shipped const stays default-OFF.

## Host-verification closeout — 2026-06-17 (verify-and-close pass)

> **Verdict: all host-implementable Part B work is verified green.** No host work remains; only R6 (device matrix) + R7 (flag flip) are outstanding, both inherently device-gated. R5 items 1+3 stay deferred (owner decision: low value). Two host fixes were applied this pass (one unrelated-concurrent, one a real 12-P2 regression).

- **Dedicated phase suites: 33/33 green** (announce-on-startup, announce use case, contact safety number, group_member_device_safety, pending_sibling_devices helpers, listener device_announce, manage_pending_sibling_device, prompt widget, legacy-fallback lock).
- **Comprehensive single-process run** (`test/features/groups/` + `test/core/database/` + `test/features/identity/` + `test/features/contacts/`, ~3036 tests). Every failure triaged:
  - **18× `group_repository_impl_test`** — `table groups has no column named last_membership_event_id` = **finding 07** stale test migration list (NOT 12-P2). **Fixed**: added `runGroupsLastMembershipEventIdMigration` (mig 083) to the test's setUp. → 33/33 green.
  - **1× `ambient_background` guard** (`production code does not import 'Test-Flight-Improv'`) = **12-P2-introduced regression**: two Part-B doc comments embedded the scratch-dir plan path. **Fixed**: reworded `multi_device_sync_flag.dart:2` + `group_multi_device_policy.dart:24` to drop the `Test-Flight-Improv/` literal. → guard 13/13 green.
  - **1× `media_attachments_db_helpers`** — `no such column: download_retry_count` = **finding 09** stale test migration (NOT 12-P2; out of scope, left for that finding).
  - **2× `group_conversation_wired`** (voice-send / GMAR-004 reopen) = **pre-existing flakiness**: both pass in isolation (full file 136/136 green); documented shared-gate-file race under concurrent flutter processes. R3's only touch to that file is `_loadSecurityStatus`, orthogonal to voice/media.
- **`flutter analyze` clean** on all changed files (0 issues).
- **Adversarial closeout audit (3 agents) — all PASS:**
  - *R2 trust gate (the self-compromise vector): fail-closed.* No auto-admit path exists — the listener wires only a HOLD seam (`main.dart:2362`); an account-signed but unverified announce is persisted pending (no `saveMember`/reopen/drain); admission runs ONLY via the explicit user-tap `_onVerifyPendingSiblingDevice` after a safety-number compare; forged/unsigned/wrong-account-key announces are rejected at three independent gates.
  - *Completeness:* every R1–R5-lock artifact is present AND reachable end-to-end; both migrations (084/085) wired in onCreate + onUpgrade.
  - *Flag-safety:* both flags compile-time default-OFF; with flags OFF every path is inert and displayed safety numbers are byte-identical v1 (zero user-visible change shipped).
