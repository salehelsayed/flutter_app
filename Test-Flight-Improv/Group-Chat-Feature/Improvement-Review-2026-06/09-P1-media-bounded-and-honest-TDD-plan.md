...
**Status: IMPLEMENTED (host-gated, 2026-06-17)** &nbsp;|&nbsp; Branch: `124-harness-refactor` &nbsp;|&nbsp; Scope: finding-09 Items **1 + 2 + 3** (+ Item 4b client half); Items 4a/4b-relay and 4c **deferred**

> **Implementation closure (2026-06-17).** All three phases landed TDD, host-gated green. **Migration number moved to `089`** (DB version **88 → 89**; tree was at 88, not the plan's 83 — migrations 084–088 also landed concurrently). **Product decisions: OQ-1 caps = image 25 MB / video 250 MB / audio 16 MB / file 100 MB / total 500 MB (video bumped to 250 after confirming the app transcodes video before upload); OQ-2 = per-type caps extended to 1:1 + share.** Key design: `perMediaLimitBytes` made nullable → SEND sites derive per-type + type-aware reason; RECEIVE/render sites pass an explicit `kGroupMediaPerAttachmentLimitBytes` (= video cap, 250 MB) cross-type backstop so already-received media is never newly hidden. `integrity_failed` made **terminal/non-retryable** (INV-DL-3, owner-confirmed) — **reverses the MD-012 quarantined-retry affordance**; the MD-012 tests were reconciled (terminal "Couldn't verify this media" + no retry; retryable-affordance coverage kept via transient `failed` fixtures). Reset-on-success folded into `dbUpdateMediaLocalPath`. A 12-agent adversarial review found 4 defects, all fixed (share skip summary made media-agnostic; 1:1 recovery loop + listener brought to parity with the group fix so a terminal `download_failed` is never downgraded to retryable in-memory). **Gates:** groups suite **0 failures** (`-j 1`); conversation/share/media/migration/widget suites green; `flutter analyze` 0 new issues; the only red is the pre-existing 115 `retry_unacked` 'inboxed' failure (unrelated). **Pending:** device matrix (iPhone 13 + Pixel 6 — host floor met, hardware verification outstanding); the e2e `media_message_journey` extensions were treated as satisfied-equivalently by the wired proofs that drive the real `GroupConversationWired`/`ConversationWired` widgets (the journey harness fires the send-gate before its network layer + has no oversized/upload seam). **Deferred:** relay-side 4a + relay half of 4b (`go-relay-server/media.go`), 4c ACL backfill (OQ-3), OQ-4.
