...
**IMPLEMENTED + host-green 2026-07-04** — commits `8e655c3b` feat, `3a46d093` test, `7242cb9d` docs, `07d24d6e` fix. On branch `new-orbit`, on top of feat(200).
...
**Adversarial review (post-verdict, fix landed `07d24d6e`).** A code-review pass on the committed diff caught a real bug the tests missed: `bottomClearance` is 58–88 in production (never the widget tests' 0), so the per-band `max(base, clearance)` collapsed the small-base elements (steppers 28, pill 40) up to the clearance — the full-width pill then overlapped the chip strip on every find-with-results, and covered/stole taps from the −/+ steppers when armed. Fixed with a single uniform lift `base + max(0, clearance − 28)` that floors the whole find/edit chrome above the nav band while preserving the 28/40/88/96/144 stagger (hence mutual disjointness). New regression **TC-201-14** (host clearance 88, armed + find + chips, pairwise-disjoint sweep) reds the old `max()` code and greens the lift. Re-verified: sculpt+arcs+loading **+92**, analyze 0 new, F17/F18/20S unaffected (clearance-0 → lift 0).
