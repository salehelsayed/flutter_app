...
Status: closed — DTR-13 Plan-green and Wave 4A aggregate accepted
Type: Modification
Spec: `Test-Flight-Improv/dead-code-and-technical-debt-removal-roadmap.md`
(`DTR-13`)
Classification: implemented / Plan-green / Wave-accepted
...
| 2026-07-28 | Terminal Plan/Wave closer | final coherent Sims reports; focused/core/analysis/hygiene; protected hashes; incremental Graphify; tested-tree aggregate archive | All four live reports independently reverified on source digest `73f4339412d85b7846f45894a122aa174f87141771fa121666a3ff06a3f0e825`; the terminal focused set, `core-host-all`, strict analysis, hygiene, protected-hash check, graph refresh, and separate full `host-all` aggregate all passed. `DTR13-AUTH-01` remains unchanged. | Terminal — DTR-13 is Plan-green and Wave 4A is accepted. |
...
- [x] Causal RED, focused GREEN, and representative mutation re-RED evidence
      are recorded.
- [x] `sims-contracts`, simulations, `groups`, `intro`, `1to1`,
      `core-host-all`, `runtime-roots`, `architecture-boundaries`,
      completeness, strict analysis, final graph refresh, scope, and hygiene
      gates pass on the final coherent source. The four exact live reports were
      independently reverified, the terminal focused set passed 14/14, and the
      final core/analysis/graph receipts are recorded below.
- [x] DTR-12 remains exactly 182/24/92 with no manifest edit.
- [x] Scope Contract And Guard is respected.
- [x] Plan 289 and DTR-13 are Plan-green, and the separate **full** Wave 4A
      aggregate `host-all` passed with stable tested-tree evidence. The accepted
      receipt includes all 1,257 Flutter paths and all eight Go tails; no
      Dart-only run was used as Wave 4A acceptance evidence.
...
- Historical blocker-closure evidence retained from before the terminal
  coherent-source re-verification:
  ...
  - At that checkpoint all four live blockers were closed on one source digest.
    Plan-green still awaited the final integrated host/analysis receipts and
    Graphify refresh; the terminal rows below supersede that pending state.
...
| 2026-07-28 | GREEN — terminal Plan-green closure | four final coherent Sims reports; final focused set; core family; strict analyzer; hygiene/protected hashes; incremental Graphify | All four reports independently reverified PASS at source digest `73f4339412d85b7846f45894a122aa174f87141771fa121666a3ff06a3f0e825`; focused final set 14/14; `core-host-all` covered 362 items, passed 2,825 Flutter tests and the renderer contract, then final PASS; strict analysis exited 0 with zero diagnostics; `git diff --check` PASS; protected 182/24/92 hashes unchanged; final incremental graph refresh PASS. | No causal failure, validation error, protected drift, or DTR-12 exception change remained. | `DTR13-AUTH-01` is preserved and Plan 289 / DTR-13 is Plan-green. | run the separate cadence-protected Wave aggregate |
| 2026-07-28 | ACCEPTED — Wave 4A aggregate | exact full `host-all`; frozen tested tree; shared index; retained evidence archive | 1,265 planned items completed: 1,257 Flutter paths produced 12,798 pass / 1 skip / 0 fail, all eight Go tails passed, and final scope PASS exited 0 in 1,136 seconds. Tested tree `14a72da9b5b702b37466e155a77007e0335d0559` was frozen as synthetic commit `669c7039846f321a8171b63df09e087e43b1f319` and tag `dtr-wave4a-tested-tree-20260728`; shared-index hash and `HEAD` were unchanged. | The retained README and gzip archive bind the accepted aggregate log to the frozen tree. | Full Wave 4A evidence accepted; no terminal blocker or placeholder remains. | terminal |

## Final Wave 4A Evidence

- Terminal current-source reports were independently reverified. Each carries
  coherent source digest
  `73f4339412d85b7846f45894a122aa174f87141771fa121666a3ff06a3f0e825`,
  a passing build dependency, `validationErrors=[]`, and a PASS verdict:
  - `android.wake_token_directionality`: 2 assertions PASS;
  ...
  - `groups.reaction_notification_campaign`: 4 assertions PASS;
  ...
  - `intro.accept_notification_campaign`: 2 assertions PASS;
    ...
    The exact-anchor convergence correction remains Plan 252 product-routing
    ownership; Plan 291 remains harness-only.
  - `notifications.ios_payload_fast_path`: 7 assertions PASS;
    `build/sims/operator-inputs/wave4a-ios-final-coherent-report.json`, SHA-256
    `1dae730ef1a55c9328ad922b9fa95b68804ffa8ce667652bfbb627e149cee5fa`;
    proof artifact SHA-256
    `36dd4c649b40b2aa9ad9f139bb423ce618f8de20923f21fefa5fea5fe88ff653`.
- Earlier coherent-source blocker-close reports are retained as historical
  evidence and are superseded only for terminal current-source acceptance. Each
  carried coherent source digest
...
- Earlier affected exact/curated/family gate results, retained as historical
  evidence:
...
- Terminal final-source gate and hygiene receipts:
  - focused final set: 14/14 PASS;
  - exact
    `./scripts/run_host_test_gates.sh core-host-all --batch-flutter --concurrency 4 --reporter failures-only`:
    362 items (361 Dart paths plus the renderer manifest), 2,825 Flutter tests
    passed, renderer profile/release contract PASS, and final scope PASS in
    about 428 seconds; log SHA-256
  ...
  - `./scripts/check_flutter_analyze_strict.sh`: PASS, exit 0, zero diagnostics,
    three reviewed suppression occurrences across three package roots, 306.8
    seconds; log SHA-256
    `196b51c6554d0235ee187117f529daf6dab39fed469f61b11b7afa22637d656b`;
  - `git diff --check`: PASS. Protected hashes are unchanged at 182 forbidden
    dependencies, 24 placements, and 92 debug exceptions; no DTR-12 exception
    changed and the `DTR13-AUTH-01` contract remains intact.
  The terminal closure receipt includes these exact commands:
...
- The original reviewed TC-DTR13-11 discovery and eleven-row live receipt
  ...
  inventory.
- Final `./graphify-arch/refresh_arch_graph.sh --incremental` ran once: 13
  changed, 2,869 unchanged, zero deleted; 62,934 nodes / 94,552 edges; overlay
  1,447 files / 14,171 tests / 1,087 production targets. Graph SHA-256 is
  ...
  and log SHA-256 is
  `932181b2df4b64e1021b2f510b45c4d250d985926784a9e0e5a5f97836c8656f`.
- Full Wave 4A aggregate command:
  `./scripts/run_host_test_gates.sh host-all --continue-on-failure --batch-flutter --concurrency 4 --reporter failures-only`.
  It completed 1,265 planned items (1,257 Flutter paths plus eight Go tails).
  Flutter finished 12,798 pass / 1 skip / 0 fail; every Go tail passed; final
  scope PASS exited 0 in 1,136 seconds.
- The tested-tree receipt is base
  `765be74523b25ce592ff200ff435653b35c66b4d`, tree
  `14a72da9b5b702b37466e155a77007e0335d0559`, synthetic commit
  `669c7039846f321a8171b63df09e087e43b1f319`, and tag
  `dtr-wave4a-tested-tree-20260728`. The shared-index SHA-256 was
  `9aa55ac41a8db648ae4a8eb12a95820c0d3882ce09a01d1fa94ead8ef1c670dc`
  both before and after, and `HEAD` remained unchanged.
- Retained evidence is indexed in the
  [Wave 4A evidence README](evidence/dtr-wave4a/README.md). The original
  `wave4a-host-all-accepted.log` SHA-256 is
  `d61df5a5ab9949f44887bcf32c1e877dcc39a10ef110682e066bfb6cbbb4f899`;
  the retained `wave4a-host-all-accepted.log.gz` SHA-256 is
