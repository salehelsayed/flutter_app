...
| 2026-08-13 | Sender-history repair | Bootstrap authoring repository/transaction; protected authority preparation/activation; strict protected queue runner; key-rotation target identity | The ordinary primary now commits its signed secret-free genesis in the same transaction as the legacy-preserving roster, pending sibling and exact bootstrap outbox. Locally authored authority commits its authenticated prepared fact with the immutable physical-recipient rows. Activation proves the local projection and appends complete under the shared non-reentrant group phase; restart recovery loads and verifies prepared history, proves the secret-free projection (including key hash), appends complete, and only then permits protected egress or exact outbox retirement. Target-specific key replays use target-qualified authority event IDs, so immutable per-device proofs cannot collide. | Source prerequisite repaired without schema, new owner, generic fallback, relay change or admission/activation change. Genesis/prepared/complete facts remain after their pending owners retire. |
...
- Current state: `POST_EXECUTION_AUDIT_CLOSED / CODE_COMPLETE / HOST_VERIFIED / RELAY-BINARY-DEPLOYED / LIVE-ACCEPTANCE-BLOCKED / ADMISSION-DEFAULT-OFF / NOT RELEASE-ELIGIBLE`; implementation base `979c243492fcb1e953db9e18dd145a8d21e551c3`, first audit closure `bb2184fe07121a12d6515d95c32b019e1e8e1151`, sender-history closure in this changeset, and Graphify current at `fa124fa2c3be08f5` (72,895 nodes / 106,832 edges).
- Live-acceptance action: obtain the separate S2 production-admission authorization/operations receipt, set `DIRECT_INBOX_ACK_CUSTODY_ADMISSION_ENABLED=true` on the already Redis-backed relay, then rerun the single discovered B1b scenario on the physical Pixel 6 plus `emulator-5554`. Do not enable authoring or claim acceptance before that proof passes.
- Receipt boundary: original final focused c4 `+9`, 4/4 mutation re-reds reverted, exact node/relay PASS, corrected DTR c4 `+7`, discovery/runtime-roots/contracts PASS, curated groups `+4119` plus all Go tails; first bounded audit-repair c4 `+5`; sender-history c4 `+5`, targeted analyzer clean, 244/244 inventory, and one sender-repair incremental Graphify refresh/review. No mandatory broad gate was repeated for this repair.
...
- Successor work: Plan 364 may pin this post-execution closure and the unchanged 244-path groups inventory for blob-free group discussion/announcement/reaction host development. Plan 365 owns group media/blob/voice; later plans own activation, capability rollout and final GAP-N01/release closure.
