...
- Successor work: the next documentation commit pins this exact deferred/restart closure SHA, Graphify `681115963b6c3b2a` and the unchanged 244-path groups inventory into Plan 364 before blob-free group discussion/announcement/reaction host development. Plan 364 must also bind a signed self-contained inner `group_content_v1` discriminator because retrieval does not preserve the request custody kind. Plan 365 owns group media/blob/voice; later plans own activation, capability rollout and final GAP-N01/release closure.
