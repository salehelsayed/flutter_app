...
| 2026-08-19 | TC-386-11 → **3/5**, reaction lane PROVEN | `run_reaction_scenario_386.sh android_group_reaction_recipient_background_connected` | **PASSED on re-run** — the 82 s overrun was a one-off, not systemic. This is the first REACTION scenario to pass, so the counter-delta repair is now proven on the lane it was written for, not only on the two message lanes | `attempted` **4 → 6 (delta 2 == `expectedRelayWakeAttempts`)**; `invalid_or_disabled` **0 → 1** (the REMOVE, correctly NOT an attempt); `duplicate_suppressed` **0 → 1** (the exact redrive, correctly NOT an attempt); `route_error` and `incapable_skipped` **absent in both phases**; sentinel 7088 → 7190 | All four transitions landed in DIFFERENT counter buckets and only the two ADDs counted — accounting the dead-marker grep could never express | 2 killed scenarios remain |
