...
- **Single-relay default pool.** `config.go:11` single `DefaultRelayAddress`; `node.go:254-255` `if relayAddresses == nil { relayAddresses = []string{DefaultRelayAddress} }`. The pool machinery exists but is never seeded with >1 target by default. CONFIRMED by Read.
...
- "Switch the 1:1 InboxStore deposit from `ForEach` to `FanOut` (redundant deposit to independent stores)." REFUTED as the durability mechanism: with a SHARED Redis backend, ForEach (deposit once, retrieve-from-any sees it — `failover_test.go` `TestTwoRelayServers_SharedInboxBackend`) is correct AND cheaper. Redundant FanOut to *independent* memory stores would split custody and make `InboxRetrieve`'s first-success `ForEachWithResult` (`inbox.go:233`) MISS messages deposited on a non-first relay. The pool's correctness comes from the shared durable backend, not from fan-out deposit. (Keep token reg/unreg on `FanOut` as-is — `inbox.go:547,614` — that is per-front-end local state, correctly broadcast.)
...
- go-mknoon/node: ship a **default relay pool** (WSS+QUIC to the default peer now; deploy-injectable additional peers) via a `DefaultRelayAddresses()` helper consumed at `node.go:254-255`.
...
- `metrics.go` — Prometheus gauge registration pattern (mirror for `relay_backend_durable`).
...
- `node.go` — relay-address default + parse (:252-289). Consume the pool helper at :254-255.
...
- `go-mknoon/node/multi_relay_test.go`: 25 tests incl. `TestInboxStore_TriesSecondRelayWhenFirstFails`, `TestBuildRelaySelector_FallsBackToDefault`, `TestNewRelaySelector_TwoDistinctRelays`, `TestRelaySelector_FanOut_*`, `TestDialPeerViaRelay_SingleRelayStillWorks` — EXIST (lock pool/failover semantics). **Reuse; do not duplicate.**
...
- **GREEN-asserts** `addrs` contains BOTH `DefaultRelayAddress` (WSS) and `DefaultQUICRelay`; selector `.Len()==1` (same peer, merged) AND that one `RelayInfo.Addrs` has length 2 (WSS+QUIC transport redundancy). This ships transport redundancy *today* without requiring relay #2.
...
- **RED-on-HEAD-because** `node.go:254-255` hardcodes `[]string{DefaultRelayAddress}` → `n.relayAddresses` lacks the QUIC entry; asserting equality to `DefaultRelayAddresses()` fails (RED).
- **GREEN-asserts** `n.relayAddresses` equals `DefaultRelayAddresses()` (after `limitRelayAddresses` with default flags, which keeps all since same-peer multi-addr and `EnableMultiRelayRouting=true`).
- **Mutation-that-re-reds** revert :254-255 to `[]string{DefaultRelayAddress}` → re-reds (missing QUIC).
- **Distinct-event discriminator** n/a.
...
| Node default uses the pool | nil RelayAddresses → pool, not single | Go unit (host) | multi_relay_test.go::TestStartNode_NilRelayAddresses_UsesDefaultPool | node.go:254-255 hardcodes single | revert to `[]string{DefaultRelayAddress}` | `cd go-mknoon && go test ./...` | auto (go test) |
...
- INV-3: Durability is machine-checkable (`IsDurable` + summary + gauge) (TC-10-01/02).
...
5. **RED** TC-10-04: `TestStartNode_NilRelayAddresses_UsesDefaultPool`. **GREEN**: at `node.go:254-255` change `relayAddresses = []string{DefaultRelayAddress}` → `relayAddresses = DefaultRelayAddresses()` (seam: the nil-default branch). Re-run existing `multi_relay_test.go` + `feature_flags_runtime_test.go` (the latter reads `n.relayAddresses` length — confirm `TestStartNode_DisablesMultiRelayRoutingWhenFlagFalse` still expects 1 after `limitRelayAddresses` truncates a >1-PEER list; note our default is 1 PEER / 2 addrs, so `limitRelayAddresses` — which truncates by ADDRESS count when flag off, `feature_flags_runtime.go:26` — would cut QUIC when the flag is off; verify that test's expectation and adjust the assertion to match the new default, or keep the default single-address when `EnableMultiRelayRouting=false`). Stop-if: `feature_flags_runtime_test.go:58-59` breaks → reconcile (the flag-off path must remain single-address; `limitRelayAddresses` already truncates to `[:1]` when the flag is off — confirm WSS sorts first).
...
- **`limitRelayAddresses` truncates QUIC when `EnableMultiRelayRouting=false`** (`feature_flags_runtime.go:20-30` cuts to `[:1]` by address count). Pinned by step-5 reconciliation + `feature_flags_runtime_test.go:58-59`. Edge: WSS must be index 0 so the truncated single address stays the proven WSS path.
...
- `feature_flags_runtime_test.go:58-59` (expects `len(n.relayAddresses)==1` with multi-relay flag OFF) MUST stay green after step 5; a failure there means WSS did not sort to index 0 in the truncation — fix the default ordering, do not weaken the test.
...
## Done Criteria
- [ ] TC-10-01..04 RED-first then GREEN; each mutation re-reds.
...
- **Collision risk:** `go-mknoon/node/node.go` (Node.Start relay-address block, :252-289) and `config.go` (relay consts/timeouts) are co-edited by sibling FDC Go-host plans (warmPeer, cold-start re-timing); `go-relay-server/main.go` is co-edited by the presence-lookup plan (P1-1). Run FDC-10 sequentially with those on the shared files. No Dart collisions (FDC-10 touches no Dart prod).
