...
Status: IMPLEMENTED (host-green, uncommitted `new-orbit`) — 2026-06-26. RT-W1..W5 + RT-S1 + RT-I1 green; RT-S1a/RT-S2/PS-3 preserved; all 5 mutations re-red; `1to1` gate full pass (+1250); `flutter analyze` 0-new; `git diff --check` clean. `core-host-all`/`feature-host-all` blockers are PRE-EXISTING (privacy-allowlist `sinceProcessStartMs`, proven disjoint via stash-revert) / a transient native-asset codesign flake (passes standalone), not FDC-13.
...
- **Live code anchors (re-verified by Read on branch `new-orbit`, this revision):**
  ...
  - **`lib/core/services/p2p_service_impl.dart` (⚠️ documented `new-orbit` line-drift hazard — anchors below are current as of this revision but DRIFT; always locate by SYMBOL, never by line number):**
...
`p2p_service_impl.dart:3025-3027` (the `_inferTransportForPeer` upgraded-set branch — *locate by symbol, this file drifts*):
```dart
if (_peersUpgradedToDirect.contains(_shortId(peerId))) {
  return 'direct';   // ← :3026; the upgrade is KNOWN here but flattened to plain direct
}
```
...
- **Lifecycle / derived-state durability:** `'upgraded'` is persisted into the existing TEXT `transport` column via `copyWith(transport:)` (`p2p_service_impl.dart:317`) → durable across reloads exactly like `'direct'` (migration 012 column is free-text, no CHECK/enum). Old `'direct'` rows are not retroactively rewritten (Open Issue O3) — acceptable; new receives carry the value. Row: covered by RT-I1 (streamed readout).
...
- **Invariant re-verification:** PS-1/PS-2 (existing glyph map) untouched — only a new arm added; pinned by the unchanged `letter_card_test.dart:469-513` cases. PS-4 (group/legacy shows no glyph) untouched — `transportStatusGlyph` gate unchanged; pinned by existing group/legacy cases (TC-18). PS-5 (metrics + census) pinned by RT-S1's `relayToDirectUpgrades==1` and the unchanged `transportMix()` asserts (RT-S1a).
...
4. **Service seam** — in `p2p_service_impl.dart` `_inferTransportForPeer` upgraded-set branch (`:3025-3027` on this revision — **locate by symbol; this file drifts**) change `return 'direct';` (`:3026`) → `return 'upgraded';`. → RT-I1 GREEN + RT-S1 DATA assert GREEN; RT-S2/PS-3 stays green. ⚠️ Do **NOT** edit `:2997` (the plan's old anchor) — on `new-orbit` that line is `_transportMetrics?.recordHolePunchAttempt();` inside the `holepunch:attempt` arm; editing it corrupts hole-punch and never fixes the badge.
...
- [x] `_transportIcon`/`_sentViaSemantic`/`_receivedViaSemantic` have an `'upgraded'` arm; `_inferTransportForPeer` upgraded-set branch returns `'upgraded'` (located by symbol; `:2997` recordHolePunchAttempt left untouched); `_canonicalTransport` aliases `'upgraded'→'direct'` (step 4a).
- [x] `p2p_service_inbound_transport_test.dart` added to `ONE_TO_ONE_TESTS`.
- [x] `1to1` full pass (+1250); `flutter analyze` 0-new; `git diff --check` clean. `core-host-all`/`feature-host-all` blockers PRE-EXISTING (privacy `sinceProcessStartMs` proven disjoint via stash-revert; transient native-asset codesign flake passes standalone) — affected files green standalone + via 1to1.
