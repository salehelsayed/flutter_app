...
- [ ] **CV-02** ⚙ Consolidated **gomobile rebuild** (07 reserve_dispatch / 08-09 PresenceSet+Get / 11 HandleLANPeerFound / 12 dcutr / 14b isRelay) + native GoBridge.swift/.kt dispatch — **rebuild + native dispatch DEFERRED (device)**. ✅ **Go half green**: `go test ./...` `ok` (all pkgs); `make lint` 0 issues (after `//nolint:funlen` on the renamed store loop — same precedent as `HandleInboxStream`); `bridge` race-clean (4 node races are pre-existing relay-recovery, not mine — see CV-14). → rebuild `.xcframework`/`.aar`
- [ ] **CV-03** ⚙🛰 **Relay redeploy** — unified binary (FDC-10 Redis + FDC-09 presence_set + wake-gate) → ≥2 front-ends, one Redis. **SAME endpoint/peer-ID · `wakeTokenGateEnforced=OFF` · opaque-routing OUT** (NET-REL-07)
- [ ] **CV-04** ⚙ Provision relay #2 + inject WSS+QUIC into client pool (currently 1-peer)
- [ ] **CV-05** ⚙ Ops runbook written (Redis flip / pool / gauge scrape)
- [ ] **CV-06** 🛰 Confirm durability gauge. `curl -s http://<relay>:2112/metrics | grep relay_backend_durable` → `1` on **every** front-end
...
- [ ] **CV-08** ⭐📱 **FDC-11 D1 — two-phone same-WiFi LAN-direct win, BOTH OS** *(manual; /sims N/A — sim shares bonsoir)* → then **CV-09**
- [ ] **CV-09** ▢🚩 Flip `EnableLibp2pLANDial` default→true + lock; **re-run `./scripts/run_test_gates.sh groups`** (group members ride the same host)
...
| `EnableLibp2pLANDial` | false → true | CV-08 D1-GREEN | CV-09 | [ ] |
