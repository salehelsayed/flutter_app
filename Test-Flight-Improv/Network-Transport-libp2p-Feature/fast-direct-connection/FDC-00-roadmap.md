...
- **Spike gates:** FDC-06←S4 · FDC-07←S1 · FDC-08/09←S3 ✅ (A read `presence_get` / C write `presence_set`, B rejected) · FDC-11/12←S2 ✅ · FDC-15←FDC-11/S2 · FDC-14←FDC-02/11. Don't
...
| **FDC-S3** ✅ CLOSED | Presence-signal decision | spike (decision) | **DECIDED 2026-06-27 → READ = Option A relay presence lookup (additive `presence_get` inbox action; coarse connectedness + last-seen "online-ish," reservation-exact truth OUT) · WRITE = Option C `presence_set` client→relay status-push (the only mechanism that expresses fg/bg) · Option B gossipsub beacon REJECTED** (continuous churn/battery, useless on iOS background, net-new mesh — pubsub stays group-only). Per its own terms the spike runs **no app/relay build** — device Methods 1/2/5 are inherited by FDC-08/09 as a device-tuning task. Starting constants (device-tunable): presence TTL 180s / fg heartbeat 60s / client `presence_get` cache 10–15s. Additive back-compat (NET-REL-07) statically confirmed (`inbox.go` single `switch req.Action` + `default: "Unknown action"` → `unknown`). **Unblocks FDC-08 (read) and FDC-09 (write).** |
...
| **FDC-S6** | libp2p-LAN soak + WS-transport retirement decision | spike (decision) | **CLOSED 2026-08-06: retire-chat = N / retire-media = N / keep-bonsoir = always.** The availability-bounded live matrix and underpowered legacy pilots did not meet the retirement bar, so the explicit fail-safe keeps both chat transports, LAN media, ranked-race, and `messageId` dedup. Bonsoir discovery remains permanent. No WS-chat/media removal plan was spawned and there is no automatic Plan 341; a future retirement attempt requires an explicit reopen and a newly resolved eligible matrix. |
...
| **FDC-08** | Relay presence lookup (additive) | P1-1 | tdd-plan (S3 resolved) | Additive `presence_get` inbox action → coarse connectedness + last-seen **"online-ish" hint** (reservation-exact truth OUT) → client-side short-TTL cache → pick direct-race vs inbox-first up front, replacing the blind 5 s `DialPeerViaRelay` probe. Reports **"online-ish," never "foreground."** Must not break older clients (NET-REL-07). **S3 resolved → read = Option A `presence_get`** (finalize: lock resolver = connectedness+last-seen, add the forced-wrong-value "still delivers" RED). |
| **FDC-09** | Foreground self-publish + push-to-wake hardening | §6.3 / §12 | tdd-plan (S3 resolved) | Peer **self-publishes** foreground/background via client→relay `presence_set` status-push (S3 Option C; gossipsub beacon Option B rejected); presence inference from connectedness is wrong on iOS. Harden push-to-wake: per-recipient **access-token** gate (only contacts can wake you), **opaque-token** routing decoupled from the message path, **visible** push (iOS throttles silent pushes ~1–2/hr). **S3 resolved → write = Option C `presence_set`** (gate `p2p_set_presence` on both pause + resume). |
...
| **FDC-16** | Convergence & Close — device campaign + FDC-S6 verdict + FDC-S0 re-measure | Phase 4 / closure | tdd-plan (closure-orchestration) | **The Phase-4 capstone** ([`FDC-16-convergence-close-tdd-plan.md`](FDC-16-convergence-close-tdd-plan.md), live tracker [`FDC-CONVERGENCE-CHECKLIST.md`](FDC-CONVERGENCE-CHECKLIST.md)). It discharges the availability-bounded closure rows across FDC-04/06/07/08/09/10/11/12/13/14/15 + S6 + S0. FDC-S6 is now dispositioned **KEEP** with no conditional WS-removal; remaining staged flag work and the FDC-S0 before→after scorecard stay independently tracked. |
...
| **Phase 2** | Additive relay deploy | **FDC-08** (S3 resolved), **FDC-09** (S3 resolved), **FDC-10** | relay/Go server (additive, NET-REL-07) | host fakes + a live-relay env; **pull FDC-10 earliest** (durability hazard) |
...
FDC-S3 (presence-signal) ✅ CLOSED  ──┬──────────►  FDC-08   (relay presence lookup · read = A `presence_get`)
  A read · C write · B rejected       └──────────►  FDC-09   (foreground self-publish + push · write = C `presence_set`)
...
FDC-S6 (CLOSED: keep chat + media + bonsoir) ─────►  no removal plan / no automatic Plan 341
...
- **E · Relay server (additive):** FDC-10 → FDC-08 → FDC-09 on `inbox.go`/push path; 08/09 — S3 resolved (A read `presence_get` / C write `presence_set`).
...
- **W0 · spikes (concurrent; they gate everything):** FDC-S5 note first (shapes 04/05), then S1/S2/S3/S4
  in parallel. FDC-S6 was the final retirement decision and is now closed KEEP.
...
- **W3 · relay presence (S3 resolved; after FDC-10):** FDC-08 → FDC-09 (serial on `inbox.go`; FDC-08 also after Track A).
- **W4 · Go host device-only (after S2):** FDC-11 → {FDC-12, FDC-15}; coordinate go-mknoon edits with 07/10.
- **W5 · late wiring + decommission decision:** FDC-13 outgoing data (after FDC-12 emits the `'upgraded'` Go label),
  FDC-14 `directReady` source (after FDC-02+11), then FDC-S6; the latter closed KEEP, so no decommission plan follows.
...
> (**CV-01..49**) for the device-proof campaign + availability-bounded FDC-S6 disposition + FDC-S0 re-measure — with
> the live check-off tracker **[FDC-CONVERGENCE-CHECKLIST.md](FDC-CONVERGENCE-CHECKLIST.md)**. Keystone =
> **FDC-11 D1**; FDC-S6 is closed KEEP; the remaining done-signal is the **FDC-S0 re-measure scorecard**.
...
2. Presence signal: relay-lookup (deploy) vs gossipsub beacon (churn) vs status-push? How to
   self-publish foreground? → **FDC-S3 ✅ RESOLVED**: read = relay-lookup (Option A `presence_get`); self-publish foreground = client→relay status-push (Option C `presence_set`); gossipsub beacon (Option B) rejected. → consumed by FDC-08/09.
...
- **FIXED — FDC-08 (presence lookup):** `lookupRelayPresence` is a NEW ungated relay primitive on the
  (top-ungated) send path → now **REQUIRES** `_allowsAccountNetworkSideEffects('p2p_get_presence')`
