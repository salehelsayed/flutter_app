...
Status: **IMPLEMENTED / DEFAULT-OFF / CODE-CLOSED / PLAN-GREEN / NOT STANDALONE RELEASE-ELIGIBLE** (executed 2026-08-08)
Type: Modification
...
Classification: implemented, bounded default-off client adoption of Plans 345 and 346 for durably prepared ordinary direct composer/voice sends and their retries
...
- [x] TC-347-02 has a retained behavioral RED against the Plan 346 production-source baseline, GREEN after implementation, and one representative re-encryption/deletion mutation re-red.
- [x] DB v111/v108 nullable bindings, one-way version floor, host migration, Android SQLCipher reopen/wrong-key/run-twice, and prior v108-v110 preservation are green without backfill.
- [x] Fresh composer/voice publish the complete v111 generation before any LAN/relay request; exact retries only reopen it, work after original plaintext removal, and concurrent preparers adopt one winner without overwrite/deletion.
- [x] A v110 attempt without v111—including partial legacy success—stays wholly legacy and retry never promotes it.
- [x] Strict upload validates/persists the complete Plan 346 proof tuple and never falls back after strict selection.
- [x] Every strict attachment commitment and v108 manifest hash/earliest expiry are exact and transaction-bound; pre-v111 rows stay legacy.
- [x] Relay envelope expiry is never later than earliest blob expiry; exact duplicates preserve expiry; text/reaction omission remains compatible.
- [x] Incoming parent/all attachments/v111 stage before all side effects; relay download commits attachment+source+ACK state atomically; source-less verified LAN adoption issues no delete/ACK and converges by expiry; restart/parent deletion are green.
- [x] Lifecycle cleanup and account transfer preserve exact DB/file authority without exposing proof material on wire/logs.
- [x] All three production activation controls remain false: the client selector and both relay admissions. Only the coordinator and strict download/ACK owner call strict bridge helpers; separate typed-schema owners are exact; external share/private/group/historical/pre-upgrade exclusions pass.
- [x] Focused/preservation gates, curated `1to1`, justified core/feature families, Android bindings, analyzer/format/diff, and one incremental Graphify refresh pass. No per-plan full `host-all` runs.
- [x] All seven current-schema SQLCipher proofs (including the connected release leg) and TC-347-09 pass on explicit rediscovered Android IDs when available. The fixture is locally created/attested/torn down, no external relay is mutated, no user taps occur, and no iOS command runs.
- [x] Plan/index/coverage closure wording leaves external share, private/group/historical/fanout, WP-07 activation, wave-level full `host-all`, operations evidence, and consolidated iOS open.
...
| Several test paths/registrations/commands were non-literal, vacuous, or used unstable concurrency. | Confirmed plan-quality defect. | Corrected paths; added tagged Redis/bridge/app-drain proofs, exact account/composer/voice tests, pinned SQLCipher/release commands, serial feature family, and separate Sims build/run lifecycle. The migration and campaign host tests are each registered once in both manual arrays; the architecture-required action relocation additionally registers `test/debug/android_direct_media_blob_custody_e2e_test.dart` once in the main `1to1` array. |
...
- Implemented the bounded default-off composer/voice and retry adoption, DB v111 plus v108 binding, exact ciphertext custody, strict receive/download/ACK convergence, expiry coupling, lifecycle/bootstrap/account-transfer ownership, relay/native plumbing, and the automated Android-pair campaign. This is code closure only; it is not production activation or standalone release eligibility.
- Focused selector-true coverage passed across 27 files with 972 tests; the exact selector-false legacy preservation sentinel passed, and gate completeness was `1436/1436`. Focused Go relay/native, shell-contract, Android binding, migration, repository, and protocol families are green.
- The affected curated `1to1` lane passed with 2,992 tests and 6 skips. `core-host-all` passed 3,201 tests across 404 paths; serial `feature-host-all` passed 8,973 tests with 7 skips across 840 paths. Per project cadence, no per-plan full `host-all` ran.
- The six focused current-schema Android SQLCipher proofs passed on physical Android `21071FDF600CSC`; the connected release Android SQLCipher leg also passed one test with a successful release build, completing all seven device schema proofs.
- Exact Sims scenario `android.direct_media_blob_custody` passed all 4 assertions on physical Android `21071FDF600CSC` plus emulator `emulator-5554`, emitted its required artifact, and confirmed disposable fixture teardown. It used no external relay, no manual taps, and no iOS command.
- Final `flutter analyze` passed with no issues; the changed-Dart format check covered 88 files with 0 changes; `git diff --check` passed; and the single incremental Graphify refresh completed with 70,093 nodes, 103,263 edges, and the refreshed deterministic TDD overlay.
- `MKNOON_DIRECT_MEDIA_BLOB_CUSTODY_CLIENT_ENABLED`, `DIRECT_INBOX_ACK_CUSTODY_ADMISSION_ENABLED`, and `DIRECT_MEDIA_BLOB_CUSTODY_ADMISSION_ENABLED` all remain production-default false. External share, private/group/historical media, fanout, the remaining GAP-N01 lanes, WP-07 activation, wave-level full `host-all`, operations evidence, and consolidated iOS proof remain explicitly open.
