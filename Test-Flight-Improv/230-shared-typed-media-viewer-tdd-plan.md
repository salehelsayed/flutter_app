...
- Per-plan closure directly runs every focused `test/shared/widgets/media/**` contract because shared-widget tests are outside the `feature-host-all` and `core-host-all` globs, then runs the curated `1to1` and `groups` lane gates as shared-caller preservation.
- Do not run `feature-host-all`, `core-host-all`, or full `host-all` as Plan 230 acceptance. Full `host-all` runs once after the complete 227-230 foundation wave, and again at final received-media rollout closure.
...
# Preservation and affected curated lane gates
...
- [x] Focused shared viewer files and exact legacy/group preservation sentinels pass. Curated `1to1` and `groups` are the per-plan lane sweeps; any historical `host-all --list` discovery is supporting evidence only, not Plan-230 acceptance.
...
- Manual registration: none for the shared tests; per-plan closure invokes each file literally because they are outside feature/core globs. The 227-230 wave-level full `host-all` supplies aggregate discovery after the foundation wave.
