...
Status: **CLOSED (2026-08-01, session 2)** — all 13 rows closed; relay **v1.7.5 deployed stable** (20:08:29Z, embedded go1.25.0, backup `relay-server.pre-320-*`, active t+92s NRestarts=0); **TC-320-13 deterministic eviction PASS with `reason=typed_unregistered`** — live FCM's 404 DOES carry the structured `UNREGISTERED` errorCode, so the typed arm fires in production and the literal arm is genuine belt-and-braces (the review's half-confirmation is now fully resolved in the typed arm's favor). Six mutation-reds run and restored. (v2, /tdd-review applied 2026-08-01 — 3-worker audit `wf_d14e6eae-e53`, **2 blockers** both closed below; the #1 bet was half-confirmed: the relay does NOT wrap the SDK error (typed predicates are reachable), but `NotRegistered` is FCM's *message string*, not the errorCode the predicates read)
Type: Bug
...
- [x] All 13 rows closed; three defects fixed; re-fan-out absent and test-locked (unfiltered relay run ok).
- [x] Causal REDs + revert mutations recorded (session 1: TC-01b RED `send calls = 3, want 1`; session 2: six mutation-reds run and restored — see Execution Progress).
- [x] Lane green on a single reaped run (3342/3342 + both Go legs, LANE_EXIT=0; first run's single red was the documented `group_conversation_wired_test` load-flake shape, proven 5/5 + 231/231 in isolation, rerun quiescent); unfiltered relay ok; completeness 1369/1369 PASS; analyzer clean; diff clean.
- [x] v1.7.5 deployed with provenance + stability evidence (embedded go1.25.0 verified pre-upload, sha `565417dd63cb…`, `Starting relay-server v1.7.5` 20:08:29Z, active t+12/52/92s NRestarts=0, 0 panics); TC-320-13 deterministic proof: scratch peer + the real retired token → ONE store → `[PUSH] Removed invalid token … (reason=typed_unregistered): NotRegistered` same-second, `retry_lines=0`, token key deleted (`verify_permanent_token_eviction_320_result.txt`).
...
|---|---|---|---|---|---|---|
| 2026-08-01 s1 | E0–E4 implemented | inbox.go (classifier + wake lines + counted declines + do-not-wrap comment), p2p_service_impl.dart (`liveFcmTokenReader` seam), production_application_bootstrap.dart (wires `FirebaseMessaging.instance.getToken()`), push_permanent_error_closure_test.go (4 rows: 01b/02/03/04), runner + probe scripts | causal RED captured (TC-01b: `NotRegistered` → send calls = 3, want 1); focused gate + unfiltered relay ok; analyzer clean | commit `e37f2f05b` | 4 Go contract rows (01/05/06/07) + both Dart rows still unwritten | session 2 |
| 2026-08-01 s2 | contract completion | NEW `test/features/p2p/push_token_reregistration_test.dart` (4 tests: TC-08, TC-09 throw-shape + empty-read shape, no-reader legacy wiring); push_permanent_error_closure_test.go +4 (TC-01 real-SDK-client `httptest` typed arm w/ opaque message text, TC-05 transition= relationship, TC-06 three declines + accounting identity, TC-07 label compat incl. no-`dispatched`-series) | all green first run (Dart 4/4; focused Go 14/14) | **six mutation-reds run and restored byte-identical**: Dart cached-only revert → 08 red; fallback removed → 09 red; `%w` wrap in `ps.send` → 01 red while 01b stayed GREEN (the wrap preserves message text, so only the typed arm dies — exactly the do-not-wrap failure mode); duplicate-line `transition=` dropped → 05 red; self-skip increment removed → 06 red via the identity; counter label `attempted`→`dispatched` → 07 + 06 + the 315 attempted-delta test red (triple-locked) | none | gates |
| 2026-08-01 s2 | gates | — | focused Go 14/14; unfiltered relay ok 15s (TC-10); `flutter analyze` 0 issues; completeness 1369/1369 PASS; graph-affected on p2p_service_impl.dart → 41 host files / 491 tests all pass; census: `isInvalidTokenError` 0 matches, `messaging.IsUnregistered` 2, `IsSenderIDMismatch` 1, `containsFold` 3, **`messaging.IsInvalidArgument(` call sites 0** (the plan's bare grep counts the exclusion COMMENT at inbox.go:1145 — recorded, comment kept deliberately); registration: **8** `TestRelayNotificationClosure_` names in the new file (plan's "expect 7" miscounted its own 8-row table), lane filter at run_test_gates.sh:1041 unchanged | groups lane run 1: 3341/3342, LANE_EXIT=1 — single red `group_conversation_wired_test.dart` "P269 … foreground lease" assertion-free 10s `runAsync` timeout = the documented load-flake shape for exactly this file; proven 5/5 anchored + 231/231 full-file; **rerun quiescent (single reaped run, zero host work during): 3342/3342 + go-mknoon leg + relay tail + toolchain contract, LANE_EXIT=0** | none | deploy |
| 2026-08-01 s2 | deploy + operational rows | docker-ws/deploy_relay_v175.sh, docker-ws/verify_permanent_token_eviction_320.sh (+ results) | **v1.7.5 deployed 20:08:29Z** (embedded go1.25.0 verified pre-upload, sha `565417dd63cb…`, backup `relay-server.pre-320-<stamp>`, active t+12/52/92s NRestarts=0, 0 panics) — TC-12 ✓. Pre-deploy baselines on the OLD binary: 4 `NotRegistered; retrying in` lines in 24h, **0 eviction lines** (P1 premise re-confirmed at deploy time). TC-13 (deterministic): scratch peer `12D3KooWNqaSe…` + the REAL retired token of `12D3KooWQAhMvhX1wBp7…` copied in redis → testpeer stored ONE chat envelope via prod QUIC → same-second `[PUSH] Removed invalid token for 12D3KooWNqaSeJDRrgSD after chat push error (reason=typed_unregistered): NotRegistered`, `retry_lines=0`, token key deleted; scratch inbox+push keys cleaned — **PASS** | **typed arm fires in production** (live FCM 404 carries the structured `UNREGISTERED` errorCode); fixture landmine recorded: host-run scripts execute under macOS bash 3.2 — `case` patterns inside `$( )` need leading-paren form | none | record + commit |
