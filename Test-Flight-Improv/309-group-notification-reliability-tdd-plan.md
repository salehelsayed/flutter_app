...
| **D1b** | `apns-collapse-id` for group reactions (iOS). **RESOLVED 2026-08-01 (user: "do what other major apps do as best practice") → PER-EVENT**, shipped in relay v1.7.4: discrete reaction events deliver individually, visually grouped by `ThreadID`; the per-event id dedupes provider retries of the same event only — mirroring the 1:1 lane and closing the iOS in-transit half of C2. TC-08 inverted as pre-specified; `boundedGroupReactionIdentity` deleted (zero consumers). | ~~Keep per-group~~ | Per-event stacks N reaction banners per group instead of merging one, reversing Plan 257's "one stable card" design. **But be explicit: keeping per-group means the iOS half of C2 is NOT fixed** — APNs discards earlier *undelivered* reactions for an offline recipient, which is the exact defect described. This plan closes C2 on Android only unless D1b flips. |
