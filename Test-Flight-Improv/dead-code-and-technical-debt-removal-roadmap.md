...
planning and execution are authorized; DTR-12 is Plan-green and DTR-13 is
implementation-complete under `DTR13-AUTH-01`. All four separately authorized
live blockers are now coherent-source green under `W4A-BLOCKERS-AUTH-01`:
wake directionality, both intro roles after the narrow Plan-252 correction,
all four group/announcement scenarios, and the seven-assertion iOS
APNs/NSE/offline-tap journey. The two earlier external-input classifications
were refuted by archived provenance, current owner-only local inputs, and
read-only live EC2 evidence. Final integrated host/analysis receipts, one
incremental Graphify refresh, and the full Wave 4A aggregate remain pending.
...
| DTR-12 | P4/P5 | Add enforceable architecture/layer boundary checks | DTR-01 | Exact boundary-tool tests; current exceptions pinned | **Plan-green / implementation-complete — [Plan 288](288-architecture-layer-boundary-guard-tdd-plan.md) pins the corrected 182 forbidden dependency identities plus 24 concrete feature-domain repository placements; the AST/Git checker, strict reviewed manifest, no-argument `architecture-boundaries` lane, and required Sims-major capability are green. DTR-13/DTR-18 own reduction; Wave 4A acceptance waits for DTR-13 and the aggregate `host-all`.** |
| DTR-13 | P4 | Move debug/E2E wiring behind a separate composition root while preserving proof entrypoints | DTR-01, DTR-12; `DTR13-AUTH-01`; narrow closure follow-ups under `W4A-BLOCKERS-AUTH-01` | `sims-contracts`; affected simulations; startup/lifecycle preservation; affected family gate | **Implementation complete / coherent-source live-green; final integrated host/graph closure pending — [Plan 289](289-debug-e2e-composition-root-tdd-plan.md) moved only the authorized construction into `lib/debug/debug_e2e_composition_root.dart`. Default composition constructs none; every profile/root/define/command/workflow is preserved and DTR-12 remains 182/24/92. The corrected iOS `result.apps` verifier, Plans [290](290-android-wake-token-cold-start-reliability-tdd-plan.md)–[292](292-group-reaction-notification-fixture-driver-stability-tdd-plan.md), and the narrow Plan-252 exact-anchor correction are implemented. Final current-source reports passed wake 2/2, intro 2/2, group/announcement 4/4, and iOS 7/7 assertions on the availability-bounded live matrix. Plan-green now waits only for the final integrated host/analysis receipts and Graphify refresh.** |
...
Planning is unblocked by Wave 3 acceptance. DTR-12 is Plan-green.
`DTR13-AUTH-01` resolves the separate QA + release decision and authorizes
Plan 289 planning and execution on its exact preservation boundary. Plan 289
is implementation-complete. Archived provenance, current owner-only inputs,
and live EC2 checks refuted the earlier missing-input classification for iOS
payload fast-path and reaction notification. The owner-authorized narrow
repairs retained all DTR-13 preservation boundaries: the verifier now inspects
`result.apps`; Plan 290 closes cold-start wake readiness; Plan 291 closes only
campaign budget/process ownership; Plan 292 closes fixture/driver stability;
and Plan 252 owns the exact intro status-convergence correction. Final
coherent-source live reports now pass wake 2/2, intro 2/2, group/announcement
4/4, and iOS 7/7 assertions. The iOS journey proves ordered NSE staging,
decryption, content handoff, airplane-before-tap visibility, restoration,
cleanup, and zero installed apps. Plan-green and the Wave 4A aggregate now
wait only for the final integrated host/analysis, Graphify, and full
`host-all` receipts.
...
| Wave 4A | DTR-12, DTR-13 | In progress; DTR-12 Plan-green; DTR-13 implementation-complete and coherent-source live-green | **Full** cadence-protected `./scripts/run_host_test_gates.sh host-all --continue-on-failure --batch-flutter --concurrency 4 --reporter failures-only` pending after the final Plan-289 host/graph receipts; no Dart-only run is acceptance evidence | Base `765be74523b25ce592ff200ff435653b35c66b4d` plus the integrated Plans 288–292 and narrow Plan-252 working-tree delta; source digest `e2ab677e61688505cf0af0a1b3b3d5e62c4a7fc9fb94cc549b0c6bc0c0237f8e` for all four final live reports | Final live PASS: wake 2/2, report SHA `7da2c832c98e9954580c46c1244fe5123869ae31083bea6cbcba1257d63d9557`; intro 2/2, SHA `85ac10b661e90b2d3bccc5a09117d5962f83fb074eeab24ed9ba94e6d9e45a47`; group/announcement 4/4, SHA `0aa1c02f5f03986a443297424aeeb12422db65d6796f45fd7fb6da7b08c0b116`; iOS APNs/NSE/offline-tap 7/7, SHA `29f4d86574a56de803058363a91d52613127c93d3d8c6a5929f915a2a62d5b46`, with `result.apps=[]`, zero child builds/manual actions, and zero NSE failure/rejection markers. Integrated gates green so far: focused causal/preservation suites; `1to1` 2,441 plus relay Go; `intro` 300; `groups` 3,239 plus 13 focused Go/toolchain; `sims-contracts` 38/38; `runtime-roots` 20/20 over 1,026 files; completeness 1,346/1,346; architecture 6/6 with 182/24 and zero issues; strict analysis zero issues in 405.8 s with the three reviewed suppressions unchanged. `core-host-all`, final Graphify, tested-tree identity, aggregate inventory/counts/archive SHA, and verdict are pending. | 2026-07-28 / project owner + Codex |
...
| Supported build profiles and debug/E2E entrypoints | DTR-13 planning and execution | QA + release | Accepted and executed for authorized structural scope; coherent-source live acceptance green; final integrated host/graph closure pending | `DTR13-AUTH-01`: the current authenticated project owner, acting as QA + Release owner, requires preservation of every currently declared Sims build profile and define; the three registered manual smoke roots (`lib/smoke_test_main.dart`, `lib/smoke_test_messages.dart`, and `lib/smoke_test_restore.dart`); and every current manual, simulation, and headless debug/E2E workflow. Retain `lib/core/debug/smoke_test_runner.dart` without deleting or newly activating it. Move only the live debug/E2E controller, polling, reset, fixture, and simulator auto-setup construction currently owned by production `lib/main.dart` into a separate debug/E2E composition root. Do not delete or rename entrypoints, change commands/defines, reduce proof capability, or change runtime behavior. Production/default builds must construct none of the debug/E2E harness wiring. DTR-14 bootstrap extraction and DTR-18 general layering relocation remain excluded. No DTR-12 boundary exception may be added or rebaselined; remove only an exact exception genuinely eliminated by this move. This receipt authorizes DTR-13 planning and implementation without another generic QA or Release approval unless scope materially changes. [Plan 289](289-debug-e2e-composition-root-tdd-plan.md) executed that boundary exactly. No DTR-12 exception/profile/root changed. Narrow blocker repairs that materially extend beyond that structural boundary are separately authorized by `W4A-BLOCKERS-AUTH-01` below. |
| Finish the four exact Wave 4A live blockers without weakening DTR-13 preservation | Plan 289 Plan-green and Wave 4A aggregate | Current authenticated project owner acting as QA + Release owner | Approved 2026-07-27; four coherent-source live rows closed 2026-07-28 | `W4A-BLOCKERS-AUTH-01`: the owner explicitly selected the fail-closed iOS verifier repair, ordered TDD plans and reviews for Plans 290–292, ordered their implementation, ordered live Sims proof on the booted available devices, and then ordered “finish Wave 4A blockers.” This authorizes only the narrow repairs required by those reproduced rows: parse and inspect iOS `devicectl` `result.apps`; repair shared token-bearing inbox-store node readiness/deadline/disposal under Plan 290; repair Plan 291's absolute campaign/phase budget, acceptance discrimination, bounded child termination, and idempotent cleanup; repair Plan 292's exact FAB semantics, bounded recovery/readiness/invite/cleanup/log-read fixture behavior; and apply the newly reproduced exact-anchor convergence correction as a **narrow corrective amendment to Plan 252**. Plan 291 remains harness scope and must not absorb that product behavior. Every Sims profile/define, entrypoint, manual smoke root, command, registered simulation/headless workflow, DTR-12 182/24/92 boundary, and the unactivated `lib/core/debug/smoke_test_runner.dart` remains preserved. This receipt does not authorize DTR-14 bootstrap extraction, DTR-18 relocation, profile/workflow retirement, schema/protocol changes, or proof weakening. Final reports pass wake 2/2, intro 2/2, group/announcement 4/4, and iOS 7/7 against one source digest; the historical intermediate and causal-failure receipts remain preserved rather than rebaselined. |
