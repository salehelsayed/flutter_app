...
Waves 2 and 3 on 2026-07-27, and Waves 4A and 4B on 2026-07-28. DTR-09
...
replay are green. DTR-16, DTR-17, and DTR-18's first increment are
`Plan-green` under `DTR16-AUTH-01`, `DTR17-AUTH-01`, and `DTR18-AUTH-01`.
The bounded DTR-18 placement closure in Plans 298–300 is implementing the
`DTR18-AUTH-02` disposition: remove all 22 remaining placement exceptions and
retain the exact 165 dependency identities with explicit owner, reason, and
revisit condition. Its focused/family/policy/analyzer verification and the
separate Wave 4C aggregate `host-all` plus justified `performance-host` have
not run to completion, so DTR-18 is not yet terminal and Wave 4C is not yet
accepted. Final release-closure `host-all` and `performance-host` remain later
obligations.
...
| DTR-16 | P5 | Decompose `GroupMessageListener` behind its existing facade | DTR-15 | `groups`; `feature-host-all`; crypto/background preservation | **Plan-green / implementation-complete under `DTR16-AUTH-01` — [Plan 296](296-group-message-listener-decomposition-tdd-plan.md) preserves the public/subclassable ingress/lifecycle facade behind four private state-owning collaborators. Causal/mutation/focused preservation, 3,265-test `groups` with all Go/relay tails, 811-path `feature-host-all` with 8,441 pass / 1 declared skip, policy, whole-tree analysis, hygiene, and Graphify evidence are green. Wave 4C and final `host-all` remain separate; no wave acceptance is claimed.** |
| DTR-17 | P5 | Decompose `P2PServiceImpl` behind stable interfaces | DTR-14 | `1to1`; `transport`; `core-host-all`; justified `performance-host` | **Plan-green / implementation-complete under `DTR17-AUTH-01` — [Plan 295](295-p2p-service-component-decomposition-tdd-plan.md) preserves the public facade, lifecycle/state/recovery and sole Bridge callback ownership while composing exact inbox-custody and peer/LAN-policy coordinators through typed private ports. Causal/mutation/focused, 21-path performance, pinned physical-Android transport, policy, hygiene, and current Graphify evidence are green. Final aggregate receipts are `1to1` 2,464 plus relay Go tails and `core-host-all` 2,840 across 365 paths plus its Android manifest tail; strict analysis reports the exact three reviewed generated-l10n suppressions and zero issues. The load-sensitive retrier blocker was repaired test-only with explicit completion synchronization; the concurrent DTR-16 suppression was replaced by a live emission-only lifecycle fence and re-proved by `groups` 3,265 plus Go/relay tails and 811-path `feature-host-all` at 8,441 pass / 1 declared skip. DTR-17 production scope is unchanged. Wave 4C and final `host-all` remain separate; no wave acceptance is claimed.** |
| DTR-18 | P5 | Move orchestration out of `core` and relocate misplaced repository implementations incrementally | DTR-12, DTR-14, DTR-16, DTR-17 | Boundary tests; affected `core-host-all` and `feature-host-all` | **First increment Plan-green; placement closure implementation/verification pending under `DTR18-AUTH-02` — [Plan 297](297-application-resume-posts-adapter-layer-relocation-tdd-plan.md) remains the closed first increment: it moved byte-identical application-resume orchestration to the app layer plus the two Posts Phase-3 adapters to data, reducing the trustworthy boundary baseline by exactly 17 dependency and two placement exceptions to 165/22. The next bounded slices are [Plan 298](298-groups-repository-placement-relocation-tdd-plan.md) for 13 Groups implementations, [Plan 299](299-conversation-repository-placement-relocation-tdd-plan.md) for three Conversation implementations, and [Plan 300](300-remaining-feature-repository-placement-relocation-tdd-plan.md) for the six remaining feature implementations. `DTR18-AUTH-02` authorizes zero placement exceptions and terminal retention of the exact 165 dependency identities with manifest-level owner, reason, and revisit condition; it does not authorize pause/P2P relocation, a dependency rebaseline, or a new exception. Plans 298–300, focused/family/policy/analyzer proof, and the Wave 4C aggregate `host-all` plus justified `performance-host` are still pending. DTR-18 is not terminal and Wave 4C is not accepted until those receipts pass.** |
...
All five exit conditions are satisfied. This clears the DTR-16 and DTR-17
predecessor dependencies for planning only; it does not authorize Wave 4C
execution. Final release-closure `host-all` and `performance-host` remain
...
Plan 297 is the Plan-green first increment, not terminal DTR-18. Plans 298–300
are the bounded placement closure: 13 Groups, three Conversation, and six
remaining feature repository implementations. `DTR18-AUTH-02` dispositions
the post-Plan-297 baseline as zero placement exceptions and the exact 165
dependency identities retained with explicit owner, reason, and revisit
condition. Implementation and focused/family/policy/analyzer verification are
in progress; the Wave 4C aggregate `host-all` and justified
`performance-host` remain pending. Neither DTR-18 terminal closure nor Wave 4C
...
Wave 5 planning is blocked on the open Product + Release decision that defines
the supported Android, iOS, macOS, Windows, Linux, and web release/CI targets.
That decision was explicitly opened on 2026-07-28 and remains unresolved; the
...
| Wave 4C | DTR-16, DTR-17, DTR-18 (Plans 297–300) | DTR-16/17 and DTR-18 Plan 297 green; Plans 298–300 implementation/verification and aggregate pending | After placement closure verification: `host-all`; justified `performance-host` | Integrated dirty working tree; completed per-plan evidence in Plans 295–297; Plans 298–300 pending | `DTR18-AUTH-02` targets zero placement exceptions and retains the exact 165 dependency identities with owner/reason/condition. Focused/family/policy/analyzer results, DTR-18 terminal verdict, full aggregate, performance replay, and wave acceptance are pending. | 2026-07-28 / closure and aggregate not yet complete |
| Wave 5 child waves | DEP-01 children | Blocked on open supported-platform decision; child-wave planning not started | Per-child `host-all` plus platform gates after Product + Release defines the supported target matrix | — | The availability-bounded device policy does not resolve product support; see the open decision ledger row. | — |
