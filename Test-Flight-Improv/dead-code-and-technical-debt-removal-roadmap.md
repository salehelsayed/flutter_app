...
Waves 2 and 3 on 2026-07-27, and Waves 4A, 4B, and 4C on 2026-07-28. DTR-09
remains terminal `Retained`, DTR-10 is terminally complete, and DTR-11 and
DTR-12 are `Wave-accepted`. DTR-13 is `Plan-green` and `Wave-accepted`; the
authorized implementation and acceptance scope of `DTR13-AUTH-01` is closed.
The final coherent live receipts, integrated core/analyzer/Graphify receipts,
and 1,265-item Wave 4A aggregate `host-all` are green. DTR-14 and DTR-15 are
`Plan-green` and `Wave-accepted` under `DTR14-AUTH-01` and `DTR15-AUTH-01`.
The Wave 4B aggregate `host-all` and required same-tree `performance-host`
replay are green. DTR-16 and DTR-17 are `Plan-green` and `Wave-accepted`.
DTR-18 is terminally complete and `Wave-accepted` under `DTR18-AUTH-01` and
closed `DTR18-AUTH-02`: Plan 297 remains its Plan-green first increment, while
Plans 298–300 closed all 22 remaining placements and retained the exact 165
dependency identities with explicit owner, reason, and revisit condition. The
trustworthy architecture result is 165 dependency exceptions, zero placement
exceptions, and zero issues; across DTR-18 the accepted delta from the DTR-12
baseline is dependency -17 and placement -24. The Wave 4C
`performance-host`-then-`host-all` aggregate is green. Final release-closure
`host-all` and `performance-host` remain later obligations.
[Stable Wave 4A evidence](evidence/dtr-wave4a/README.md).
[Stable Wave 4B evidence](evidence/dtr-wave4b/README.md).
[Stable Wave 4C evidence](evidence/dtr-wave4c/README.md).
...
| DTR-14 | P5 | Extract `main.dart` bootstrap phases behind stable application interfaces | DTR-13 | Startup, lifecycle, push, Move Account, and affected core/feature gates | **Plan-green / Wave-accepted — [Plan 293](293-main-bootstrap-application-interfaces-tdd-plan.md) reduced `main.dart` from 7,262 to 14 lines, introduced stable bootstrap/prepared/host interfaces, separated the concrete production graph from the application root, migrated exact-owner locks and runtime-root evidence, and preserved the public facade, startup, lifecycle, push, Move Account, DTR-13, profile, and headless-handler boundaries. Focused RED/GREEN plus five mutation re-reds, 825 lifecycle/push tests, curated lanes, `move-feature`, 365-item core and 805-path feature families, policy gates, strict analysis, and Graphify are green under `DTR14-AUTH-01`. The Wave 4B aggregate `host-all` and required same-tree `performance-host` replay passed. [Stable evidence](evidence/dtr-wave4b/README.md).** |
| DTR-15 | P5 | Extract shared direct/group conversation controllers without a shared base `State` | DTR-12 | `1to1`; `groups`; `feature-host-all`; required `performance-host` | **Plan-green / Wave-accepted — [Plan 294](294-shared-conversation-controller-extraction-tdd-plan.md) extracted four plain compositional controllers under `DTR15-AUTH-01`, kept both private States independent, froze public facades/widget APIs, and preserved lane-specific send/security/cancellation/voice policy. Causal and mutation proof, 1:1 (2,458), groups (3,259), 810-path feature (8,438 pass / 1 skip), 21-path performance (106), policy, strict analysis, diff hygiene, and Graphify are green. The Wave 4B aggregate `host-all` and required same-tree `performance-host` replay passed. [Stable evidence](evidence/dtr-wave4b/README.md).** |
| DTR-16 | P5 | Decompose `GroupMessageListener` behind its existing facade | DTR-15 | `groups`; `feature-host-all`; crypto/background preservation | **Plan-green / Wave-accepted under `DTR16-AUTH-01` — [Plan 296](296-group-message-listener-decomposition-tdd-plan.md) preserves the public/subclassable ingress/lifecycle facade behind four private state-owning collaborators. Causal/mutation/focused preservation, 3,265-test `groups` with all Go/relay tails, 811-path `feature-host-all` with 8,441 pass / 1 declared skip, policy, whole-tree analysis, hygiene, and Graphify evidence are green. The later Wave 4C `performance-host` and full `host-all` aggregate passed; final release closure remains separate. [Stable evidence](evidence/dtr-wave4c/README.md).** |
| DTR-17 | P5 | Decompose `P2PServiceImpl` behind stable interfaces | DTR-14 | `1to1`; `transport`; `core-host-all`; justified `performance-host` | **Plan-green / Wave-accepted under `DTR17-AUTH-01` — [Plan 295](295-p2p-service-component-decomposition-tdd-plan.md) preserves the public facade, lifecycle/state/recovery and sole Bridge callback ownership while composing exact inbox-custody and peer/LAN-policy coordinators through typed private ports. Causal/mutation/focused, 21-path performance, pinned physical-Android transport, policy, hygiene, and current Graphify evidence are green. Final per-plan receipts are `1to1` 2,464 plus relay Go tails and `core-host-all` 2,840 across 365 paths plus its Android manifest tail; strict analysis reports the exact three reviewed generated-l10n suppressions and zero issues. The load-sensitive retrier blocker was repaired test-only with explicit completion synchronization; the concurrent DTR-16 suppression was replaced by a live emission-only lifecycle fence and re-proved by `groups` 3,265 plus Go/relay tails and 811-path `feature-host-all` at 8,441 pass / 1 declared skip. DTR-17 production scope is unchanged. The later Wave 4C `performance-host` and full `host-all` aggregate passed; final release closure remains separate. [Stable evidence](evidence/dtr-wave4c/README.md).** |
| DTR-18 | P5 | Move orchestration out of `core` and relocate misplaced repository implementations incrementally | DTR-12, DTR-14, DTR-16, DTR-17 | Boundary tests; affected `core-host-all` and `feature-host-all` | **Terminally complete / Wave-accepted under `DTR18-AUTH-01` and closed `DTR18-AUTH-02` — [Plan 297](297-application-resume-posts-adapter-layer-relocation-tdd-plan.md) remains the closed Plan-green first increment: it moved byte-identical application-resume orchestration to the app layer plus the two Posts Phase-3 adapters to data, reducing the trustworthy boundary baseline by exactly 17 dependency and two placement exceptions from 182/24 to 165/22. Plan-green [Plan 298](298-groups-repository-placement-relocation-tdd-plan.md), [Plan 299](299-conversation-repository-placement-relocation-tdd-plan.md), and [Plan 300](300-remaining-feature-repository-placement-relocation-tdd-plan.md) then moved the exact 13 Groups, three Conversation, and six remaining feature implementations, closing the other 22 placements without a shim or implementation-body change. The terminal architecture result is 165 dependency exceptions, zero placement exceptions, and zero issues: overall DTR-18 delta dependency -17 and placement -24. The exact dependency identities remain manifest-owned with reason and revisit condition; the full disposition-tuple SHA-256 is `5f24faf4c5f693d0f19eb18503e5d37c4db4580c6ccbc91806abc75a78dcf132`. Focused/family/policy/analyzer/Graphify proof and the Wave 4C `performance-host` then full `host-all` aggregate are green. [Stable evidence](evidence/dtr-wave4c/README.md).** |
| DEP-01 | P4/P5 | Dependency modernization split by pure Dart, persistence/crypto, Android, and iOS boundaries | A green preceding wave; no concurrent architecture rewrite | Per-package causal proof, affected family/native gates, then child-wave closure | Wave 4C predecessor green; child-plan decomposition remains blocked on the open Product + Release supported-target decision, and no execution is authorized |
...
now `Plan-green` and `Wave-accepted`. The complete `host-all` and required
same-tree `performance-host` replay passed against frozen tested tree
...
- The required same-tree `performance-host` replay passes.

All five exit conditions are satisfied. At Wave 4B acceptance this cleared the
DTR-16 and DTR-17 predecessor dependencies for planning only; Wave 4C was
later separately authorized and accepted. Final release-closure `host-all`
and `performance-host` remain separate later obligations.

### Wave 4C — Listener, transport, and layering decomposition

Plans: DTR-16 and DTR-17, followed by DTR-18 Plans 297–300.
...
Plan 297 remains the Plan-green first increment, not DTR-18 as a whole.
Plan-green Plans 298–300 closed the bounded placement family: 13 Groups, three
Conversation, and six remaining feature repository implementations.
`DTR18-AUTH-02` terminally retains the exact 165 dependency identities with
explicit owner, reason, and revisit condition and closes placement at zero.
The trustworthy architecture result is 165 dependency exceptions, zero
placement exceptions, and zero issues; the overall DTR-18 delta from the
DTR-12 baseline is dependency -17 and placement -24. Focused, family, policy,
analyzer, hygiene, and Graphify proof passed. The Wave 4C
`performance-host`-then-`host-all` aggregate also passed, so DTR-18 is
terminally complete and Wave 4C is accepted.
...
All six exit conditions are satisfied. This accepts Wave 4C and closes DTR-18
terminally. It clears DEP-01's preceding-wave gate but does not resolve the
separate Product + Release supported-platform decision blocking Wave 5
planning. [Stable Wave 4C evidence](evidence/dtr-wave4c/README.md).
...
Wave 4C's temporal predecessor gate is green, but Wave 5 planning remains
blocked on the open Product + Release decision that defines the supported
Android, iOS, macOS, Windows, Linux, and web release/CI targets. That decision
was explicitly opened on 2026-07-28 and remains unresolved; the
availability-bounded device-test policy constrains executable proof but does
not decide which platforms the product supports.
...
| Wave 4B | DTR-14, DTR-15 | **Wave-accepted**; both plans Plan-green/Wave-accepted | `./scripts/run_host_test_gates.sh host-all --continue-on-failure --batch-flutter --concurrency 1 --reporter failures-only`; 1,265 exact Flutter paths + 8 serial Go tails = 1,273 items; then `./scripts/run_host_test_gates.sh performance-host`; 21 exact Flutter paths | Base and unchanged `HEAD` `765be74523b25ce592ff200ff435653b35c66b4d`; alternate-index tested tree `51455d3c1905a6bd70be63c59fdcc3860623778f`; synthetic commit `4f36940866dd12e65f15c302c0c37e2d2a65899f`; annotated tag `dtr-wave4b-tested-tree-20260728` | `host-all` exit 0 in 1,898 s: Flutter 12,825 passed, 1 skipped, 0 failed; all eight Go tails passed; final scope marker present. Same-tree `performance-host` exit 0 in 97 s: 106 passed, 0 skipped across 21 paths; final scope marker present. Original/archive SHA-256: host `24b623e2b9142e8c50aa8add4ae031ddb385fad1b958a4bd6f80defb912f1e97` / `e6c66cc032380ef755c32565f7e491a851b80cc5af918f182333dcf057c35bed`; performance `b0fb2b84ad77a98032cc3a12365fa580810c080ceb3db9bcda133785861dbb63` / `579c25b54a4fec6a28ba361ff82bac43805845eb7cef499ec0526f43d16a3256`. [Stable evidence](evidence/dtr-wave4b/README.md). | 2026-07-28 / project owner + Codex |
| Wave 4C | DTR-16, DTR-17, DTR-18 (Plans 295–300) | **Wave-accepted**; Plans 295–300 Plan-green/Wave-accepted; DTR-18 terminally complete | First `./scripts/run_host_test_gates.sh performance-host --batch-flutter --concurrency 4 --continue-on-failure --reporter failures-only` (21 exact Flutter paths), then `./scripts/run_host_test_gates.sh host-all --batch-flutter --concurrency 4 --continue-on-failure --reporter failures-only` (1,269 exact Flutter paths + 8 serial Go tails = 1,277 items) | Base and unchanged `HEAD` `765be74523b25ce592ff200ff435653b35c66b4d`; alternate-index tested tree `8b4052ea418a537d1a9af145da44dd20d367fb7b`; no synthetic commit or tag created | `performance-host` passed 106 tests across 21 paths in 17 s. `host-all` exited 0 in 785 s: Flutter 12,845 passed, 1 skipped, 0 failed; all eight Go tails passed and the final scope marker is present. Architecture passed at 165 dependency / 0 placement / 0 issues; full disposition-tuple SHA-256 `5f24faf4c5f693d0f19eb18503e5d37c4db4580c6ccbc91806abc75a78dcf132`. Archive SHA-256: host `09ec61bc113a850fe053eb55c85032bbba7f08f06069a5ad7911fce84412d55b`; performance `cf0c00445e8a22ae2192bdaf74a706543b0ac11bce8aebaa42be4179881cefe2`. [Stable evidence](evidence/dtr-wave4c/README.md). | 2026-07-28 / project owner + Codex |
| Wave 5 child waves | DEP-01 children | Preceding-wave gate green; blocked on open supported-platform decision; child-wave planning not started | Per-child `host-all` plus platform gates after Product + Release defines the supported target matrix | — | The availability-bounded device policy does not resolve product support; see the open decision ledger row. | — |
...
its authorized scope. The later Wave 4B receipt below records DTR-14 and
DTR-15 acceptance. Final release-closure `host-all` and `performance-host`
remain later, separate gates.

Wave 4B used two sequential acceptance runs after DTR-14 and DTR-15 were
Plan-green. The 1,273-item `host-all` gate exited 0 with 12,825 Flutter tests
passed, one skipped, all eight Go tails passed, and the final scope marker
present. Without intervening edits, the required 21-path `performance-host`
replay exited 0 with 106 tests passed and its final scope marker present. An
alternate index preserved the shared index while proving the same frozen tree
before and after both runs; annotated tag `dtr-wave4b-tested-tree-20260728`
retains that tree. Both deterministic log archives, original/archive hashes,
tested-state identities, and workspace disclosure are recorded in the
[Wave 4B evidence archive](evidence/dtr-wave4b/README.md). DTR-14 and DTR-15
are `Plan-green` and `Wave-accepted`. Final release-closure `host-all` and
`performance-host` remain later, separate gates.

Wave 4C used two sequential acceptance runs after Plans 295–300 were
Plan-green. The justified 21-path `performance-host` gate ran first and passed
106 tests in 17 seconds. The complete 1,277-item `host-all` gate then exited 0
in 785 seconds with 12,845 Flutter tests passed, one skipped, all eight Go tails
passed, and the final scope marker present. The same alternate-index tested
tree `8b4052ea418a537d1a9af145da44dd20d367fb7b` preserved the shared index
against unchanged `HEAD` `765be74523b25ce592ff200ff435653b35c66b4d`;
no synthetic commit or tag was created. Architecture passed at 165 dependency
exceptions, zero placement exceptions, and zero issues. The commands, archive
hashes, tested-state identities, workspace disclosure, and closure receipts are
recorded in the [Wave 4C evidence archive](evidence/dtr-wave4c/README.md).
DTR-16, DTR-17, and DTR-18 are `Wave-accepted`; DTR-18 is terminally complete.
Final release-closure `host-all` and `performance-host` remain later, separate
gates.
...
| Supported Android, iOS, macOS, Windows, Linux, and web release/CI targets | Any platform cleanup or DEP-01 decomposition/planning; Wave 5 cannot start | Product + release | **Open — blocking Wave 5 as of 2026-07-28** | No supported target matrix is recorded. Product + Release must explicitly state the supported Android/iOS bands and whether macOS, Windows, Linux, and web are release, CI-only, or unsupported targets. The availability-bounded device-test policy governs executable proof but does not answer this product/release decision. No resolution is inferred. |
...
| Extract production bootstrap and application-root phases behind stable interfaces | DTR-14 / Plan 293 | Current authenticated project owner | **Plan-green / Wave-accepted / closed 2026-07-28** | `DTR14-AUTH-01`: the owner explicitly ordered `$tdd-plan`, an independent critical `$tdd-review` with only necessary repairs, and implementation of DTR-14. The authorized boundary adds stable `ApplicationBootstrap`, `PreparedApplication`, and `ApplicationHost` seams; reduces the supported `main.dart` entrypoint to one delegation while preserving its three public symbols; keeps the concrete graph in one production bootstrap; moves `MyApp` lifecycle/push/root ownership to `application_root.dart`; migrates only exact source locks and runtime-root evidence; and preserves DTR-13 profiles and the standalone FCM handler. It excludes DTR-15 through DTR-18 decomposition, payload/schema/native/wire/profile changes, and any DTR-12 rebaseline. Plan 293's focused, mutation, lifecycle/push, DTR-13, shared, curated, Move Account, core/feature, policy, analyzer, hygiene, and Graphify gates are green. The later Wave 4B aggregate `host-all` and required same-tree `performance-host` replay passed; both remain mandatory again at final release closure. [Stable evidence](evidence/dtr-wave4b/README.md). |
| Extract shared direct/group conversation controllers without shared State inheritance | DTR-15 / Plan 294 | Current authenticated project owner | **Plan-green / Wave-accepted / closed 2026-07-28** | `DTR15-AUTH-01`: after ordering `$tdd-plan` and a critical `$tdd-review`, the owner explicitly ordered Codex to set the implementation goal and implement Plan 294. The authorized boundary extracts only composer snapshot, upload activity, recorder session, and reaction projection into four plain controllers; keeps both Wired States as independent composition roots; freezes public facades/widget APIs; and preserves lane-owned send, security, cancellation, auto-stop, background-task, reaction-admission, and batching policy. Retarget, ordinary-unmount, and ABA proof required captured group send-lane dependencies plus a monotonic binding generation distinct from mounted UI lifetime. All focused, curated, feature, performance, policy, analyzer, hygiene, and Graphify gates are green. The later Wave 4B aggregate `host-all` and required same-tree `performance-host` replay passed; both remain mandatory again at final release closure. [Stable evidence](evidence/dtr-wave4b/README.md). |
| Decompose `GroupMessageListener` behind its stable public facade | DTR-16 / Plan 296 | Current authenticated project owner | **Plan-green / Wave-accepted / closed 2026-07-28** | `DTR16-AUTH-01`: the owner explicitly ordered `$tdd-plan`, a critical `$tdd-review` with only necessary in-place repairs, and implementation of DTR-16. The authorized boundary keeps the public constructor, members, subclassability, subscriptions, stream controllers, account gate, cached identity, message queue, central `_handleMessage` ordering seam, in-flight tracking, and stop/dispose ownership on `GroupMessageListener`; extracts only four private state-owning collaborators for signed system transitions, durable membership-dependent buffering, incoming reactions, and media receive-task ownership; and requires component-specific typed ports without a facade back-reference, aggregate god-context, mixin, or extension substitute. It excludes schema/migration, repository, payload, crypto-policy, bridge/native/Go, notification-policy, DTR-12 exception, DTR-17, and DTR-18 changes. Plan 296's focused, mutation, `groups`, `feature-host-all`, policy, analyzer, hygiene, and Graphify proof is green; this closes the authorization receipt on its exact scope. The later separately authorized Wave 4C `performance-host` and full `host-all` aggregate passed; final release closure remains separate. [Stable evidence](evidence/dtr-wave4c/README.md). |
| Decompose `P2PServiceImpl` behind its stable public facade | DTR-17 / Plan 295 | Current authenticated project owner | **Plan-green / Wave-accepted / closed 2026-07-28** | `DTR17-AUTH-01`: the owner explicitly ordered `$tdd-plan`, a critical `$tdd-review` with only necessary in-place repairs, and implementation of DTR-17. The authorized boundary keeps the exact public facade/constructor/capabilities, raw node and Bridge callback ownership, subscriptions, lifecycle/readiness/recovery/state projection, push recovery, controllers, and final disposal; extracts the literal reviewed inbox-custody and peer/LAN-policy owners into two private part-file coordinators behind immutable typed primitive ports; and preserves Plan-290 wake readiness, post-stop callback differences, callback reinitialize behavior, and the eight exact DTR-12 exception identities. It excludes schema/wire/native/crypto changes, new async hops, DTR-16, DTR-18 relocation, and any exception rebaseline. Plan 295's focused, mutation, `1to1`, physical-Android `transport`, `core-host-all`, justified `performance-host`, policy, strict-analyzer, hygiene, and Graphify proof is green. The aggregate blockers were closed with one deterministic test-only retrier correction and one separately owned DTR-16 lifecycle-port correction, both re-proved through their affected families without widening DTR-17. The later separately authorized Wave 4C `performance-host` and full `host-all` aggregate passed; final release closure remains separate. [Stable evidence](evidence/dtr-wave4c/README.md). |
| Relocate the first DTR-18 layering increment without behavioral change | DTR-18 / Plan 297 | Current authenticated project owner | **Plan-green first increment / Wave-accepted / closed 2026-07-28** | `DTR18-AUTH-01`: the owner explicitly ordered `$tdd-plan`, a critical `$tdd-review` with only necessary in-place repairs, and implementation of DTR-18. The authorized increment moves the byte-identical `handle_app_resumed.dart` orchestration from core to the DTR-14 app lifecycle owner, moves only the two Posts Phase-3 concrete adapters from domain to data, rewrites their exact import/source-read consumers, removes only the resulting 17 dependency and two placement exceptions, and adds the causal structural contract plus existing Posts gate registration. It excludes pause, `P2PServiceImpl`, every other repository family, compatibility shims, behavior/schema/helper/native/Go/wire/crypto changes, and any new exception. Causal RED/GREEN, three mutation re-reds, 33-test preservation, DTR-14/16/17 sentinels, pinned Android Posts integration, curated/core/feature families, trustworthy 165/22 architecture, drift-free runtime roots, completeness, zero-issue analysis, hygiene, and refreshed Graphify evidence are green. This closes only Plan 297's first increment. Plans 298–300 later closed the remaining placement family, and the separately authorized Wave 4C aggregate passed; final release closure remains separate. [Stable evidence](evidence/dtr-wave4c/README.md). |
| Close the remaining DTR-18 placement family and disposition the dependency residual | DTR-18 / Plans 298–300 / Wave 4C aggregate | Current authenticated project owner acting as Architecture + Release owner | **Accepted, executed, and closed / DTR-18 terminal and Wave-accepted 2026-07-28** | `DTR18-AUTH-02`: preserve Plan 297 as the closed first increment, then relocate without compatibility shims the exact 13 Groups, three Conversation, and six remaining feature concrete repository implementations from feature `domain/repositories/` to `data/repositories/`, updating only imports and exact source locks while preserving implementation bodies and behavior/schema/native/wire/crypto boundaries. Exactly all 22 remaining placement exception identities were removed. Exactly the 165 post-Plan-297 dependency identities are retained; every retained manifest entry carries a concrete owner, reason, and revisit condition, and no identity was added or silently changed. The retained groups are 92 `lib/core/debug` proof-capability identities protected by `DTR13-AUTH-01`; eight `P2PServiceImpl` transport-boundary identities protected by `DTR17-AUTH-01`; eight application-pause lifecycle identities requiring future iOS/background proof; and 57 other source-owner identities requiring a future bounded source-family relocation with causal and preservation gates. The full disposition-tuple SHA-256 is `5f24faf4c5f693d0f19eb18503e5d37c4db4580c6ccbc91806abc75a78dcf132`. This is a first-class terminal disposition, not permission to reopen prior receipts or a TODO to drain the residual now. Plans 298–300 focused/family/policy/analyzer/Graphify verification passed at trustworthy 165 dependency / 0 placement / 0 issues, an overall DTR-18 delta of dependency -17 and placement -24. The Wave 4C 21-path `performance-host` passed 106 tests, then the 1,277-item full `host-all` passed 12,845 Flutter tests with one skip and all eight Go tails. DTR-18 is terminally complete and Wave 4C is accepted; final release closure remains separate. [Stable evidence](evidence/dtr-wave4c/README.md). |
