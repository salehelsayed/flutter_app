...
| 2026-07-04 | perf BEFORE capture | — | `flutter test -d macos …` → **BUILD FAILED**: "Cycle in dependencies … 'Pods-Runner' and 'Runner'" | macOS build-cycle blocker (not a 202 product issue; pre-existing) — root-caused + FIXED below | proceed, fix cycle |
...
| 2026-07-04 | named gates | — | groups **1041**, feed **285**, `flutter analyze` 0 new, `git diff --check` clean | gates green | QA |
...
| 2026-07-04 | macOS build-cycle fix + perf | macos/Podfile, pbxproj | drop `:output_files` from Runner's GoMknoon-bindings script → cycle broken (`8e04b8fb`); macOS build green; harness runs 5 ORBIT scenarios | perf lane unblocked | perf B/A |
| 2026-07-04 | perf BEFORE + AFTER | — | pre-202 vs 202 (host macOS): unread avg raster **3.39→2.53ms −25%** (missed 4→1); sculpt/find avg raster **4.08→2.84ms −30%**; build flat | measurable raster-cost drop in targeted scenarios | closed |
...
**Perf before/after — RESOLVED (was a macOS build-cycle blocker; fixed in `8e04b8fb`).** The report-only PERF_TARGET=ORBIT lane was initially blocked by `** BUILD FAILED **` — *"Cycle in dependencies between targets 'Pods-Runner' and 'Runner'"* (`Pods-Runner → GoMknoon → Runner → Pods-Runner`): the `Runner` target's `[CP-User] Ensure GoMknoon macOS Bindings` script declared `GoMknoon.xcframework` as its `:output_files`, and the `GoMknoon` pod's `[CP] Copy XCFrameworks` consumes that xcframework → Xcode ordered the copy after the Runner script, closing the cycle. **Fix:** drop the script's `:output_files` (`macos/Podfile`) — the xcframework is already built at `pod install` and the before_compile script is a fast idempotent freshness guard, so it runs every build harmlessly. macOS host build now green; the harness runs all 5 ORBIT scenarios. **Before/after (host macOS, same pinned config, single run each):** rasterizer cost drops measurably in exactly the two targeted scenarios — `orbit_open_unread_indicators` (canvas stream) avg raster **3.39 → 2.53 ms (−25%)**, missed-raster-budget **4 → 1**, p99 raster **24.2 → 14.8 ms (−39%)**; `orbit_open_arcs_expanded_sculpt_find` (chrome/drag stream) avg raster **4.08 → 2.84 ms (−30%)**. Build times are flat (the edits cut rasterizer re-work — blur/shadow re-raster — not build cost); the coalescer + anim-hoist are structurally unevidenceable here (harness bypasses OrbitWired with AlwaysStoppedAnimation — host widget rows carry those). The other three scenarios (no heavy rotation/drag) are flat within first-frame-outlier noise. All averages sit well under the 16.67 ms 60fps budget.

**Closure:** host-only (no DB migration, no device/relay/crypto boundary). All Done Criteria met, incl. the perf before/after numbers (the macOS-build blocker was root-caused and fixed).
