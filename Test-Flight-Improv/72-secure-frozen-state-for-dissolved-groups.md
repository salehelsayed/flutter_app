...
- Acceptance evidence should not rely on one or two happy-path checks. This feature should be accepted only when its observable behavior is covered across `unit`, `integration`, `smoke`, and `simulator` verification, because dissolve authority and frozen-state behavior must remain consistent across live, replay, recovery, and UI surfaces.
...
- No current repo evidence was found for dissolved-group reaction blocking, dissolved-state propagation into writeability, a former-admin-or-former-creator dissolve rejection case, replay/recovery authority protection for dissolve, or a user-facing local-delete-after-dissolv acceptance case.
