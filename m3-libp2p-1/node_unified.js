 ...
 * node_unified.js
 *
 * Complete unified P2P chat node combining sender (Node_A) and receiver (Node_B) functionality.
 *
 * Features:
 * - Registers on rendezvous (so others can find this node)
 * - Discovers other peers via rendezvous
 * - Handles incoming chat messages
 * - Sends outgoing chat messages
 * - Checks inbox for offline messages on startup
 * - Stores messages in inbox when peer is offline
 *
 * Usage:
 *   node node_unified.js <credentials-file> [target-peer-id] [message]
 *
 * Examples:
 *   node node_unified.js ./Creds_User_A.txt
 *   node node_unified.js ./Creds_User_A.txt 12D3KooW...
 *   node node_unified.js ./Creds_User_A.txt 12D3KooW... "Hello!"
 */
...
import { storeInInbox, retrieveFromInbox, ResponseStatus, INBOX_PROTOCOL } from './core_lib_js/dist/inbox.js'
...
const REGISTER_TTL_S = 2 * 60 * 60 // 2 hours
...
const log = (...args) => console.log('[Node]', ...args)
const warn = (...args) => console.warn('[Node]', ...args)
...
async function loadIdentity(credsPath) {
  const contents = await readFile(credsPath, 'utf8')
  ...
  return await createLibp2p({
    ...
    addresses: {
      listen: [
        ...
        '/ip4/0.0.0.0/tcp/0'  // TCP for DCUtR hole punching
      ]
    },
    transports: [
      webSockets(),
...
async function registerOnce(node, namespace) {
  const circuitAddrs = getCircuitAddresses(node)
  if (circuitAddrs.length === 0) throw new Error('No circuit addresses available')

  const connection = await node.dial(multiaddr(RENDEZVOUS_POINTS[0]), { signal: AbortSignal.timeout(20000) })
  const stream = await connection.newStream(RENDEZVOUS_PROTOCOL, { signal: AbortSignal.timeout(10000) })

  try {
    const privateKey = node.components?.components?.privateKey || node.components?.privateKey
    if (!privateKey) throw new Error('Cannot access node privateKey')
    ...
    if (response.type !== Message.MessageType.REGISTER_RESPONSE) throw new Error(`Unexpected response: ${response.type}`)
...
async function registerForever(node, namespace, signal) {
  while (!signal.aborted) {
    try {
      const ttl = await registerOnce(node, namespace)
...
async function discoverFromPoint(node, pointAddr, namespace) {
  let stream = null
  try {
...
async function discoverTarget(node, namespace, targetPeerId, timeoutMs = DISCOVER_TIMEOUT_MS) {
  const deadline = Date.now() + timeoutMs

  while (Date.now() < deadline) {
    const results = await Promise.allSettled(
      RENDEZVOUS_POINTS.map(addr => discoverFromPoint(node, addr, namespace))
    )
...
async function sendChatMessage(node, targetPeerId, message) {
  ...
  try {
    ...
    // Send message
    const framed = encodeFrame(encoder.encode(message))
    ...
    for await (const chunk of source) {
      ...
      if (buffer.length >= 4) {
        ...
        if (buffer.length >= 4 + len) {
          return decoder.decode(buffer.subarray(4, 4 + len))
        }
      }
    }
    throw new Error('Stream ended before reply')
  } finally {
    clearTimeout(timer)
...
function setupChatHandler(node) {
    ...
    try {
      ...
      for await (const chunk of source) {
        ...
        if (buffer.length >= 4) {
          ...
          if (buffer.length >= 4 + len) {
            const message = decoder.decode(buffer.subarray(4, 4 + len))
            console.log(`\n[${remotePeer.slice(0, 20)}...] ${message}`)

            // Send reply
            const reply = `received: ${message}`
            const framed = encodeFrame(encoder.encode(reply))
            stream.send(framed)
            break
          }
        }
...
async function storeOfflineMessage(node, toPeerId, message) {
  ...
  try {
    ...
    warn(`Inbox store failed: ${err?.message ?? err}`)
  }
  return false
}

async function sendWithFallback(node, targetPeerId, message) {
  try {
    const reply = await sendChatMessage(node, targetPeerId, message)
    ...
    if (stored) console.log('===== MESSAGE STORED IN INBOX =====\n')
  }
}
...
async function main() {
  ...
  const targetPeerId = args[1] || null
  const oneShotMessage = args.length > 2 ? args.slice(2).join(' ') : null

  if (!credsPath) {
    console.log('Usage: node node_unified.js <credentials-file> [target-peer-id] [message]')
    console.log('')
    console.log('Examples:')
    console.log('  node node_unified.js ./Creds_User_A.txt                     # Start node, wait for connections')
    console.log('  node node_unified.js ./Creds_User_A.txt 12D3KooW...         # Connect to peer, interactive chat')
    console.log('  node node_unified.js ./Creds_User_A.txt 12D3KooW... "Hi!"   # Send one message and exit')
    process.exit(1)
  }

  // Load identity and create node
  log('Loading identity...')
  const { privateKey, peerId } = await loadIdentity(credsPath)
  ...
  // Register on rendezvous (using own peer ID as namespace)
  const myNamespace = `mknoon:chat:${peerId.toString()}`
  log(`Namespace: ${myNamespace}`)

  const registrationAbort = new AbortController()
  void registerForever(node, myNamespace, registrationAbort.signal)
  ...
  if (targetPeerId) {
    ...
    if (result) {
      ...
      for (const ma of result.multiaddrs) {
        try {
          await node.dial(ma, { signal: AbortSignal.timeout(30000) })
  ...
  console.log('')
  if (targetPeerId) {
    console.log(`Type a message to send to ${targetPeerId.slice(0, 20)}...`)
  } else {
    console.log('Use /send <peer-id> <message> or /dial <peer-id> to connect')
  }
  console.log('')
  ...
  rl.on('line', (line) => {
    ...
    pending = pending.then(async () => {
      if (input.startsWith('/')) {
        ...
        switch (cmd) {
          ...
          case 'dial':
            ...
            const dialNs = `mknoon:chat:${dialId}`
            log(`Discovering ${dialId}...`)
            const disc = await discoverTarget(node, dialNs, dialId)
            if (disc) {
      ...
      // Send to target peer
      if (targetPeerId) {
        await sendWithFallback(node, targetPeerId, input)
      } else {
