 ...
 * node_unified_A.js
 *
 * Unified P2P chat node for User A with JSON message exchange.
 * Registers on rendezvous, discovers peers, sends and receives JSON chat messages.
 *
 * Identity: loaded from Creds_User_A.txt (same as node_A_sender_rendezvous.js)
 *
 * Usage:
 *   node node_unified_A.js [target-peer-id] [message]
 */
...
const CREDS_PATH = new URL('./Creds_User_A.txt', import.meta.url)
...
export async function loadIdentity(credsPath) {
  const contents = await readFile(credsPath ?? CREDS_PATH, 'utf8')
  const hex = extractPrivateKeyHex(contents)
...
async function main() {
  ...
  log('Loading identity from Creds_User_A.txt...')
  const { privateKey, peerId } = await loadIdentity()
