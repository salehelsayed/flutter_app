...
double _bottomTapPokeOracle({
  ...
  final bottom = layout.seats
      .map((s) => s.dy + math.max(s.avatarSize, kOrbitMinTapTarget) / 2)
      .reduce(math.max);
...
void main() {
  group('orbit_arc_layout pure math (198 F1)', () {
    ...
    test('arc radius formula: og isolates the first gap; sp scales all', () {
      ...
      expect(
        orbitArcRadius(_g(), 1) - orbitArcRadius(_g(), 0),
      ...
      expect(
        orbitArcRadius(_g(og: 2.5), 1) - orbitArcRadius(_g(og: 2.5), 0),
        closeTo(46, 1e-9),
      );
      ...
      expect(
        orbitArcRadius(_g(sp: 1.5), 1) - orbitArcRadius(_g(sp: 1.5), 0),
        closeTo(69, 1e-9),
      );
    });
    ...
    test(
      'arcPhi: base below 170, pinch-then-release above, never reaches pi',
      () {
    ...
    test(
      'cv ≤ 1.6: no seat exceeds 170px horizontal from centre (390px safe)',
      () {
        final layout = computeOrbitLayout(
          memberCount: 50,
          geometry: _g(cv: 1.6),
    ...
    test(
      '24 members distribute [8,3], nothing culled, partial arc centred',
      () {
    ...
    test('50 members: 4+ arcs, nothing culled', () {
      ...
      final arcs = layout.seats
          .where((s) => s.arcIndex != null)
          .map((s) => s.arcIndex!);
    ...
    test(
      'overflow fills arcs in member order; seat 14 is the first arc seat',
      () {
        final layout = computeOrbitLayout(memberCount: 24, geometry: _g());
    ...
    test('overhang: 0 without overflow, grows with the outermost arc', () {
      expect(
        orbitArcOverhang(memberCount: 13, geometry: _g(), centerY: 195),
        0,
      );
      expect(
        orbitArcOverhang(memberCount: 14, geometry: _g(), centerY: 195),
        closeTo(22, 1e-9),
      );
      expect(
        orbitArcOverhang(memberCount: 50, geometry: _g(), centerY: 195),
        closeTo(206, 1e-9),
      );
    });
    ...
    test(
      'badge re-spread: overflow re-spreads ring 2 to 9 slots, badge at 245°',
      () {
    ...
    test('TC-223-01 bottom overhang = max tap-box poke over ALL seats', () {
      const centerY = kOrbitCanvasCenter;
      final expanded = _g(av: 1.4, sp: 1.5);
      final expected = _bottomTapPokeOracle(
        memberCount: 50,
        geometry: expanded,
        centerY: centerY,
      );
      ...
      expect(expected, closeTo(25.4, 0.5));
      expect(
    ...
    test('TC-223-05a arcWrap droop counted (all-seats)', () {
      const centerY = kOrbitCanvasCenter;
      final wideWrap = _g(cv: 2.5);
      final expected = _bottomTapPokeOracle(
        memberCount: 50,
        geometry: wideWrap,
        centerY: centerY,
      ...
      expect(expected, greaterThan(100));
      expect(
        orbitArcBottomOverhang(
          memberCount: 50,
          geometry: wideWrap,
  ...
  group('198 fidelity — drag-mapping helpers (rows 21/22)', () {
    ...
    test('og drag sensitivity scales with spacing (÷46·sp)', () {
      ...
      expect(
        orbitGapDragDelta(-23, 1.0) / orbitGapDragDelta(-23, 1.5),
        closeTo(1.5, 1e-9),
      );
    });
    ...
    test('cv pointer-angle mapping: |atan2|/1.25 clamped to [0.5, 2.5]', () {
      ...
      expect(
        orbitArcWrapFromPointer(centre, const Offset(300, 217)),
        closeTo(math.pi / 2 / 1.25, 1e-9),
      );
      // Pointer due left mirrors through |atan2| — same wrap (RTL-safe).
      expect(
        orbitArcWrapFromPointer(centre, const Offset(20, 217)),
        closeTo(math.pi / 2 / 1.25, 1e-9),
      );
      ...
      expect(
        orbitArcWrapFromPointer(centre, const Offset(260, 159)),
