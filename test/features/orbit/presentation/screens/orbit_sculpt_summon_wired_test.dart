...
OrbitItem _friend(int i, {int unread = 0}) => OrbitFriendItem(
  OrbitFriend(
...
OrbitItem _group(String name) => OrbitGroupItem(
  OrbitGroup(
    group: GroupModel(
      id: 'g-$name',
      name: name,
...
void main() {
  ...
  ScrollPosition orbitScrollPosition(WidgetTester tester) =>
      tester.state<ScrollableState>(find.byType(Scrollable).first).position;

  Future<void> jumpOrbitScrollTo(WidgetTester tester, double offset) async {
    final position = orbitScrollPosition(tester);
  ...
  Future<void> tapAwayBg(WidgetTester tester) async {
    ...
    ); // clear double-tap window
  }

  Finder bannerF() => find.byKey(const ValueKey('orbit-edit-banner'));
  Finder handleF(OrbitKnob k) => find.byKey(ValueKey('orbit-handle-${k.name}'));
  ...
  String bubbleText(WidgetTester tester) => tester
      .widget<Text>(find.descendant(of: bubbleF(), matching: find.byType(Text)))
      .data!;
  ...
  group('198 F6 — edit session (Group B)', () {
    testWidgets(
      'TC-198-14 long-press empty space enters edit (banner, Reset)',
      (tester) async {
  ...
  group('198 F6 — persistence (Group D)', () {
    testWidgets('TC-198-33 sculpt persists across a remount', (tester) async {
      ...
      await tester.pumpWidget(
        host(_friends(8), store: store, key: const ValueKey('a')),
      );
      await settle(tester);
      await longPressBg(tester);
      await tester.tap(handleF(OrbitKnob.avatarScale));
      ...
      await tester.pump();
      await tester.pumpWidget(
        host(_friends(8), store: store, key: const ValueKey('b')),
      );
      await settle(tester);
      await longPressBg(tester);
      await tester.tap(handleF(OrbitKnob.avatarScale));
  ...
  group('198 F6 — composition (Group F) + reset (Group R)', () {
    ...
    testWidgets('TC-198-52 chip tap opens chat AND ends edit', (tester) async {
      ...
      expect(
        bannerF(),
        findsNothing,
        reason: 'chip tap ended the edit session',
      );
      await settle(tester);
    });
    ...
    testWidgets(
      'TC-198-63 reset signal collapses arcs, exits edit, clears find',
      (tester) async {
  ...
  group('198 fidelity — geometry-anchored handles (F rows)', () {
    ...
    testWidgets(
      'TC-198F-01 five handles seated at geometry anchors (expanded)',
      (tester) async {
        await tester.pumpWidget(host(_friends(20)));
        await settle(tester);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final gDrag = await tester.startGesture(
          tester.getCenter(handleF(OrbitKnob.spacingScale)),
        );
        ...
        for (final k in OrbitKnob.values) {
          ...
          expect(
            (tester.getCenter(handleF(k)) - expected).distance,
            lessThan(2.0),
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        for (final k in visibleKnobs) {
          expect(
            tester.getCenter(handleF(k)).dy - before[k]!.dy,
            closeTo(dy, 2),
        ...
        expect(
          bubbleF(),
          findsOneWidget,
          reason: 'armed og survived the scroll',
        );
        ...
        await settle(tester, count: 4);
        expect(handleF(OrbitKnob.spacingScale), findsOneWidget);
        final spExpected = expectedCenter(
          tester,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        for (var i = 0; i < 6; i++) {
          ...
          await tester.pump();
          final drift = (tester.getCenter(centreAvatarF()).dy - userBefore.dy)
              .abs();
          expect(
            drift,
            lessThan(8),
    ...
    testWidgets('TC-198F-06 RTL: tip↔knob assignment pinned in screen space', (
      ...
      final cvExpected = expectedCenter(
        tester,
        OrbitKnob.arcWrap,
        g0,
        20,
      ...
      final prExpected = expectedCenter(
        tester,
        OrbitKnob.maxPerArc,
        g0,
        20,
      ...
      expect(
        (cv - cvExpected).distance,
        lessThan(2.0),
        reason: 'cv follows the mirrored +φ tip',
      );
      expect(
        (pr - prExpected).distance,
        lessThan(2.0),
    ...
    testWidgets(
      'TC-198F-08 keyboard during edit+find does not desync handles',
      (tester) async {
        await tester.pumpWidget(host(_friends(20)));
        await settle(tester);
        await expandBadge(tester);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await longPressBg(tester);
        await tester.pump();
        final discF = find.byKey(
          const ValueKey('orbit-handle-disc-avatarScale'),
        );
        final haloF = find.byKey(
          const ValueKey('orbit-handle-halo-avatarScale'),
        );
        ...
        expect(
          haloOpacity(),
          isNot(closeTo(o0, 0.01)),
          reason: 'pulse is running',
        );
    ...
    testWidgets(
      'TC-198F-15 bubble tracks the handle and updates live mid-drag',
      (tester) async {
        await tester.pumpWidget(host(_friends(8)));
        await settle(tester);
        await longPressBg(tester);
    ...
    testWidgets('TC-203-11 bubble hugs the SCALED disc at 1.4 (shared '
    ...
    testWidgets(
      'TC-198-20S −/+ steppers land at the bottom corners while armed',
      (tester) async {
        await tester.pumpWidget(host(_friends(8)));
        await settle(tester);
        await longPressBg(tester);
        await settle(tester);
        expect(
          find.byKey(const ValueKey('orbit-edit-step-decrease')),
          findsNothing,
        ...
        final surface = tester.getRect(
          find.byType(InnerCircleInteractiveSurface),
        );
        ...
        expect(
          surface.bottom - dec.bottom,
          closeTo(28, 2),
          reason: '− rides the nav-bar line',
        );
        expect(
          surface.bottom - inc.bottom,
          closeTo(28, 2),
          reason: '+ rides the nav-bar line',
        );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final rects = <String, Rect>{
          'decrease': tester.getRect(
            find.byKey(const ValueKey('orbit-edit-step-decrease')),
          ),
          'increase': tester.getRect(
            find.byKey(const ValueKey('orbit-edit-step-increase')),
          ),
        ...
        for (var i = 0; i < names.length; i++) {
          for (var j = i + 1; j < names.length; j++) {
            expect(
              rects[names[i]]!.overlaps(rects[names[j]]!),
              isFalse,
        ...
        expect(
          surface.bottom - rects['increase']!.bottom,
          closeTo(120 + 28, 2),
        );
        expect(
          surface.bottom - rects['decrease']!.bottom,
          closeTo(120 + 28, 2),
        );
        expect(surface.bottom - rects['pill']!.bottom, closeTo(120 + 88, 2));
        for (final k in chipKeys) {
          expect(
            surface.bottom - rects[k]!.bottom,
            closeTo(120 + 144, 6),
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await longPressBg(tester);
        await settle(tester);
        OrbitalVisualization viz() => tester.widget<OrbitalVisualization>(
          find.byType(OrbitalVisualization),
        );
    ...
    testWidgets(
      ...
      (tester) async {
        await tester.pumpWidget(host(_friends(8)));
        await settle(tester);
        final dynamic vizIdle = tester.widget(
          find.byType(OrbitalVisualization),
        );
        ...
        await longPressBg(tester);
        await settle(tester);
        final dynamic vizEdit = tester.widget(
          find.byType(OrbitalVisualization),
        );
    ...
    testWidgets(
      'TC-198F-22 horizontal drag on the cv handle changes the wrap',
      (tester) async {
        await tester.pumpWidget(host(_friends(20)));
        await settle(tester);
        await expandBadge(tester);
    ...
    testWidgets('TC-198F-23 banner + Reset wear the green terminal chrome', (
      ...
      final bannerDeco =
          tester.widget<Container>(bannerF()).decoration! as BoxDecoration;
      expect(
        bannerDeco.border,
        isNotNull,
        reason: 'banner wears the green-tinted bordered pill',
      );
      expect((bannerDeco.border! as Border).top.color, const Color(0x591DB954));
      ...
      expect(
        (bRect.center.dx - surface.center.dx).abs(),
        lessThan(2.0),
      ...
      expect(
        find.descendant(of: resetF, matching: find.text('Reset')),
        findsOneWidget,
        reason: 'copy unchanged',
      );
      expect(
        find.ancestor(
          of: find.text('Reset'),
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        for (final k in OrbitKnob.values) {
          ...
          expect(
            tester.getSemantics(f).flagsCollection.isButton,
            isTrue,
        ...
        await tester.tap(handleF(OrbitKnob.avatarScale), warnIfMissed: false);
        await tester.pump();
        final incLabel = en.orbit_edit_step_increase(
          names[OrbitKnob.avatarScale]!,
        );
        final decLabel = en.orbit_edit_step_decrease(
          names[OrbitKnob.avatarScale]!,
        );
        ...
        expect(
          find.bySemanticsLabel(en.orbit_find_pill_semantics),
          findsOneWidget,
        );
        await settle(tester);
        semantics.dispose();
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          await store.read(OrbitGeometryPrefs.storageKey),
          isNull,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pumpWidget(
          host(_friends(20), platform: TargetPlatform.iOS),
        ); // fits the viewport
        await settle(tester);
        await expandBadge(tester);
        await longPressBg(tester);
        ...
        for (var i = 0; i < 4; i++) {
          ...
          await tester.pump();
          expect(
            canvasOrigin(tester).dy,
            closeTo(originBefore.dy, 1.0),
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          b.bottom,
          lessThanOrEqualTo(surface.bottom + 0.1),
          reason: 'bubble never paints past the surface bottom',
        );
        expect(
          b.top,
          greaterThanOrEqualTo(surface.top - 0.1),
  ...
  group('223 — expanded arc bottom overhang', () {
    testWidgets(
      'TC-223-02 bottom node is TAPPABLE under large expanded geometry',
      (tester) async {
        final geometry = OrbitGeometryPrefs.defaults.copyWith(
          avatarScale: 1.4,
          spacingScale: 1.5,
        );
        final store = FakeSecureKeyStore();
        ...
        final bottomSeat = _bottomTapSeat(memberCount: 50, geometry: geometry);
        await jumpOrbitScrollTo(tester, position.maxScrollExtent);

        final node = find.byKey(ValueKey('orbit-node-${bottomSeat.index}'));
        expect(node, findsOneWidget);
        await tester.tap(node, warnIfMissed: false);
        await tester.pump();
        expect(tappedFriends.single.username, 'friend${bottomSeat.index}');
      },
    );

    testWidgets(
      'TC-223-03 spacing-handle drag keeps the circle planted (scroll compensates)',
      (tester) async {
        await tester.pumpWidget(host(_friends(80)));
        await settle(tester);
        await expandBadge(tester);
        ...
        for (var i = 0; i < 6; i++) {
          ...
          expect(
            drift,
            lessThan(8),
            reason: 'circle planted during spacing drag (step ${i + 1})',
          );
        }
        await gesture.up();
        await tester.pump();
        ...
        expect(
          find.text('1.0×'),
          findsNothing,
          reason: 'spacing actually changed during the drag',
        );
        await settle(tester);
      },
    );
    ...
    testWidgets(
      'TC-223-04 expanded-but-fitting population gains no phantom scroll',
      (tester) async {
        await tester.pumpWidget(host(_friends(50)));
        await settle(tester);
        await expandBadge(tester);
        expect(orbitScrollPosition(tester).maxScrollExtent, closeTo(0.0, 0.1));
        await settle(tester);
      },
    );

    testWidgets('TC-223-05b bottom ARC node tappable at arcWrap=2.5', (
      tester,
    ) async {
      final geometry = OrbitGeometryPrefs.defaults.copyWith(arcWrap: 2.5);
      final bottomSeat = _bottomTapSeat(
        memberCount: 50,
        geometry: geometry,
        arcsOnly: true,
      );
      expect(bottomSeat.kind, OrbitSeatKind.arc);
      expect(
        _seatBottomTapPoke(bottomSeat) - kOrbitCanvasCenter,
        greaterThan(100),
      );
      final store = FakeSecureKeyStore();
      await store.write(
    ...
    testWidgets(
      'TC-223-06 handles stay seated & circle planted at sp=1.5 with bottom-growth active',
      (tester) async {
        const itemCount = 80;
        final geometry = OrbitGeometryPrefs.defaults.copyWith(
          spacingScale: 1.5,
        ...
        await tester.pumpWidget(host(_friends(itemCount), store: store));
        await settle(tester);
        await expandBadge(tester);
        final position = orbitScrollPosition(tester);
        expect(position.maxScrollExtent, greaterThan(0));
        ...
        for (final knob in OrbitKnob.values) {
          final handle = handleF(knob);
          if (handle.evaluate().isEmpty) continue;
          visible++;
          final expected = expectedCenter(
            tester,
            knob,
          ...
          final actual = tester.getCenter(handle);
          expect(
            (actual - expected).distance,
            lessThan(2.0),
            reason: '${knob.name} stayed seated under bottom growth',
          );
        }
        ...
        for (var i = 0; i < 6; i++) {
          ...
          expect(
            drift,
            lessThan(8),
            reason: 'sp=1.5 planted guard stayed stable (step ${i + 1})',
          );
        }
        await gesture.up();
        await tester.pump();
  ...
  group('201 — element identity + find UX (TC-201)', () {
    ...
    testWidgets('TC-201-01 find TextField State survives edit enter and exit', (
      ...
      expect(
        identical(tester.state(find.byType(EditableText)), s1),
        isTrue,
    ...
    testWidgets('TC-201-03 expanded find pill is a full-width >=48-high bar', (
      ...
      final chip0 = tester.getRect(
        find.byKey(const ValueKey('orbit-find-chip-0')),
      );
      expect(
        pill.overlaps(chip0),
        isFalse,
    ...
    testWidgets('TC-201-08 label double-tap does not remount orbit nodes', (
      ...
      await settle(tester);
      final before = tester
          .stateList<State>(find.byType(OrbitalAvatar))
          .toList();
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pump();
        expect(
          identical(tester.state(find.byType(OverflowBadge)), b1),
          isTrue,
    ...
    testWidgets(
      'TC-201-14 bottomClearance preserves the staggered bottom bands',
      (tester) async {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        for (final knob in const [
          OrbitKnob.avatarScale,
          OrbitKnob.spacingScale,
        ]) {
          expect(
            find.byKey(
              ValueKey('orbit-handle-boundary-${knob.name}'),
              skipOffstage: false,
            ),
            findsOneWidget,
    ...
    testWidgets(
      ...
      (tester) async {
        var builds = 0;
        await tester.pumpWidget(
          host(_friends(8), onSurfaceBuild: () => builds++),
        );
        await settle(tester);
  ...
  group('205 — canvas centering + edit-mode chrome (TC-205)', () {
    ...
    Widget innerScreen({
      ...
      header: OrbitHeaderProjection(
        userPeerId: 'me',
        innerItems: _friends(friends),
      ),
      onToggleView: onToggleView ?? () {},
      onInnerEdit: onInnerEdit,
    );

    testWidgets('TC-205-05 orbit canvas is vertically centered', (
      tester,
    ) async {
    ...
    testWidgets('TC-205-06 entering edit hides the toggle; Reset '
    ...
    testWidgets('TC-205-07 edit-active still forwarded upward (198 swipe-yield '
