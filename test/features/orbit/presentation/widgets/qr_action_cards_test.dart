  ...
  group('QRActionCards', () {
    testWidgets('renders two action cards in a Row', (tester) async {
      await tester.pumpWidget(
        wrap(QRActionCards(onMyQR: () {}, onScanQR: () {})),
      );
      expect(find.byType(Row), findsWidgets);
      expect(find.byType(Expanded), findsNWidgets(2));
    });

    testWidgets('renders qr_code and camera_alt_outlined icons', (
      tester,
    ) async {
      await tester.pumpWidget(
        wrap(QRActionCards(onMyQR: () {}, onScanQR: () {})),
    ...
    testWidgets('calls onMyQR when first card tapped', (tester) async {
      ...
      await tester.pumpWidget(
        wrap(QRActionCards(onMyQR: () => myQRTapped = true, onScanQR: () {})),
      );
    ...
    testWidgets('calls onScanQR when second card tapped', (tester) async {
      ...
      await tester.pumpWidget(
        wrap(QRActionCards(onMyQR: () {}, onScanQR: () => scanQRTapped = true)),
      );
      await tester.tap(find.text('Scan QR Code'));
      expect(scanQRTapped, isTrue);
    });

    testWidgets('renders title and subtitle text for each card', (
      tester,
    ) async {
      await tester.pumpWidget(
        wrap(QRActionCards(onMyQR: () {}, onScanQR: () {})),
    ...
    testWidgets('uses readable colors on the daylight theme', (tester) async {
      const colors = BackgroundReadableColors.representativeLight;

      await tester.pumpWidget(
        wrap(
          QRActionCards(onMyQR: () {}, onScanQR: () {}),
          readableColors: colors,
        ),
      );

      final title = tester.widget<Text>(find.text('My QR Code'));
      final subtitle = tester.widget<Text>(find.text('Share to add friends'));

      expect(title.style!.color, colors.textPrimary);
      expect(subtitle.style!.color, colors.textMuted);
      expectTextContrast(title.style!.color!, const Color(0xFFE5F4EA));
      expectTextContrast(subtitle.style!.color!, const Color(0xFFE5F4EA));
    });
  });
}
