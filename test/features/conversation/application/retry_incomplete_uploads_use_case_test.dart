...
void main() {
  ...
  group('retryIncompleteUploads', () {
    ...
    test(
        ...
        () async {
      final msg = _makeMsg('msg-00001', status: 'failed', contactPeerId: 'peer-bob');
      messageRepo.seed([msg]);
      mediaRepo.seed([
        _pendingAtt(
            id: 'att-00001',
    ...
    test(
      ...
      mediaRepo.seed([
        _pendingAtt(messageId: 'msg-00001')
    ...
    test('returns 1 after successful re-upload and send (single attachment)',
        () async {
      final msg = _makeMsg('msg-00001', status: 'failed', contactPeerId: 'peer-bob');
      messageRepo.seed([msg]);
      identityRepo.seed(FakeIdentityRepository.makeIdentity());
    ...
    test(
        ...
        () async {
      final msg =
          _makeMsg('msg-multi-001', status: 'failed', contactPeerId: 'peer-bob');
      messageRepo.seed([msg]);
      identityRepo.seed(FakeIdentityRepository.makeIdentity());
      mediaRepo.seed([
        _pendingAtt(
    ...
    test(
        ...
        () async {
      final msg1 =
          _makeMsg('msg-00001', status: 'failed', contactPeerId: 'peer-bob');
      final msg2 =
          _makeMsg('msg-00002', status: 'failed', contactPeerId: 'peer-bob');
      ...
      fakeUploadFn.willReturnForPath(
          '/tmp/file2.m4a', _doneAttachment('blob-2', 'msg-00002'));
    ...
    test('two messages each with multiple attachments: independent recovery',
        () async {
      final msg1 =
          _makeMsg('msg-00001', status: 'failed', contactPeerId: 'peer-bob');
      final msg2 =
      ...
      expect(p2pService.storeInInboxCallCount, 2); // 2 sends (one per message)
    });

    // G.8.2.1
    test('first upload failure keeps row as upload_pending (retryable)',
        () async {
      final msg =
          _makeMsg('msg-test-t1', status: 'failed', contactPeerId: 'peer-bob');
      messageRepo.seed([msg]);
      identityRepo.seed(FakeIdentityRepository.makeIdentity());
      mediaRepo.seed([
        _pendingAtt(
    ...
    test(
        ...
        () async {
      final msg =
          _makeMsg('msg-test-t2', status: 'failed', contactPeerId: 'peer-bob');
      messageRepo.seed([msg]);
      identityRepo.seed(FakeIdentityRepository.makeIdentity());
      mediaRepo.seed([
        _pendingAtt(
    ...
    // G.8.2.3
    test('missing local file is immediately terminal regardless of retryCount',
        () async {
      final msg =
          _makeMsg('msg-test-t3', status: 'failed', contactPeerId: 'peer-bob');
      messageRepo.seed([msg]);
      identityRepo.seed(FakeIdentityRepository.makeIdentity());
      mediaRepo.seed([
        MediaAttachment(
    ...
    // G.8.3.1
    test('partial upload crash: re-uploads only pending, combines with done',
        () async {
      final msg = _makeMsg('msg-partial-001',
          status: 'failed', contactPeerId: 'peer-bob');
      messageRepo.seed([msg]);
      identityRepo.seed(FakeIdentityRepository.makeIdentity());
      ...
      fakeUploadFn.willReturn(
        _doneAttachment('att-pending-3', 'msg-partial-001', mime: 'image/jpeg'),
      );
    ...
    // G.10.1.1
    test('retry after interrupted sendLocalMedia uses relay, not local WiFi',
        () async {
      final msg = _makeMsg('msg-voice-local-001',
          status: 'failed', contactPeerId: 'peer-bob');
      messageRepo.seed([msg]);
      identityRepo.seed(FakeIdentityRepository.makeIdentity());
      mediaRepo.seed([
      ...
      fakeUploadFn.willReturn(
        _doneAttachment('voice-local-att', 'msg-voice-local-001',
            mime: 'audio/mp4'),
      );
