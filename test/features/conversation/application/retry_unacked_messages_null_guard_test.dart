...
const _validV2Envelope =
    '{"type":"chat_message","version":"2","id":"msg-good-envelope","senderPeerId":"peer-alice","encrypted":{"kem":"fake-kem","ciphertext":"abc123","nonce":"n-1"}}';
const _legacyV1Envelope = '{"type":"chat_message","version":"1","payload":{}}';
...
void main() {
  ...
  group('retryUnackedMessages -- null wireEnvelope guard', () {
    ...
    // ------------------------------------------------------------------
    test('skips message with null wireEnvelope without crashing', () async {
    ...
    // ------------------------------------------------------------------
    test('skips message with empty wireEnvelope string', () async {
      final badMsg = ConversationMessage(
    ...
    // ------------------------------------------------------------------
    test('processes message with valid wireEnvelope normally', () async {
      ...
      expect(p2pService.lastStoreInInboxMessage, _validV2Envelope);
      expect(count, 1);
    });

    test(
      'skips legacy v1 wireEnvelope as unsafe and marks the row failed',
      () async {
        final legacyMsg = ConversationMessage(
          id: 'msg-legacy-envelope',
          contactPeerId: 'peer-bob',
          senderPeerId: 'peer-alice',
          text: 'Hello',
          timestamp: _testTs,
        ...
        expect(p2pService.storeInInboxCallCount, 0);
        expect(count, 0);
        expect(messageRepo.lastSavedMessage?.id, 'msg-legacy-envelope');
        expect(messageRepo.lastSavedMessage?.status, 'failed');
      },
    );
