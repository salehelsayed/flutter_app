...
void main() {
  ...
  test(
    'active exit distinguishes degraded notice native and post-commit cleanup outcomes',
    () async {
      Future<
        ({
          InMemoryGroupRepository groupRepo,
      ...
      final degraded = await seeded();
      final inboxFailureBridge = FakeBridge();
      inboxFailureBridge.responses['group:inboxStore'] = {
        'ok': false,
        'errorCode': 'INBOX_FAILED',
      };
      final degradedResult = await LeaveGroupAndDeleteLocalHistoryUseCase(
        bridge: inboxFailureBridge,
        groupRepo: degraded.groupRepo,
        groupMessageRepo: degraded.messageRepo,
        identityRepo: identityRepository(),
      ).call(groupId);
      expect(degradedResult.status, LeaveGroupAndDeleteLocalHistoryStatus.left);
      expect(
        inboxFailureBridge.commandLog.where(
          (command) => command == 'group:leave',
        ),
        hasLength(1),
      );
      expect(await degraded.groupRepo.getGroup(groupId), isNull);
      expect(
        await degraded.messageRepo.getLatestSystemEventTimestampForTarget(
          groupId,
  ...
  test(
    ...
    () async {
      ...
      final rollbackIdentity = identityRepository();
      final rollbackBridge = _AfterFirstSignBridge(() async {
        rollbackIdentity.seed(null);
      });
