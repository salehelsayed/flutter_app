...
/// when user-B deletes a dissolved group from Orbit (swipe-left -> Delete),
/// only that group is removed. B's friends list and 1:1 chat history with each
/// friend must remain intact, and other users (A, C) must not have their local
...
void main() {
  group('Group delete from Orbit preserves friends and 1:1 chat history', () {
    ...
    ContactModel contactFor(GroupTestUser other) => ContactModel(
      peerId: other.peerId,
    ...
    test(
      'B deletes the dissolved group: B keeps friends + DMs; A and C local group state untouched',
      () async {
        ...
        // 1:1 message repo (only Bob's DMs are asserted below)
        final bobMessages = InMemoryMessageRepository();

        // Mutual friendships: each user knows the other two.
        await aliceContacts.addContact(contactFor(bob));
        await aliceContacts.addContact(contactFor(charlie));
        await bobContacts.addContact(contactFor(alice));
        await bobContacts.addContact(contactFor(charlie));
        await charlieContacts.addContact(contactFor(alice));
        await charlieContacts.addContact(contactFor(bob));
        ...
        final bobIncomingTexts = (await bob.loadGroupMessages(
          groupId,
        )).where((m) => m.isIncoming).map((m) => m.text).toList();
        expect(
          bobIncomingTexts,
        ...
        final bobContactsBefore = (await bobContacts.getAllContacts())
            .map((c) => c.peerId)
            .toSet();
        final bobDmsWithAliceBefore = await bobMessages.getMessagesForContact(
          alice.peerId,
        );
        final bobDmsWithCharlieBefore = await bobMessages.getMessagesForContact(
          charlie.peerId,
        );
        ...
        // -- act: Bob has observed the dissolve and deletes its read-only
        //          local history from Orbit (swipe-left -> Delete). Alice and
        //          Charlie intentionally remain on their existing local
        //          snapshots to prove Bob's cleanup cannot reach their repos.
        final bobGroupBeforeDelete = (await bob.groupRepo.getGroup(groupId))!;
        await bob.groupRepo.updateGroup(
          bobGroupBeforeDelete.copyWith(
            isDissolved: true,
            dissolvedAt: DateTime.utc(2026, 4, 10, 12),
            dissolvedBy: alice.peerId,
          ),
        );
        ...
        expect(
          ...
          isEmpty,
          reason:
              'Bob should no longer have any messages for the deleted group',
        );

        // -- assert: Bob's friends are intact (the regression we are guarding).
        final bobContactsAfter = (await bobContacts.getAllContacts())
            .map((c) => c.peerId)
            .toSet();
        expect(
          bobContactsAfter,
        ...
        final bobDmsWithAliceAfter = await bobMessages.getMessagesForContact(
          alice.peerId,
        );
        expect(
        ...
        final bobDmsWithCharlieAfter = await bobMessages.getMessagesForContact(
          charlie.peerId,
        );
        expect(
    ...
    test(
      'B has multiple groups: deleting one dissolved group does not touch the others',
      () async {
        final alice = GroupTestUser.create(
          peerId: 'alice-peer',
          username: 'Alice',
        ...
        await alice.createGroup(groupId: 'group-keep', name: 'Group To Keep');
        ...
        await alice.sendGroupMessage(groupId: 'group-keep', text: 'this stays');
        ...
        // -- act: only the target has reached Bob as dissolved; the other
        //          group remains active and must survive the local cleanup.
        final bobGroupBeforeDelete = (await bob.groupRepo.getGroup(
          'group-delete',
        ))!;
        await bob.groupRepo.updateGroup(
          bobGroupBeforeDelete.copyWith(
