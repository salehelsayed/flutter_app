...
void main() {
  ...
  group('I1 happy: text + media over loopback', () {
    test('text: A -> B delivers, B acks, content intact', () async {
      ...
      try {
        ...
        expect(
          ...
          reason: 'unconfigured B should send the legacy parse-time ack',
        );
    ...
    test('media production-wiring: PUT -> mediaReadyStream -> persistMedia '
        'lands a SHA-256-verified file in the persistent media dir', () async {
    ...
    test(
      ...
      () async {
        ...
        try {
          ...
          final liveSend = await serverA.sendMessageWithAck(
            'localhost',
            portB,
            'after recovery',
            peerA,
          ...
          expect(
            liveSend,
            LanSendAck.legacyAck,
            reason: 'stale entry must not block legacy ack reuse',
          );
