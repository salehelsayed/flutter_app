...
void main() {
  ...
  Future<void> runUpgradePathFromV1(
    ...
    await productionUpgradeMigrations
        ...
        .run(db);
    // Plan 345 advances the repository fixture to the v110 direct-message
    // mapper shape. The intervening migrations do not add message columns.
    await productionUpgradeMigrations
        .singleWhere((entry) => entry.version == 110)
        .run(db);
  }
  ...
  group('Production migration registries (TC-228-01)', () {
    ...
    test('VC2-02 production registry reaches v117 exactly once', () {
      ...
      for (final registry in [
        ...
        expect(registry.where((entry) => entry.version == 117), hasLength(1));
        ...
        final index117 = registry.indexWhere((entry) => entry.version == 117);
        expect(index117, index116 + 1);
        final index118 = registry.indexWhere((entry) => entry.version == 118);
        expect(index118, index117 + 1);
        expect(index118, registry.length - 1);
        expect(registry[index118].name, '118_call_history_read_state');
        expect(
          registry[index118].run,
          same(runCallHistoryReadStateMigration),
        );
        ...
        expect(registry[index117].name, '117_call_history');
        expect(registry[index117].run, same(runCallHistoryMigration));
      }
    });
    ...
    test(
      ...
      () async {
        ...
        try {
          ...
          expect(
            await getRegistryTableNames(fresh),
            contains('group_exit_diagnostics'),
          );
          expect(await fresh.query('group_exit_diagnostics'), isEmpty);
          expect(await fresh.query('group_exit_intents'), isEmpty);
          await fresh.close();

          final upgradePath = '${tempDir.path}/upgrade.db';
          ...
          await predecessor.insert('groups', <String, Object?>{
            'id': 'v103-group',
            'name': 'v103 predecessor bytes stay exact',
            'type': 'chat',
            'topic_name': 'topic-v103',
          ...
          final groupsBefore = await predecessor.query('groups');
          final intentsBefore = await predecessor.query('group_exit_intents');
          ...
          expect(await upgraded.query('groups'), groupsBefore);
          expect(await upgraded.query('group_exit_intents'), intentsBefore);
          expect(await upgraded.query('group_exit_diagnostics'), isEmpty);
