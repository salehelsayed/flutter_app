...
void main() {
  ...
  test('expired presence entry is evicted on read (never a stale reachable)', () async {
    var now = DateTime.utc(2026, 6, 27, 12);
    await withClock(Clock(() => now), () async {
      bridge.presence = 'reachable';
      expect(await service.lookupRelayPresence('peer-y'), RelayPresence.reachable);
      ...
      expect(r, isNot(RelayPresence.reachable)); // the stale reachable is gone
  ...
  test('lookupRelayPresence is gated by _allowsAccountNetworkSideEffects', () async {
