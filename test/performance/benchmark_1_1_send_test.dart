...
void main() {
  ...
  group('Benchmark: Per-Step 1:1 Send Breakdown', () {
    test(
      'A1: Cold send emits CHAT_MSG_SEND_TIMING with per-step breakdown',
      () async {
    ...
    test('A2: Warm send shows connectionReused = true', () async {
      ...
      // A non-circuit connection is eligible for the reuse fast path. A
      // circuit-only connection intentionally enters the ranked race instead.
    ...
    test('A3: Cold and warm sends report different sendPath', () async {
      ...
      final coldTimings = harness.filterEvents(
        coldEvents,
        'CHAT_MSG_SEND_TIMING',
      );
      expect(coldTimings, isNotEmpty);
      final coldDetails = coldTimings.first['details'] as Map<String, dynamic>;
      expect(coldDetails['connectionReused'], isFalse);
      expect(coldDetails['sendPath'], isNot('reuse'));

      // Warm send — mark bob as directly connected. Circuit-only connections
      // are deliberately not classified as reuse.
      alice.p2pService.testConnections.add(
        p2p.ConnectionState(
          peerId: bob.peerId,
          multiaddrs: ['/ip4/10.0.0.2/tcp/4001'],
      ...
      final warmTimings = harness.filterEvents(
        warmEvents,
        'CHAT_MSG_SEND_TIMING',
      );
      expect(warmTimings, isNotEmpty);
      final warmDetails = warmTimings.first['details'] as Map<String, dynamic>;
    ...
    test('A4: Sequential sends show connection reuse effect', () async {
      ...
      // After first send, simulate a reusable non-circuit connection.
      alice.p2pService.testConnections.add(
        p2p.ConnectionState(
          peerId: bob.peerId,
          multiaddrs: ['/ip4/10.0.0.2/tcp/4001'],
      ...
      for (var i = 2; i <= 10; i++) {
        ...
        allEvents.addAll(harness.filterEvents(events, 'CHAT_MSG_SEND_TIMING'));
      }
