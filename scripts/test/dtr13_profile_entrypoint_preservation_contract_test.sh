...
# The bootstrap/application-root split is structural only. Neither extracted
# owner is an entrypoint and every production/Sims command continues to target
# the same public root.
main = read("lib/main.dart")
production_bootstrap = read(
    "lib/app/bootstrap/production_application_bootstrap.dart"
)
application_root = read("lib/app/application_root.dart")
composition = read("lib/debug/debug_e2e_composition_root.dart")
require(main, "application_bootstrap.dart", "lib/main.dart")
require(main, "production_application_bootstrap.dart", "lib/main.dart")
require(main, "runApplicationBootstrap(", "lib/main.dart")
require(
...
assert "main.dart" not in "\n".join(
    line for line in application_root.splitlines()
    if line.lstrip().startswith("import ")
), "the application root must receive dependencies without importing main.dart"
