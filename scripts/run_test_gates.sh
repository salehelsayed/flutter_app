...
readonly ONE_TO_ONE_TESTS=(
  ...
  # the additive short-lived TURN credential bridge contract. The real
  # PeerConnection create/close proof remains a separately pinned device leg.
  ...
  "test/features/settings/presentation/widgets/app_diagnostics_settings_section_test.dart"
  ...
  # Plan 400 (b): PushKit call wake channel.
  "test/features/call/infrastructure/ios_call_wake_channel_test.dart"
  ...
  "test/core/bridge/bridge_blob_decrypt_test.dart"
  ...
  "test/features/orbit/presentation/widgets/orbit_call_preview_test.dart"
...
classify_path() {
  ...
  if [[ "$path" =~ ^test/core/(bootstrap|bridge|constants|database|device|inbox|l10n|local_discovery|media|permissions|secure_storage|theme|utils|widgets)/.*_test\.dart$ ]]; then
