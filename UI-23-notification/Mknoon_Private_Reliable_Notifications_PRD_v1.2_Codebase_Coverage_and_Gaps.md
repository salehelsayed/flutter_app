...
- **Repository baseline:** Branch `protected-view`; Plan 344 implementation commit `185ba42027440a49bd84311038a70ab10435914c` (execution baseline `6bab4c485dd4d13887392915abaaaabf85ea4b21`)
- **Assessment scope:** Current implementation through the bounded, default-off Plan 344 direct-inbox ACK-or-expiry custody slice. Plan 344 focused/preservation, relay/native/mixed-version, curated `1to1`/`feed`, core/feature family, Android-binding, analyzer, formatting, and diff-hygiene receipts are included alongside the retained Plan 342/343 device evidence; production admission and full rollout/device-campaign evidence remain outside this bounded slice.
...
- fresh ordinary direct text and direct reaction ADD/REMOVE retain sender-owned exact-envelope custody independently of live ACK and now have a default-off protected Redis lane with non-destructive capacity and exact ACK-or-expiry proof, but media, edits/deletes, group ordering, device fanout, production activation, and historical legacy rows still lack the universal PRD contract;
...
Plans 342 through 344 materially narrow GAP-N01 for fresh ordinary direct text and direct reaction ADD/REMOVE. Plan 344 implements scoped non-destructive relay capacity, exact ACK-or-expiry proof, and mixed-version receive/ACK behavior for those DB v108/v109 obligations, but no bounded slice fully closes another composite A-control. A-17 remains Partial because generic opaque wake, recipient outcome ownership, universal normalization, remaining event/device lanes, and production activation remain open. The strict count therefore remains 2 Compliant, 19 Partial, 9 Missing, or **38.3%**.
...
For capable DB v108/v109 senders using an admission-enabled Plan 344 relay, the sender-held persistence-only window is closed only by exact protected acceptance, and protected capacity cannot discard an older unacknowledged obligation to accept a new one. The default deployment state is still admission-off; historical legacy rows, old clients, and media/edit/delete/group/device-fanout lanes retain their existing risk. None of this supplies the completed local notification outcome required by GAP-N03, so a recipient can still persist an event and suspend before presentation ownership is established.
...
- Extend the protected ACK-or-expiry contract beyond DB v108/v109 to every required event kind and independently addressed recipient device; the additive protected lane is a bounded mechanism, not universal inbox custody.
...
- Plans 342/343 keep fresh ordinary direct text and direct-reaction ADD/REMOVE in sender-owned custody despite direct/authenticated delivery, and Plan 344 permits retirement only after exact protected ACK-or-expiry relay acceptance. None records a completed recipient-side policy/presentation outcome; remaining GAP-N01 lanes retain their older reachability-dependent custody behavior.
...
|---|---|---|---|
| A-01 | Partial | Sender-generated identities are reused and receiver persistence deduplicates them. Plan 342 binds fresh ordinary direct text to one immutable envelope/outbox incarnation and gives current edits matching authenticated outer/inner `eventId` plus a separate relay namespace. Plan 343 atomically binds each direct-reaction transition ID to its exact envelope, preserves receiver event/action/target parity, and gives reaction events a distinct relay namespace. Plan 344's protected lane admits duplicates only for the exact recipient + namespaced key + authenticated sender + byte-identical envelope tuple. Group pubsub and inbox reuse one envelope: `go-mknoon/node/pubsub.go:388-453`. | Extend one authenticated `event_id` and content hash contract across every direct/group event, recipient device, relay entry, replay, and notification ledger; the bounded text/reaction slices do not supply universal identity/custody. |
| A-02 | Partial | Relay inbox paths commit before push (`go-relay-server/inbox.go`), Plan 342 makes fresh ordinary direct-text sender custody independent of live ACK through DB v108, and Plan 343 does the same for direct-reaction ADD/REMOVE through DB v109. Plan 344 gives those lanes additive Redis-backed non-destructive capacity: exact duplicates remain accepted, while a new distinct obligation at cap is rejected without eviction or push. | Activate and evidence the default-off protected lane, extend sender/inbox custody to media, edit/delete, group, and every-device lanes, retire legacy eviction after compatibility drain, and make group durability independent of concurrent pubsub. |
| A-03 | Partial | Non-destructive pending retrieval and explicit stable-entry ACK exist (`go-relay-server/inbox_store.go`, `go-relay-server/inbox.go`). Plans 342/343 stage exact text/reaction custody before transport; Plan 344 makes every DB v108/v109 owner retire only on exact `stored|duplicate` plus `ack_or_expiry_v1`, provides protected retrieve/ACK across configured relays, and preserves rows on full, legacy, missing-proof, or ambiguous outcomes. | Retire the legacy destructive retrieve/evict contract after old-client drain, extend exact sender custody beyond bounded text/reactions, prove production activation/persistence, and decide whether encrypted staging before successful decrypt/canonical persistence meets the final durable-ack contract. |
...
| A-20 | Partial | Deterministic fixtures and extensive native/platform harnesses exist. Plan 342 adds host, relay, discovery, and physical-Android SQLCipher receipts for its bounded text-storage boundary; Plan 343 adds focused/preservation, relay, curated `1to1`, core/feature family, analyzer/diff, six pinned-Pixel Flutter SQLCipher, and PB266 connected-release receipts for its bounded reaction-custody boundary. Plan 344 adds causal RED/mutation proof, six relay parents, full affected relay/native suites, Redis process handoff, framed mixed-version handlers, race/vet, rollout source contracts, 494 focused Dart tests, `1to1` (2,919), `feed` (323), `core-host-all` (3,170), stable serial `feature-host-all` (8,904), Android binding, analyzer, and changed-file hygiene. Broader platform/presentation and live-activation evidence remain incomplete. | After GAP-N01 through GAP-N11 pass their host/native and Android gates, produce current, provenance-bound real-iPhone APNs evidence in the consolidated GAP-N12 closure phase and complete the availability-bounded platform state matrix. |
...
| A-30 | Partial | Dedupe, claim, CAS, and some direct/FCM order tests exist. Plan 342 adds atomic stage/drain, relay namespace, lifecycle, account-transfer, migration, and real Android SQLCipher reopen/rollback tests for fresh text. Plan 343 adds the corresponding direct-reaction transition, drain/lifecycle, tuple/CAS, stale-UI, cross-type relay, transfer, preservation, six pinned-Pixel Flutter SQLCipher proofs, and PB266 release-build proof. Plan 344 adds non-destructive full/duplicate/conflict/expiry/promotion tests, old/new relay-sender-receiver combinations, Redis process handoff, strict Dart retirement, and old-receiver-to-upgrade convergence; its stable per-plan families are green with the recorded concurrency-2 harness caveat. | Complete production activation/rollback evidence plus direct→push, push→direct, open-during-build, background-during-build, late-wake-after-read, historical-legacy, and crash-after-persistence tests across event kinds and owners. |
...
| AC-10: all path orders converge to one event/unread/card/normal alert | Partial | GAP-N01, GAP-N03, GAP-N05, GAP-N06, GAP-N12 | Preserve Plans 342/343's bounded stage/drain/identity tests and Plan 344's exact duplicate plus old-receiver-to-upgrade convergence, then complete the arrival-order and crash matrix over event kinds, direct, pubsub, inbox, NSE/FCM, WorkManager, and reconciler. |
...
| AC-12: launch after missed pushes synchronizes retained inbox exactly once | Partial | GAP-N01, GAP-N06, GAP-N12 | Plans 342/343 prove bounded sender lifecycle retries, and Plan 344 proves protected receiver replay/ACK plus temporal old-client-to-upgrade convergence for v108/v109, not a full cold-launch notification reconciliation campaign. Add multiple pages/event kinds, local dedupe, per-device ACK, and no duplicate card/unread/tone. |
...
9. Durable event claims and per-conversation tone leases using file locking.
10. Stable conversation notification IDs and exact generation-safe replace/cancel.
11. Direct/group display, read, and reconciliation outboxes.
12. Exact frontmost-conversation suppression in the Flutter live path.
13. iOS App Group atomic file primitives, Apple request custody, badge serialization, and exact delivered-notification retirement.
14. Android dropped-FCM-batch durable marker and WorkManager recovery scaffolding.
15. Group mute UI/model/SQL state, shared display policy, iOS group-context projection, Android encrypted-database enforcement, and exact-card reconciliation. The standalone unused mute Keychain sentinel is excluded unless a consumer is explicitly assigned.
16. Existing deterministic fixtures, platform harnesses, and pinned device orchestration.
...
Current iOS NSE and Android background processing require rich provider fields. Switching the relay to a generic wake before compatible inbox-fetch clients ship would remove notification resolution for older clients. Plans 342 and 343 introduce distinct current text/edit and reaction-event relay dedupe namespaces. Plan 344 adds separate store/retrieve/ACK actions, exact proof, per-relay negotiation, and a machine-checked S0 old binary → S1 new binary/admission off → S2 admission on → S3 capable client sequence for only DB v108/v109 custody. That scoped mechanism does not authorize production admission or solve the later generic-wake migration; future planning must preserve both rollout boundaries and a pinned rollback artifact.
...
Existing tone claims, recent-remote gates, conversation ID registries, iOS recovery JSON, Android recovery state, and SQLCipher outboxes now include Plan 342's DB v108 direct-text custody table and Plan 343's DB v109 reaction custody table. Plan 344 reuses both schemas and changes only the proof required to retire their rows. They may coexist with the new ledger during rollout. Owner precedence, one-time migration, rollback, account-transfer inventory, and garbage-collection rules need explicit design; future schema plans must preserve pending v108/v109 custody rather than silently recreating or omitting it.
...
| WP-01: Authenticated event and durable custody contract | A-01 through A-04, A-17, A-18, A-24, A-26 | Preserve Plans 342 through 344's implemented and per-plan-green fresh-text/direct-reaction/protected-capacity slices, then close media/edit/delete/group/device-fanout so every notification-worthy event has one authenticated identity/hash and per-device inbox custody independent of fast path; carry Plan 344 production activation and legacy-retirement gates into WP-07. |
...
- This refresh is source-, plan-, and repository-evidence-based. It includes retained Plans 342/343 host and Android receipts plus Plan 344 causal/mutation, relay/native/mixed-version, Redis process-handoff, focused Dart, curated `1to1`/`feed`, justified core/feature family, Android-binding, analyzer, changed-file-format, and diff-hygiene evidence. It does not include a production relay deployment, admission activation, actual Redis-server restart/restore, pinned old-binary rollback, live traffic thresholds/provenance, or a full device/presentation campaign.
