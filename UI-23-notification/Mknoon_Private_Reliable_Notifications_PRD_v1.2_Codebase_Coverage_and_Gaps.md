...
- **Repository baseline:** Branch `protected-view`; Plan 343 execution began from checkpoint HEAD `38e6b91058431c67f6b091bbf90b076f4d788c12`
- **Assessment scope:** Current implementation through the bounded Plan 343 direct-reaction sender-custody slice. Focused/preservation, relay, curated `1to1`, core/feature family, full analyzer/diff, six physical-Android Flutter SQLCipher, and PB266 connected-release receipts are included; full rollout/device-campaign evidence remains outside this bounded slice.
...
- fresh ordinary direct text and direct reaction ADD/REMOVE now retain sender-owned exact-envelope custody independently of live ACK, but media, edits/deletes, group ordering, device fanout, and relay-overflow retention still lack the universal PRD contract;
...
Plans 342 and 343 materially narrow GAP-N01 for fresh ordinary direct text and direct reaction ADD/REMOVE, but neither bounded slice fully closes another composite A-control. A-17 remains Partial because generic opaque wake, recipient outcome ownership, relay ACK-or-expiry retention, and universal normalization remain open. The strict count therefore remains 2 Compliant, 19 Partial, 9 Missing, or **38.3%**.
...
- Plan 343 adds DB v109 immutable per-transition direct-reaction custody. ADD and REMOVE fail closed without the explicit repository/detailed-store capabilities, atomically stage the canonical projection or tombstone plus exact encrypted envelope before node/network work, and retain custody across node-stopped, live-success, and failed-store paths (`lib/features/conversation/application/send_reaction_use_case.dart:53-183`; `remove_reaction_use_case.dart:45-174`; `lib/core/database/helpers/direct_reaction_inbox_custody_outbox_db_helpers.dart`). Only typed relay `stored`/`duplicate` acceptance attempts exact local retirement; lifecycle drains share the existing production cadence (`lib/features/conversation/application/deliver_outgoing_direct_reaction_custody.dart:21-114`; `drain_direct_reaction_inbox_custody_outbox_use_case.dart`; `lib/app/bootstrap/production_application_bootstrap.dart:5834`). A generation/sentinel guard prevents a late ADD completion from replacing a newer ADD/REMOVE intent, and the relay now isolates valid v2 ADD/REMOVE transition IDs under `reaction-event-id:` while preserving ADD-only wake eligibility (`lib/features/conversation/presentation/controllers/reaction_optimistic_attempt_guard.dart`; `go-relay-server/inbox.go:1312`).
...
For fresh ordinary direct text and direct reaction ADD/REMOVE, the sender-held persistence-only window is now closed through current typed relay acceptance. Equivalent windows remain for media, edit/delete, group, and device-fanout lanes, and current relay overflow can still discard an older unacknowledged event after a sender has transferred custody. None of this supplies the completed local notification outcome required by GAP-N03, so a recipient can still persist an event and suspend before presentation ownership is established.
...
|---|---|---|---|
| A-01 | Partial | Sender-generated identities are reused and receiver persistence deduplicates them. Plan 342 binds fresh ordinary direct text to one immutable envelope/outbox incarnation and gives current edits matching authenticated outer/inner `eventId` plus a separate relay namespace. Plan 343 atomically binds each direct-reaction transition ID to its exact envelope, preserves receiver event/action/target parity, and gives reaction events a distinct relay namespace. Group pubsub and inbox reuse one envelope: `go-mknoon/node/pubsub.go:388-453`. | Extend one authenticated `event_id` and content hash contract across every direct/group event, recipient device, relay entry, replay, and notification ledger; the bounded text/reaction slices do not supply universal identity/custody. |
| A-02 | Partial | Relay inbox paths commit before push (`go-relay-server/inbox.go`), Plan 342 makes fresh ordinary direct-text sender custody independent of live ACK through DB v108, and Plan 343 does the same for direct-reaction ADD/REMOVE through DB v109. | Extend sender/inbox custody to media, edit/delete, group, and every-device lanes; make relay capacity non-destructive for unacknowledged rows and group durability independent of concurrent pubsub. |
| A-03 | Partial | Non-destructive pending retrieval and explicit stable-entry ACK exist (`go-relay-server/inbox_store.go`, `go-relay-server/inbox.go`); Plans 342/343 stage exact text/reaction custody before transport and retire a sender row only on typed relay acceptance with an exact compare-and-delete. | Retire the legacy destructive retrieve API, eliminate oldest-unacknowledged relay eviction, extend exact sender custody beyond bounded text/reactions, and decide whether encrypted staging before successful decrypt/canonical persistence meets the final durable-ack contract. |
...
| A-17 | Partial | Reactions have stable event identity, receiver parity checks, LWW/tombstone dedupe, display outboxes, and local rendering. Plan 343 now atomically commits direct ADD/REMOVE canonical state plus an exact encrypted DB v109 obligation before transport, retries on the shared lifecycle cadence, isolates reaction transition IDs in relay dedupe, and generation-fences optimistic completion. Focused host/relay suites and one physical-Android SQLCipher proof are green. | Remove specialized rich reaction push lanes and normalize reactions into the generic-wake/inbox/ledger/decision pipeline without losing ordering or silent REMOVE behavior; close relay ACK-or-expiry overflow and recipient outcome/presentation proof before raising this control above Partial. |
| A-18 | Partial | Direct inbox addressing and ACK are per transport/device peer; Plans 342/343's durable text/reaction slices still target only the current peer. Group tests explicitly keep unread and notifications device-local across siblings. | Specify and prove independent per-device event fanout, inbox cursors, and ACKs while preserving the resolved rule that a sibling read does not clear this device. |
...
| A-26 | **Missing** | No outcome ACK exists. Plans 342/343 prevent live delivery from cancelling fresh ordinary text or direct-reaction sender custody, but neither outbox nor any current protocol proves a completed local notification outcome, and the rule is not universal across event lanes. | Implement bounded debounce plus opaque `wake_not_required` only after a completed approved outcome. |
...
| A-30 | Partial | Dedupe, claim, CAS, and some direct/FCM order tests exist. Plan 342 adds atomic stage/drain, relay namespace, lifecycle, account-transfer, migration, and real Android SQLCipher reopen/rollback tests for fresh text. Plan 343 adds the corresponding direct-reaction transition, drain/lifecycle, tuple/CAS, stale-UI, cross-type relay, transfer, preservation, six pinned-Pixel Flutter SQLCipher proofs, and PB266 release-build proof; its focused, curated, and family closure evidence is green. | Complete direct→push, push→direct, open-during-build, background-during-build, late-wake-after-read, relay-overflow, and crash-after-persistence tests across event kinds and owners. |
...
- This refresh is source-, plan-, and repository-evidence-based. It includes Plan 343 focused/preservation and relay execution, curated `1to1`, justified core/feature family gates, full analyzer/diff hygiene, six pinned-Android Flutter SQLCipher proofs, and the PB266 connected-release proof, but not a full rollout/device campaign.
- Plan 343 execution began from checkpoint HEAD `38e6b91058431c67f6b091bbf90b076f4d788c12`; this refresh assesses the implementation source and tests carried with the plan rather than treating the planning checkpoint as the current-code boundary.
...
- Plan 343's DB v109, reaction custody, UI-generation guard, relay namespace, account-transfer inventory, per-plan host closure, and Android SQLCipher/release-build boundaries are implementation evidence. They strengthen existing Partial controls but do not change the strict **38.3%** score because relay ACK-or-expiry retention, generic wake/outcome ownership, remaining event/device lanes, and rollout-level closure evidence are still open.
