...
- **Implementation/evidence refresh:** 9 August 2026; this carries the assessment through Plan 351 — adding Plan 348's default-off external-share adopter, Plan 349's ordinary direct-text mutation custody, Plan 350's internal media-forward adopter, and Plan 351's atomic ordinary direct-media delete-for-everyone custody, plus their host closure evidence — without changing the strict score or claiming production activation
...
- **Plan 351 scope caveat (carry forward):** the plan's premise that a nonblank `direct_media_blob_custody_fingerprint` is the strict-media discriminator holds only on the RECEIVER side — that column is written solely by the strict incoming stage, never for outgoing attachments. Strict-media selection therefore rests on v111/v108 authority for the bound-live, unbound-active, and cleanup-pending matrix rows; the persisted fingerprint is used only as the tiebreak when v111 is fully absent, and as the crossed-proof guard that refuses a well-formed digest belonging to another commitment. The practical consequence is bounded and deliberate: a strict parent whose v111 generation has fully drained after v108 completion takes the unchanged legacy deletion lane and mints no v109 event, exactly as the plan's historical-compatibility row specifies. Caption EDIT for media and crash-complete local artifact erasure remain explicitly out of scope.
