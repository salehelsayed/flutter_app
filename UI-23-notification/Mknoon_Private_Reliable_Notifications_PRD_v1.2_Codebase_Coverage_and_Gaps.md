...
- **Implementation/evidence refresh:** 10 August 2026; this carries the assessment through Plan 353's implemented ordinary-direct media caption custody and Plan 354's default-off Protected/View-Once implementation, including the later behavioral audit that invalidated Plan 354's code-closure claim. Plan 355 is the reviewed bounded repair plan; it is not implemented. The strict score remains unchanged and no production activation is claimed.
- **Repository baseline:** Branch `protected-view` at `6f66a8098` (`docs: mark Plan 354 complete`). Plan 352 is planned in `22a577b3d` and implemented in `ac6eba539`; Plan 353 is planned in `e98afc436` and implemented in `7fa8d1a9c`; Plan 354's sender/receiver implementation includes `59cb33264` and `6380db907`, with historical test/closure receipts through `6f66a8098`. The append-only status and Plan-354 addendum now record the post-execution defects rather than treating those receipts as sufficient closure.
- **Assessment scope:** Implemented default-off custody through Plan 353 is in scope. Plan 354's sender preparation/envelope custody and receiver atomic stage exist, but explicit strict-private local commit, strict-vs-legacy download selection, terminal replay routing, and terminal presentation convergence remain post-execution-review incomplete until Plan 355 executes. Disappearing media, private mutations, group/announcement lanes, historical/no-intent rows, linked-device fanout, crash-complete general artifact erasure, production activation/operations/cohort selection, aggregate local quota/UX, wave-level full `host-all`, consolidated iOS, and release closure remain outside this slice.
...
Plan 354 implements the default-off sender preparation, exact private envelope/v108/v111 binding, restart retry, atomic incoming parent/attachment/v111 stage, and bounded lifecycle retention for v1 Protected image/video and View-Once image. Its recorded 21/21 named tests and host-family receipts remain historical evidence, but a later behavioral audit disproved the original code-closure conclusion: explicit private strict download still cannot complete its ordinary-only/stale-row final CAS, fingerprinted strict rows can escape into legacy handling or unsafe ACK-pending convergence, terminal replay can enter generic duplicate/display retry, and post-stage thumbnail/marker/publication work can race terminal state. Plan 355 is the reviewed, not-yet-implemented closure repair over the same v111/private-lifecycle/display owners. It adds no schema, protocol, owner, scheduler, scanner, native/device work, activation, composite-control closure, or score movement. Until Plan 355 executes and its behavioral gates pass, Plan 354 is implemented but post-execution-review incomplete and Protected/View-Once is not host-code-closed.
...
- Plan 354 implements the bounded Protected/View-Once initial sender and atomic receive stage over the existing private lifecycle plus v108/v111, but post-execution review found its explicit strict-download and terminal presentation paths are not behaviorally closed. Plan 355 is now the reviewed repair plan: it pins the strict/no-legacy download state table, exact private local-path+ACK transaction, reversible LAN recovery, terminal replay identity/routing, one incumbent exclusive lifecycle span, exact message-marker retirement, and terminal notification eligibility. Plan 355 is not implemented, so the private-initial lane remains open for closure.
