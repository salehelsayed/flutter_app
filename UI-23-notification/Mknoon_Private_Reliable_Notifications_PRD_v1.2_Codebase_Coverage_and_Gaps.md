...
- **Implementation/evidence refresh:** 9 August 2026; this carries the assessment through Plan 350 — adding Plan 348's default-off external-share adopter, Plan 349's ordinary direct-text mutation custody, and Plan 350's internal media-forward adopter, plus their host closure evidence — without changing the strict score or claiming production activation
- **Repository baseline:** Branch `protected-view`; Plans 347-350 are each committed in isolation — `9d344e3867e8ee253a393fcadb43f1ef9a76b160` (`feat: adopt direct media blob custody`), `d06072d80` (`feat: add external share media blob custody`), `f11a732568aa0c72c8d4ed8bfecd5b1348adf130` (`feat: add direct text mutation custody`), and `10ee18fc09ca3a305e7c1e5bd18822ce191329ec` (`feat: adopt internal media-forward blob custody`). The follow-up `2d81569d4` (`fix: run android manifest contracts through the host bridge`) repairs a container-only gate-harness gap and changes no application code.
- **Assessment scope:** Current implementation through Plan 350's default-off adoption of the protected media-blob lane for already-authorized internal ordinary media forwards to direct contacts, in addition to Plan 348's marker-free external-OS media, Plan 349's ordinary direct-text edit/delete custody, and Plan 347's prepared composer/voice and exact reopen-only lanes. Retained Plans 342-349 evidence plus Plan 348's same-schema fresh stage, production capability binding, authority-aware share flow, source-free restart, focused/preservation/curated/feature/core host gates, analyzer, hygiene, and Graphify evidence are in scope. Private/disappearing media, edit/delete, group/announcement, historical/no-intent rows, per-device fanout, production activation/operations/cohort selection, aggregate local quota/UX, wave-level full `host-all`, consolidated iOS, and release closure remain outside this slice.
...
- Production activation of the default-off external-share adopter, direct private/disappearing media, edit/delete custody, group/announcement media, every-device fanout, historical/no-intent media, and the group inbox-first/truthful-order contract remain open. Group pubsub and inbox work still launch concurrently rather than establishing inbox custody first: `go-mknoon/node/pubsub.go:367-453`.
...
| A-24 | Partial | If Flutter is still executing with a correct lifecycle value, background state leads to local notification. Plans 342/343 remove the sender persistence-only window for fresh ordinary direct text and direct reactions; Plan 345 extends that sender-owned initial-envelope protection to fresh ordinary direct media/voice. Plan 344 ensures only exact protected ACK-or-expiry relay acceptance can retire those rows. Plans 347/348 adopt Plan 346's bounded blob primitive for prepared ordinary-direct composer/voice, exact reopen retry, and marker-free external-OS media -> ordinary-direct sends, including exact ciphertext and envelope/blob expiry coupling. No completed recipient presentation outcome exists, all three controls remain off by default, and excluded lanes remain reachability-dependent. | Activate/evidence protected custody with the required cohort/operations/quota UX, extend it across the remaining GAP-N01 lanes, then require confirmed local/provider presentation ownership before wake suppression. |
...
| WP-01: Authenticated event and durable custody contract | A-01 through A-04, A-17, A-18, A-24, A-26 | Preserve Plans 342 through 350's implemented fresh-text, fresh ordinary-media/voice initial-envelope, direct-reaction, protected-inbox-capacity, relay/native media-blob primitive, bounded prepared composer/voice adopter, marker-free external-OS media -> ordinary-direct adopter, ordinary direct-text edit/delete custody, and internal media-forward -> ordinary-direct adopter slices. Plans 347/348/350 provide durable exact-ciphertext reuse, strict upload/download/source-pinned ACK, persisted relay proof, v108 manifest/expiry coupling, and same-schema fresh absent-parent authority — across exactly two canonical parent shapes — for complete DB-v111 generations and their reopen-only retries. Next close private/disappearing media, ordinary-media edit/delete, group/announcement, historical/no-intent, and device-fanout custody so every notification-worthy event has one authenticated identity/hash and per-device inbox custody independent of fast path. Carry all three default-off activation controls, operations/cohort selection, aggregate quota/UX, wave-level full `host-all`, consolidated iOS, legacy retirement, and release evidence into WP-07. |
