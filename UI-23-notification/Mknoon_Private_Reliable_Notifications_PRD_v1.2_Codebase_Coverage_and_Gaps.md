...
- **Implementation/evidence refresh:** 10 August 2026; this carries the assessment through Plan 353's implemented ordinary-direct media caption custody and Plan 354's default-off Protected/View-Once implementation, including the later behavioral audit that invalidated Plan 354's code-closure claim and Plan 355's implemented bounded repair that supplies the missing behavioral proof. The strict score remains unchanged and no production activation is claimed.
- **Repository baseline:** Branch `protected-view` at `6f66a8098` (`docs: mark Plan 354 complete`). Plan 352 is planned in `22a577b3d` and implemented in `ac6eba539`; Plan 353 is planned in `e98afc436` and implemented in `7fa8d1a9c`; Plan 354's sender/receiver implementation includes `59cb33264` and `6380db907`, with historical test/closure receipts through `6f66a8098`. The append-only status and Plan-354 addendum record the post-execution defects rather than treating those receipts as sufficient closure; Plan 355 is planned in `ac557beee` and implemented across `c48c4a9c2`, `ab00a1d80` and `65cb399bd`.
- **Assessment scope:** Implemented default-off custody through Plan 353 is in scope. Plan 354's sender preparation/envelope custody and receiver atomic stage exist, and Plan 355 repaired and behaviorally proved explicit strict-private local commit, strict-vs-legacy download selection, terminal replay routing, and terminal presentation convergence over real SQLite and real files. Disappearing media, private mutations, group/announcement lanes, historical/no-intent rows, linked-device fanout, crash-complete general artifact erasure, production activation/operations/cohort selection, aggregate local quota/UX, wave-level full `host-all`, consolidated iOS, and release closure remain outside this slice.
