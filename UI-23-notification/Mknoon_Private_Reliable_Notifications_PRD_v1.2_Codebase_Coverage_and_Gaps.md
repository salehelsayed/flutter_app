...
- **Repository baseline:** Branch `protected-view`; Plan 345 implementation commit `d46be5712bb212b339028c4ae194b638f3e61d65`, plus the bounded Plan 346 implementation in this assessment update (Plan 346 execution began from dirty Plan 345 work at `976c660ff`)
- **Assessment scope:** Current implementation through Plan 346's bounded default-off relay/native media-blob custody primitive. Retained Plans 342-345 evidence plus Plan 346's relay/native/process, curated aggregate `1to1`, justified core, Android binding, analyzer, hygiene, and Graphify receipts are recorded; exact-ciphertext client reuse, persisted relay proof, v108 blob commitment/expiry coupling, private/disappearing media, edit/delete/group/device fanout, production admission, and full rollout/device-campaign evidence remain outside this slice.
...
- fresh ordinary direct text, fresh ordinary direct media/voice initial chat envelopes, and direct reaction ADD/REMOVE retain sender-owned exact-envelope custody independently of live ACK and use the default-off protected Redis lane's non-destructive capacity and exact ACK-or-expiry proof; Plan 346 adds a separate default-off protected relay-blob primitive, but no production feature adopts it or couples exact ciphertext and expiry to the envelope, so private/disappearing media, edits/deletes, group ordering, device fanout, production activation, historical legacy rows, and uploaded media blobs still lack the universal PRD contract;
...
Plans 342 through 346 materially narrow GAP-N01 for fresh ordinary direct text, fresh ordinary direct media/voice initial chat envelopes, direct reaction ADD/REMOVE, and the relay/native media-blob durability primitive. Plan 344 implements scoped non-destructive relay capacity, exact ACK-or-expiry proof, and mixed-version receive/ACK behavior for DB v108/v109 obligations; Plan 345 adds DB v110's local manifest-bound preparation token and atomically commits complete ordinary media descriptors plus their exact encrypted initial envelope into the existing v108 obligation. Plan 346 adds a separate default-off filesystem custody lane with exact encrypted-byte proof, non-destructive capacity, restart-safe commit recovery, strict download, and exact ACK/expiry, but no feature caller adopts it. No bounded slice fully closes another composite A-control. A-17 remains Partial because generic opaque wake, recipient outcome ownership, universal normalization, exact-ciphertext client reuse, blob/envelope expiry coupling, remaining event/device lanes, and production activation remain open. The strict count therefore remains 2 Compliant, 19 Partial, 9 Missing, or **38.3%**.
...
- Plan 346 adds an isolated `.custody-v1` relay filesystem lane on the existing media protocol. Strict state is keyed by the exact recipient/blob/hash/size/MIME tuple, accepts an exact pending duplicate before capacity, rejects distinct full writes without eviction, commits with a sidecar-last durability marker, reconciles or fails closed on restart, serves only exact protected bytes, and retires through an exact source-pinned ACK tombstone or expiry (`go-relay-server/media_custody.go`; `go-mknoon/node/media.go`; `go-mknoon/bridge/bridge.go`; `lib/core/bridge/p2p_bridge_client.dart`). Custody-specific relay/native/bridge/Dart/process, full affected packages, justified core, Android binding, hygiene, and dedicated gate tests are green; the final aggregate `1to1` passed 2,968 Flutter tests and every registered non-Flutter tail. Admission defaults off and the no-adopter contract excludes conversation feature callers. Execution was user-directed from active dirty Plan 345 work at HEAD `976c660ff`, so no clean post-345 baseline or raw first-RED receipt exists.
- Protected-store admission is deliberately default-off through `DIRECT_INBOX_ACK_CUSTODY_ADMISSION_ENABLED` and fails boot if enabled without durable Redis; protected retrieval, ACK, expiry, and metrics remain available while admission is off (`go-relay-server/server_bootstrap.go:19-116`). Production S2/S3 activation still requires every relay front-end to share the same durable Redis and contract revision plus real persistence/restart, rollback-artifact, threshold, and provenance evidence. There is no historical backfill: old clients and legacy-only rows retain legacy eviction behavior until fleet retirement.
- Direct private/disappearing media, edit/delete custody, every-device fanout, historical/no-token media, production adoption of protected relay-blob retention, exact ciphertext reuse, blob/envelope expiry coupling, and the group inbox-first/truthful-order contract remain open. Group pubsub and inbox work still launch concurrently rather than establishing inbox custody first: `go-mknoon/node/pubsub.go:367-453`.
...
For capable DB-v110 senders, v108 now owns fresh ordinary text and ordinary media/voice initial envelopes while v109 owns direct reaction transitions. With an admission-enabled Plan 344 relay, the sender-held persistence-only window closes only on exact protected acceptance, and protected capacity cannot discard an older unacknowledged obligation to accept a new one. Plan 346 removes the analogous destructive-capacity/restart defect inside its bounded media-blob primitive, but the default deployment state remains admission-off and no feature sender preserves and reuses the exact ciphertext or couples blob lifetime to the envelope. Old clients, historical/no-token rows, private/disappearing media, and edit/delete/group/device-fanout lanes therefore retain their existing risk, and Plan 345 media sends do not yet gain an end-to-end retrievability guarantee. None of this supplies the completed local notification outcome required by GAP-N03, so a recipient can still persist an event and suspend before presentation ownership is established.
...
- Activate Plan 344's protected contract only after the S2 operational gates prove same-Redis/same-revision front ends, real persistence/restart behavior, a pinned rollback artifact, thresholds, and provenance; retain read/ACK continuity under the kill switch, then retire the legacy eviction path only after old-client traffic drains.
- Extend the protected ACK-or-expiry contract beyond bounded v108 text/ordinary-media and v109 reaction obligations to every required event kind, uploaded blob where required, and independently addressed recipient device; the additive protected lane is a bounded mechanism, not universal inbox custody.
...
- Plans 342/343 keep fresh ordinary direct text and direct-reaction ADD/REMOVE in sender-owned custody despite direct/authenticated delivery; Plan 345 extends that initial-envelope rule to fresh ordinary direct media/voice, and Plan 344 permits retirement only after exact protected ACK-or-expiry relay acceptance. Plan 346 supplies a default-off exact ACK-or-expiry relay-blob primitive, but no feature caller adopts it or couples its expiry to the envelope. None records a completed recipient-side policy/presentation outcome; remaining GAP-N01 lanes and production media uploads retain their older reachability-dependent custody behavior.
...
| A-24 | Partial | If Flutter is still executing with a correct lifecycle value, background state leads to local notification. Plans 342/343 remove the sender persistence-only window for fresh ordinary direct text and direct reactions; Plan 345 extends that sender-owned initial-envelope protection to fresh ordinary direct media/voice. Plan 344 ensures only exact protected ACK-or-expiry relay acceptance can retire those rows. Plan 346 provides the corresponding bounded blob primitive, but it is default-off and unadopted. No completed recipient presentation outcome exists, production admission is still off by default, and other lanes plus current production blob uploads remain reachability-dependent. | Activate/evidence protected custody, adopt exact ciphertext reuse and envelope/blob expiry coupling, extend custody across the remaining GAP-N01 lanes, then require confirmed local/provider presentation ownership before wake suppression. |
...
10. Plan 346's isolated media-blob filesystem custody lane, exact encrypted-byte tuple/proof, duplicate-before-cap and non-destructive-full behavior, sidecar-last recovery, strict protected download, exact tombstone ACK/expiry, phase-aware single-relay selection, and source-pinned ACK. Preserve the default-off/no-adopter boundary until exact ciphertext reuse and blob/envelope TTL coupling are implemented.
...
Plan 346 supplies the analogous bounded media-blob primitive in an isolated filesystem lane. It binds custody to the exact encrypted bytes, refuses destructive capacity eviction, recovers committed state across a sequential process reopen, serves strict downloads only with exact proof, and retains an exact ACK tombstone through original expiry. Legacy media upload/list/download/delete behavior remains separate. This is still an enabling primitive rather than end-to-end media custody: strict admission is default-off, no feature caller adopts it, Plan 345 does not durably preserve/reuse the same ciphertext artifact, and blob acceptance/expiry is not coupled or preflighted with the v108 envelope. Plan 347 owns that ordinary-direct client adoption boundary. GAP-N01 and the media-blob work item therefore remain open and release-ineligible.
...
| WP-01: Authenticated event and durable custody contract | A-01 through A-04, A-17, A-18, A-24, A-26 | Preserve Plans 342 through 346's implemented fresh-text, fresh ordinary-media/voice initial-envelope, direct-reaction, protected-inbox-capacity, and relay/native media-blob primitive slices. Plan 347 must adopt Plan 346 from the ordinary-direct production sender with durable exact-ciphertext reuse, strict upload/download/source-pinned ACK, persisted relay proof, and v108 blob commitment plus blob/envelope expiry coupling or preflight before custody transfer. Then close private/disappearing media, edit/delete, group, historical, and device-fanout custody so every notification-worthy event has one authenticated identity/hash and per-device inbox custody independent of fast path. Carry Plan 344/346 production activation and legacy-retirement gates into WP-07. |
...
- Plan 346 closes the destructive-capacity/restart/strict-ACK defect only inside a default-off relay/native media-blob primitive. No feature caller adopts it; exact ciphertext preservation/reuse, persisted relay proof, and blob/envelope expiry coupling remain open under Plan 347, so neither the media-blob work item nor GAP-N01 is closed or release-eligible. Execution was user-directed from active dirty Plan 345 work at HEAD `976c660ff`, without a clean post-345 baseline or raw first-RED receipt. Its custody-specific gates, full affected relay/native packages, `core-host-all` (402 paths / 3,190 tests), Android bindings, analyzer/hygiene, and final aggregate `1to1` (2,968 Flutter tests plus all non-Flutter tails) are green. This strengthens the same Partial controls without changing the strict **38.3%** score.
