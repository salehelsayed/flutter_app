...
- **Repository baseline:** Branch `protected-view`; Plan 344 implementation commit `185ba42027440a49bd84311038a70ab10435914c`; Plan 345 execution baseline `976c660ff3eb7bd928f43309769dc72ae508f1e3`, plus the bounded Plan 345 implementation in this assessment update
- **Assessment scope:** Current implementation through Plan 345's bounded fresh ordinary direct media/voice initial-envelope custody slice. Relay-blob durability, private/disappearing media, edit/delete/group/device fanout, production admission, and full rollout/device-campaign evidence remain outside this slice.
...
- fresh ordinary direct text, fresh ordinary direct media/voice initial chat envelopes, and direct reaction ADD/REMOVE retain sender-owned exact-envelope custody independently of live ACK and use the default-off protected Redis lane's non-destructive capacity and exact ACK-or-expiry proof; private/disappearing media, edits/deletes, group ordering, device fanout, production activation, historical legacy rows, and uploaded media blobs still lack the universal PRD contract;
...
Plans 342 through 345 materially narrow GAP-N01 for fresh ordinary direct text, fresh ordinary direct media/voice initial chat envelopes, and direct reaction ADD/REMOVE. Plan 344 implements scoped non-destructive relay capacity, exact ACK-or-expiry proof, and mixed-version receive/ACK behavior for DB v108/v109 obligations; Plan 345 adds DB v110's local manifest-bound preparation token and atomically commits complete ordinary media descriptors plus their exact encrypted initial envelope into the existing v108 obligation. No bounded slice fully closes another composite A-control. A-17 remains Partial because generic opaque wake, recipient outcome ownership, universal normalization, remaining event/device lanes, blob durability, and production activation remain open. The strict count therefore remains 2 Compliant, 19 Partial, 9 Missing, or **38.3%**.
...
- Plans 342/343 keep fresh ordinary direct text and direct-reaction ADD/REMOVE in sender-owned custody despite direct/authenticated delivery; Plan 345 extends that initial-envelope rule to fresh ordinary direct media/voice, and Plan 344 permits retirement only after exact protected ACK-or-expiry relay acceptance. None records a completed recipient-side policy/presentation outcome; remaining GAP-N01 lanes and uploaded blobs retain their older reachability-dependent custody behavior.
...
| A-24 | Partial | If Flutter is still executing with a correct lifecycle value, background state leads to local notification. Plans 342/343 remove the sender persistence-only window for fresh ordinary direct text and direct reactions; Plan 345 extends that sender-owned initial-envelope protection to fresh ordinary direct media/voice. Plan 344 ensures only exact protected ACK-or-expiry relay acceptance can retire those rows. No completed recipient presentation outcome exists, production admission is still off by default, and other lanes/blobs remain reachability-dependent. | Activate/evidence protected custody, extend it across the remaining GAP-N01 lanes, then require confirmed local/provider presentation ownership before wake suppression. |
...
10. Durable event claims and per-conversation tone leases using file locking.
11. Stable conversation notification IDs and exact generation-safe replace/cancel.
12. Direct/group display, read, and reconciliation outboxes.
13. Exact frontmost-conversation suppression in the Flutter live path.
14. iOS App Group atomic file primitives, Apple request custody, badge serialization, and exact delivered-notification retirement.
15. Android dropped-FCM-batch durable marker and WorkManager recovery scaffolding.
16. Group mute UI/model/SQL state, shared display policy, iOS group-context projection, Android encrypted-database enforcement, and exact-card reconciliation. The standalone unused mute Keychain sentinel is excluded unless a consumer is explicitly assigned.
17. Existing deterministic fixtures, platform harnesses, and pinned device orchestration.
...
### 8.9 Protected ACK-or-expiry capacity exists beside the legacy eviction contract

Legacy memory and Redis `store` paths still evict the oldest unacknowledged legacy row at capacity and report `stored`; Plan 344 intentionally freezes that behavior for installed clients. The new Redis-only protected lane instead accepts an exact duplicate before checking capacity and rejects a distinct full-capacity write without deleting protected authority or its legacy shadows. Capable DB-v110 clients use v108 for bounded text/ordinary-media initial events and v109 for reactions; those owners retire only on exact `stored|duplicate` plus `ack_or_expiry_v1`, so a legacy `OK` cannot transfer custody. This closes the scoped code defect, not the rollout: admission defaults off, pre-activation/old-client rows are not backfilled, production Redis/rollback receipts are absent, media blobs are independent, and other event/device lanes do not use the protected contract.
...
| WP-01: Authenticated event and durable custody contract | A-01 through A-04, A-17, A-18, A-24, A-26 | Preserve Plans 342 through 345's implemented fresh-text, fresh ordinary-media/voice initial-envelope, direct-reaction, and protected-capacity slices; then close media-blob, private/disappearing media, edit/delete, group, historical, and device-fanout custody so every notification-worthy event has one authenticated identity/hash and per-device inbox custody independent of fast path. Carry Plan 344 production activation and legacy-retirement gates into WP-07. |
