...
- **Reading the statuses:** "HOST-STRICT" below means the disposition is now *enforced by a machine contract* (`scripts/test/notification_sound_disposition_contract_test.sh`) and the device scenario exists and is registered, but no device run has yet produced its evidence — the G9 harness blocker is fixed (§4.11); the campaign run is pending.
...
**The structural point (updated):** host proves the *decision*, and the disposition contract now makes the device tier assert it. The device tier has not yet shown anything for the sound smoke — the harness-start blocker (G9) is fixed as of 2026-08-17 (§4.11), and the first full S1–S16 campaign run is the remaining step.

---

## 4. Gap register — what no e2e verifies on the device pair

Read §4.1 and §4.10 together: Plan 378 closed six gaps *as work*. The harness-start blocker **G9** is FIXED (2026-08-17, see §4.11) — both roles now complete device setup — but the full S1–S16 campaign has not yet been executed, so nothing in §4.1 is claimed as device-proven yet.
...
**Verified before filing here** (4 evidence strands + 3 adversarial lenses, 2026-08-18). One lens refuted a wholesale move: G17's statement was written wider than what was fixed, so the mechanism closure is recorded below and the three sub-claims that did NOT close were carved out to **G21** (first-wake storage deadline) and **G22** (1:1 device evidence), which stay in §4.10, with **G19** covering the killed-path group-text hole — G19 itself is now CLOSED by Plan 384 (§4.6). Read this row together with those three.
...
in §4.10, found by this plan's producer census and explicitly out of its scope.
...
was simply not being run. This is the same class as **G13** (§4.11): a stale pin inside `sims-contracts`
...
not fix. That red is now **G28** (§4.10): it is a behavior decision, not gate hygiene.
...
plan owns**, by the cold-wake storage deferral — see **G21** in §4.10, whose row now records the measurement.
...
first wake losing its alert — is unchanged and stays in §4.10; the plan's own hard rule is *"do not add
...
are still open, and both plans' rules say Wave 3 waits — its first scenario,
`android_announcement_reaction_recipient`, is blocked by G21 itself. One plan-fix for whoever resumes it:
...
### 4.11 Defects found and fixed during Plan 378 execution (recorded so they are not re-derived)
