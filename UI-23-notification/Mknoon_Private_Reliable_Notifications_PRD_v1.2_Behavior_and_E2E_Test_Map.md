...
- 1:1 reaction device proof (`run_1to1_reaction_notification_device.dart`, capture-owned artifacts).
  **`android_typed_reaction_smoke` added and device-proven 2026-08-20**
  ...
  only in §4.12, not in §4.13. Two caveats remain: the card came from the non-durable fallback, not
  the durable arm, and the lane is runner-validated rather than sims-registered, so its coverage
  recurs only when someone runs it. **A second scenario
...
**Verified before filing here** (4 evidence strands + 3 adversarial lenses, 2026-08-18). One lens refuted a wholesale move: G17's statement was written wider than what was fixed, so the mechanism closure is recorded below and the three sub-claims that did NOT close at that time were carved out to **G21** (first-wake storage deadline), **G22** (1:1 device evidence), and **G19** (the killed-path group-text hole). G19 is now CLOSED by Plan 384 (§4.6), and the 1:1 portion of G22 is CLOSED by Plan 391 (§4.12); only the renamed media-only **G22b** residual remains in §4.13. Read this row together with those closures and G21.
...
| **G22b — killed-path group/announcement MEDIA only** | **This is the only G22 obligation still open.** The 1:1 message leg is closed by Plan 380; the 1:1 reaction leg is closed and freshly revalidated by [Plan 391](../Test-Flight-Improv/391-killed-path-1to1-reaction-device-proof-tdd-plan.md) in §4.12; the non-media announcement variant is refuted there and must not be re-filed. What remains is a real-device observation that killed recipients receive the intended group/announcement media notification path, including which relay branch handled an oversized media envelope. Host green is not sufficient because the host rows stub the resolver that runs on device. | No device observation of group/announcement media exists. The `android_typed_reaction_smoke.json` artifact at `5f4df8c0a` is closure evidence for direct reaction only and is deliberately not cited as media evidence. | **Unowned, and costed.** It needs an emulator-role media send that does not exist (`lib/debug/group_notification_projection_e2e_action.dart:80-82` rejects every role but `physical_author`, and the physical is the device that lane kills), a `_sendGroupMedia` driver that does not exist, and new lifecycle-stage and validator-kind enum members in Plan 389's `group_reaction_notification_device_criteria.dart`. Its payoff is contingent: `go-relay-server/inbox.go:882-896` degrades an oversized group-media envelope to routing-only `preview_unavailable: "1"`, so the leg must assert which relay branch it took. |
