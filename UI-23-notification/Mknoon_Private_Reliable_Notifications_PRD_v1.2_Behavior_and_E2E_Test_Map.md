...
- **Plan-393 Android device-wave closure:** [Plan 393](../Test-Flight-Improv/393-android-notification-lifecycle-killed-path-and-recovery-closure-tdd-plan.md) is **EXECUTION-COMPLETE 2026-08-21** at required Android device + host tier. It closed its assigned N11, G21, G22b, G27, G30, and controlled-cohort fixed-wake/recovery scope; §4.14 preserves that dated evidence and §4.15 retains only Apple/release/OEM/optional exclusions.
- **Plan-394 bounded post-audit host closure:** [Plan 394](../Test-Flight-Improv/394-android-notification-direct-lifecycle-and-final-barrier-closure-tdd-plan.md) is **EXECUTION-COMPLETE / HOST-CLOSED 2026-08-21**. It closes two later current-source audit findings without changing the Plan-393 device boundary: direct and linked-group null initial lifecycle is non-resumed authority, and direct nondurable message/reaction publication has non-vacuous final canonical/visibility denial plus exact event/tone rollback proof. TC-394-01 through TC-394-05, the focused owners, affected `1to1`/`groups` lanes, Plan-393 census, and scoped hygiene are green. The composite host receipt covers 14,602 Flutter tests, 11 intentional skips, and all 16 registered Go/relay/native tails: the official concurrency-2 command completed the Flutter batch plus the first 13 tails, then exact #1385–#1387 passed against identical source after restoring unchanged ignored wrapper/Pods prerequisites. No device campaign was added or repeated.
...
| §6 row 8 / AC-08 | Already read/handled: no re-alert; cancel/update stale | HOST Plans 393–394 require explicit `resumed` plus the exact non-null tracked direct, main-group, or linked-group conversation; activation cleanup remains independent and generation-safe; the final barrier rereads canonical read/delete authority. DEVICE Plan 393 a6 replay-before-ack, read-zero exact cancel, and controlled killed-recovery order remain retained. | HOST + DEVICE (bounded Android); PARTIAL Apple/release |
...
| AC-06 / OQ-02 | Opening A during build prevents stale A without clearing B | HOST Plans 372–394 serialize final publication, reread exact canonical/visibility authority immediately before native entry, deny stale A with zero card, and release exact owners while generation-safe activation cleanup preserves B and a newer A. | HOST (bounded Android mechanism); PARTIAL Apple/release |
| AC-07 | Backgrounding between decision and post → final recheck still notifies | HOST Plans 372–394 prove the inverse barrier: background, unknown, or visibility-reader error remains notification-eligible rather than failing closed. Plan 393 retains the Android background/killed recovery-order device proof. | HOST + DEVICE (bounded Android); PARTIAL Apple/release |
...
| Open A while A's notification is being built | HOST Plans 372–394: final canonical/visibility denial prevents the stale native effect, exact event/tone ownership rolls back, and activation cleanup preserves B/newer A | HOST (bounded Android mechanism); Apple/release partial |
| Background between decision and post | HOST Plans 372–394 inverse barrier: non-resumed, unknown, and visibility-error state remains notification-eligible; Plan 393 retains Android background/killed recovery proof | HOST + DEVICE (bounded Android); Apple/release partial |
...
| Late push after read → no recreation | HOST Plans 372–394 final canonical read/delete reread plus strict direct/main-group/linked-group resumed-and-exact read predicate; DEVICE a6 + read-zero exact cancel and Plan-393 recovery order | HOST + DEVICE (bounded Android); Apple/release partial |
...
| Final gate (open A or background between decision and post) | HOST Plans 372–394: direct/group final canonical/visibility barriers deny stale publication, preserve background/unknown eligibility, and release exact owners; Plan-393 Android recovery/device proof retained. Apple/platform-release evidence remains GAP-N12/WP-07. |
| Acknowledgement separation | TARGET (GAP-N03) |
| Shared-ledger concurrency | HOST (Plan 372 owners); native adoption ongoing (N07/N08) |
| Read/cancel (dismiss, open, mark read, delayed wake) | HOST Plans 393–394 for direct/main-group/linked-group explicit resumed-plus-exact reads, independent generation-safe activation cleanup, and final canonical reread; DEVICE Plan-393 Android projection/recovery proof retained. Apple/release remains PARTIAL. |
...
Read §4.1, §4.13, §4.14, and §4.14a together: Plan 378 closed six gaps *as work*. The harness-start blocker **G9** is FIXED (2026-08-17, see §4.16), and Plan 392 closed both the destructive state-ownership hole and the positive S1–S16 device acceptance. The §4.1 sound/suppression rows are now device-proven where §4.13 says so; Plan 393 closed its Android device-wave residuals in §4.14, and Plan 394 closes the later bounded current-source audit findings at host tier in §4.14a without recapturing device evidence.
...
### 4.14 Closed by Plan 393 (2026-08-21) — Android device-wave closure
...
**Plan-393 execution-time required Android residual: none.** The later Plan-394
post-audit correction below does not invalidate this dated device-wave result. The fixed-wake proof
is deliberately availability-bounded and filtered, so it does not claim production activation or
release eligibility.

### 4.14a Closed by Plan 394 (2026-08-21) — bounded post-audit host closure

[Plan 394](../Test-Flight-Improv/394-android-notification-direct-lifecycle-and-final-barrier-closure-tdd-plan.md)
closes two current-source findings discovered after the Plan-393 device wave. It changes no DB,
relay, wire, native, FCM, Sims, device, or release surface, and the Plan-392/393 device artifacts
above remain the Android OS/cross-device evidence.

| Item | Implemented by Plan 394 | Host proof |
|---|---|---|
| **Unknown initial lifecycle** | Direct `ConversationWired` now initializes null Flutter lifecycle as non-resumed authority; `LinkedGroupConversationWired` requires literal `resumed` before activation/read. Direct, main-group, and linked-group reads therefore share the same explicit resumed-plus-exact non-null conversation rule, while a later real resume can still activate/read the exact surface. | TC-394-01 causally fails under the former direct null-to-resumed fallback; TC-394-02 preserves the post-mount exact-peer reread; TC-394-03 causally fails under the former linked-group `null || resumed` predicate. Focused owners are green. |
| **Direct nondurable final barrier** | Direct message canonical-read denial and direct reaction exact-visible denial both occur after exact event/tone owners reach `publishing`, produce zero native cards, emit the exact suppression reason, and roll both owners back for exact reacquisition. Unknown canonical or visibility authority continues to fail toward one managed typed notification. | TC-394-04 exercises the message/read, reaction/visible, and visibility-error legs with real Android durable leases and exact tone-sidecar identity; TC-394-05 preserves the incumbent unknown-canonical direct message/reaction controls. Canonical, visibility, event-rollback, tone-rollback, and tone-key mutations each re-red and were restored. |
| **Scope preservation** | Plan-393 group final barriers, lifecycle behavior, device capabilities, and retained evidence are unchanged. No Android/iOS device campaign was rerun because this is a deterministic host-only correction below the native boundary. | Focused direct/group/handler owners, exact preservation sentinels, affected `1to1`/`groups` lanes, the Plan-393 census, and scoped hygiene are green. The composite host receipt covers 14,602 Flutter tests, 11 intentional skips, and all 16 registered tails: the official concurrency-2 command completed the Flutter batch plus 13 tails, and exact #1385–#1387 passed after unchanged ignored prerequisites were restored against identical source. |

**Plan-394 bounded Android host residual: none.** The composite host receipt covers 14,602 Flutter
tests, 11 intentional skips, and all 16 registered tails: the official concurrency-2 command
completed the Flutter batch plus 13 tails, and exact #1385–#1387 passed after unchanged ignored
prerequisites were restored against identical source. Apple/release/cohort/OEM/optional work remains
in the existing owners below.

### 4.15 Still open

No required Android implementation or device-evidence item assigned to Plans 393–394 remains here.
Plan 394 added no device-evidence obligation; its complete-worktree host snapshot supersedes Plan
393's earlier host snapshot for the resulting tree, while Plan 393's dated Android device evidence
remains unchanged.
...
## 5. Maintenance notes
- **Plans 393–394 supersede the historical open-status statements for Android N11/final barriers, G21, G22b, G27, G30, and controlled-cohort fixed wake below.** Keep old measurements as provenance: §4.14 owns the dated Android device wave, §4.14a owns the bounded later host correction, and §4.15 owns the Apple/release/OEM/optional residuals.
