...
- **Reading the statuses:** "HOST-STRICT" below means the disposition is now *enforced by a machine contract* (`scripts/test/notification_sound_disposition_contract_test.sh`) and the device scenario exists and is registered, but no device run has yet produced its evidence — the G9 harness blocker is fixed (§4.14); the campaign run is pending.
...
- 1:1 reaction device proof (`run_1to1_reaction_notification_device.dart`, capture-owned artifacts). **`android_typed_reaction_smoke` added and device-proven 2026-08-20** ([Plan 391](../Test-Flight-Improv/391-killed-path-1to1-reaction-device-proof-tdd-plan.md), §4.12): the recipient is killed on the common path, the reaction is driven by real UI on the sender, and the artifact records a MEASURED `recipientProcessAbsentBeforeReaction` beside the typed card, its tap route and the unread lifecycle. Provider attribution is DEVICE-SIDE — the relay's peer-attributed push line no longer exists. Two caveats: the card came from the non-durable fallback, not the durable arm, and the lane is runner-validated rather than sims-registered, so its coverage recurs only when someone runs it. **A second scenario `android_durable_reaction_background_connected` (`TC-DURABLE-DIRECT-REACTION`) was added 2026-08-20** to grade the durable arm itself: it BACKGROUNDS the recipient instead of killing it, records a measured `recipientProcessAliveBeforeReaction`, and fails closed unless the recipient's own log shows a durable `direct_reaction` show. **It is RED on device today and that is the finding, not a broken lane** — see **G29** in §4.12a.
...
**The structural point (updated):** host proves the *decision*, and the disposition contract now makes the device tier assert it. The device tier has not yet shown anything for the sound smoke — the harness-start blocker (G9) is fixed as of 2026-08-17 (§4.14), and the first full S1–S16 campaign run is the remaining step.

---

## 4. Gap register — what no e2e verifies on the device pair

Read §4.1 and §4.13 together: Plan 378 closed six gaps *as work*. The harness-start blocker **G9** is FIXED (2026-08-17, see §4.14) — both roles now complete device setup — but the full S1–S16 campaign has not yet been executed, so nothing in §4.1 is claimed as device-proven yet.
...
**Verified before filing here** (4 evidence strands + 3 adversarial lenses, 2026-08-18). One lens refuted a wholesale move: G17's statement was written wider than what was fixed, so the mechanism closure is recorded below and the three sub-claims that did NOT close were carved out to **G21** (first-wake storage deadline) and **G22** (1:1 device evidence), which stay in §4.13, with **G19** covering the killed-path group-text hole — G19 itself is now CLOSED by Plan 384 (§4.6). Read this row together with those three.
...
in §4.13, found by this plan's producer census and explicitly out of its scope.
...
was simply not being run. This is the same class as **G13** (§4.14): a stale pin inside `sims-contracts`
...
not fix. That red is now **G28** (§4.13): it is a behavior decision, not gate hygiene.
...
plan owns**, by the cold-wake storage deferral — see **G21** in §4.13, whose row now records the measurement.
...
first wake losing its alert — is unchanged and stays in §4.13; the plan's own hard rule is *"do not add
...
§4.13. What closed is that the reaction lane no longer puts a graded push in the un-gradeable
...
still lost when it trips; alerting stays with the dropped-push-recovery activation wave (§4.13). Note
...
### 4.12 Closed by Plan 391 (device tier, 2026-08-20) — G22(a), the killed-path 1:1 reaction, and the durable-arm residual it left (G29, device tier, same day)

[Plan 391](../Test-Flight-Improv/391-killed-path-1to1-reaction-device-proof-tdd-plan.md) —
EXECUTED, 4 causal host rows + 3 sentinels + one device leg, 8 of 8 mutations re-red.
...
**Device proof.** `dart run integration_test/scripts/run_1to1_reaction_notification_device.dart
...
artifact it had just written. `android_typed_reaction_smoke.json` @ `50898bf8e`, relay v1.9.0:
...
`unreadLifecycle.unreadCounts [0,1,1,2,0]`. Gates: `groups` 4586 Flutter + Go exit 0, support suite
112/112, `completeness-check` 1469/1469. Zero `lib/` diff.

**What this leg does NOT prove — read with G21.** The graded push produced its card through the
...
is the only thing §4.13 tracks. The product decision it leaves is named at the end of this block.
...
isolate closing SQLCipher under the live runtime, stays open as **G30** in §4.13.
...
### 4.14 Defects found and fixed during Plan 378 execution (recorded so they are not re-derived)
