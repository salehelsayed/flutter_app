#!/bin/bash
# Deploy relay v1.7.4 (plan 309 D1b per-event apns-collapse-id) — PINNED TOOLCHAIN RULE (go1.25.0), embedded
# check, backup, 90s stability window, TC-08 baselines, plan-315 wake check.
set -euo pipefail
REPO="$(cd "$(dirname "$0")/.." && pwd)"
KEY="$REPO/se.pem"
HOST=ubuntu@13.60.15.36
OUT="$REPO/docker-ws/deploy_relay_v174_result.txt"

{
  echo "=== v1.7.4 deploy started $(date -u '+%Y-%m-%dT%H:%M:%SZ') ==="
  cd "$REPO/go-relay-server"
  GOTOOLCHAIN=go1.25.0 GOOS=linux GOARCH=amd64 go build -o relay-server-linux-v174 .
  echo "--- embedded toolchain (must be go1.25.0) ---"
  go version relay-server-linux-v174
  shasum -a 256 relay-server-linux-v174

  scp -i "$KEY" -o StrictHostKeyChecking=accept-new relay-server-linux-v174 "$HOST":/tmp/relay-server-v174
  ...
  ssh -i "$KEY" -o StrictHostKeyChecking=accept-new "$HOST" '
    ...
    sudo cp /usr/local/bin/relay-server /usr/local/bin/relay-server.pre-d1b-$STAMP
    sudo install -m0755 /tmp/relay-server-v174 /usr/local/bin/relay-server
  ...
  '
  echo "=== v1.7.4 deploy finished $(date -u '+%Y-%m-%dT%H:%M:%SZ') ==="
} > "$OUT" 2>&1
