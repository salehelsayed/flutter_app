#!/usr/bin/env bash
# Plan 321 rollout STAGE 1: client to the Pixel ONLY (staged protocol — one
...
RESULT_FILE="docker-ws/deploy_pixel_only_321_result.txt"
: > "$RESULT_FILE"
...
sleep 6
adb -s "$SERIAL" shell uiautomator dump /sdcard/smoke321_a.xml >/dev/null
A=$(adb -s "$SERIAL" shell cat /sdcard/smoke321_a.xml)
case "$A" in
...
sleep 6
adb -s "$SERIAL" shell uiautomator dump /sdcard/smoke321_b.xml >/dev/null
B=$(adb -s "$SERIAL" shell cat /sdcard/smoke321_b.xml)
case "$B" in
